...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->show_equip != cp->show_equip) || (p->show_costumes != cp->show_costumes) || (p->hotkey_rowshift2 != cp->hotkey_rowshift2) ||
	...
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`hotkey_rowshift`='%d', `clan_id`='%d', `title_id`='%lu', `show_equip`='%d', `show_costumes`='%d', `hotkey_rowshift2`='%d',"
			...
			p->hotkey_rowshift, p->clan_id, p->title_id, p->show_equip, p->show_costumes, p->hotkey_rowshift2,
	...
	if( char_restock_save(char_id,p) )
		strcat(save_status, " restock");
	else
		errors++;

	// [RomuloSM]: Battleground Expansion
	if( bg_ranking_save(p->char_id, &p->bg_ranking, &cp->bg_ranking) )
		strcat(save_status, " bg_ranking");
	else
		errors++;
...
int char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count ) {
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 58, SQLDT_UINT16, &p.show_costumes, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 76, SQLDT_UINT16, &p->show_costumes, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
	...
	// [RomuloSM]: Restock
	char_restock_load(char_id, p->restock);
	StringBuf_AppendStr(&msg_buf, " restock");

	// [RomuloSM]: Battleground Expansion
	bg_ranking_load(char_id, &p->bg_ranking);
	StringBuf_AppendStr(&msg_buf, " bg_ranking");
