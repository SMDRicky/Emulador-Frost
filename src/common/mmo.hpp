...
#define MAX_QUEST_OBJECTIVES 30 ///Max quest objectives for a quest
...
#define MAX_CLAN 500
#define MAX_CLANALLIANCE 1
#ifndef MAX_BARTER_REQUIREMENTS
	#define MAX_BARTER_REQUIREMENTS 7
#endif
...
#define MAX_MEMOPOINTS 5
#define MAX_SKILLCOOLDOWN 20

//Size of the fame list arrays.
#define MAX_FAME_LIST 100
...
// [RomuloSM]: Pack Guild Enable
//#define PACK_GUILD_ENABLE
...
// A data structure in which structures of various units will be embedded
// The content defined here is in map_session_data, npc_data, mob_data, homun_data,
// Mercenary_data, ucd members in elemental_data structure will have both
struct s_unit_common_data {
	struct s_ucd_aura {
		uint32 id = 0;			// The number of auras this unit has enabled
		std::vector<std::shared_ptr<s_aura_effect>> effects;	// The combination of special effects in effect for this unit
	} aura;
};
...
// [RomuloSM]: Fake Guild
// Old Struct
struct guild_skill_f {
	int id,lv;
};

struct guild_position_f {
	char name[NAME_LENGTH];
	int mode;
	int exp_mode;
...
struct guild {
	...
	struct guild_position_f position[MAX_GUILDPOSITION];
	char mes1[MAX_GUILDMES1],mes2[MAX_GUILDMES2];
	int emblem_len,emblem_id;
	char emblem_data[2048];
	//struct guild_alliance alliance[MAX_GUILDALLIANCE];
	//struct guild_expulsion expulsion[MAX_GUILDEXPULSION];
	struct guild_skill_f skill[MAX_GUILDSKILL];
...
struct s_battleground_ranking {
	// Battle
	struct {
		unsigned int wins, loss, tied;
		unsigned int score_wins, score_loss;
	} battle;

	// Skills
	struct {
		unsigned int success, fail, damage_taken, damage_given;
		unsigned int support_success, support_fail;
	} skill;

	// Healing
	struct {
		unsigned int hp, sp, item_hp, item_sp;
	} heal;

	// Items
	struct {
		unsigned int heal, ammos, poison_bottle, fire_bottle, acid_bottle;
		unsigned int oridecon, elunium, steel, emveretarcon, wooden_block;
		unsigned int stone, yellow_gemstone, red_gemstone, blue_gemstone;
	} item;

	// Vs Players
	struct {
		unsigned int deaths, kills;
		unsigned int damage_taken, damage_given;
	} player;

	// Vs Rune Stones
	struct {
		unsigned int kills, damage, repair;
	} runestone;

	// Vs Emperium
	struct {
		unsigned int kills, damage;
	} emperium;
	...
	// Vs Flags
	struct {
		unsigned int kills, damage, capture, recapture;
	} flag;
...
struct mmo_charstatus {
...
#endif
	bool show_equip,allow_party, disable_call, show_costumes;
	short rename;
	...
	struct item restock[MAX_RESTOCK];

	// [RomuloSM]: Battleground Expansion
	struct s_battleground_ranking bg_ranking;
};
