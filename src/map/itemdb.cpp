...
const std::string LaphineCreationDatabase::getDefaultLocation(){
	return std::string( db_path ) + "/laphine_creation.yml";
}

uint64 LaphineCreationDatabase::parseBodyNode( const ryml::NodeRef& node ){
	t_itemid item_id;
	...
	std::shared_ptr<s_laphine_creation> entry = this->find( item_id );
	bool exists = entry != nullptr;

	if( !exists ){
		if( !this->nodesExist( node, { "Requirements" } ) ){
			return 0;
		}

		entry = std::make_shared<s_laphine_creation>();
		entry->item_id = item_id;
	}
	...
	if( this->nodeExists( node, "Requirements" ) ){
		for( const ryml::NodeRef& requirementNode : node["Requirements"] ){
			...
			std::shared_ptr<s_laphine_creation_requirement> requirement = util::umap_find( entry->requirements, id->nameid );
			bool requirement_exists = requirement != nullptr;

			if( !requirement_exists ){
				requirement = std::make_shared<s_laphine_creation_requirement>();
				requirement->item_id = id->nameid;
			}
			...
			if( this->nodeExists( requirementNode, "Type" ) ){
				uint16 type;

				if( !this->asUInt16( requirementNode, "Type", type ) ){
					return 0;
				}
	...
	if( this->nodeExists( node, "RandomOptionsGroup" ) ){
		for( const ryml::NodeRef& randomNode : node["RandomOptionsGroup"] ){
			std::string name;

			if( !this->asString( randomNode, "Option", name ) ){
				return 0;
			}
			...
			if( !random_option_group.option_get_id( name, id ) ){
				this->invalidWarning( randomNode["Option"], "Unknown random option group \"%s\".\n", name.c_str() );
				return 0;
			}

			if( !util::vector_exists( entry->randomOptionGroups, id ) ){
				entry->randomOptionGroups.push_back( id );
			}
		}
	}
...
LaphineCreationDatabase laphine_creation_db;

const std::string LaphineOptionsDatabase::getDefaultLocation(){
	return std::string( db_path ) + "/laphine_options.yml";
}

uint64 LaphineOptionsDatabase::parseBodyNode( const ryml::NodeRef& node ){
	...
	std::shared_ptr<s_laphine_options> entry = this->find( skill_id );
	bool exists = entry != nullptr;
	...
	if( !exists ){
		...
		entry = std::make_shared<s_laphine_options>();
	...
	if( this->nodeExists( node, "TargetItems" ) ){
		for( const ryml::NodeRef& targetNode : node["TargetItems"] ){
			...
			if( !this->asString( targetNode, "Item", name ) ){
				this->invalidWarning( targetNode["Item"], "Falhou em.\n" );
				return 0;
			}
	...
	if( !exists ){
		this->put( entry->skill_id, entry );
	}

	return 1;
}

LaphineOptionsDatabase laphine_options_db;
// ================
		...
		for( const ryml::NodeRef& slotNode : node["Slots"] ){
			...
			if( this->nodeExists( slotNode, "RandomUpgrades" ) ){
				for( const ryml::NodeRef& upgradeNode : slotNode["RandomUpgrades"] ){
					...
					if( !this->asString( upgradeNode, "Enchant", upgrade_name ) ){
						return 0;
					}
					...
					std::shared_ptr<s_item_enchant_random_upgrade> random_upgrade = util::umap_find( enchant_slot->random_upgrade.enchants, upgrade_item->nameid );
					bool random_upgrade_exists = random_upgrade != nullptr;

					if( !random_upgrade_exists ){
						if( !this->nodesExist( upgradeNode, { "Items" } ) ){
							return 0;
						}

						random_upgrade = std::make_shared<s_item_enchant_random_upgrade>();
						random_upgrade->enchant_item_id = upgrade_item->nameid;
					}
					...
					if( this->nodeExists( upgradeNode, "Items" ) ){
						for( const ryml::NodeRef& itemNode : upgradeNode["Items"] ){
							...
							if( !this->asString( itemNode, "Item", upgrade_name ) ){
								return 0;
							}
							...
							std::shared_ptr<s_item_enchant_normal_sub> enchant = util::umap_find( random_upgrade->enchants, upgrade_item->nameid );
							bool enchant_exists = enchant != nullptr;
							...
							if( !enchant_exists ){
								...
								enchant = std::make_shared<s_item_enchant_normal_sub>();
								enchant->item_id = upgrade_item->nameid;
							}
							...
							if (!enchant_exists) {
								random_upgrade->enchants[enchant->item_id] = enchant;
							}
						}
					}

					if (random_upgrade->enchants.empty()) {
					...
					if( this->nodeExists( upgradeNode, "Price" ) ){
						...
						random_upgrade->zeny = zeny;
					}else{
						if( !random_upgrade_exists ){
							random_upgrade->zeny = 0;
						}
					}

					if( !this->parseMaterials( upgradeNode, random_upgrade->materials ) ){
						return 0;
					}

					if( !random_upgrade_exists ){
						enchant_slot->random_upgrade.enchants[random_upgrade->enchant_item_id] = random_upgrade;
					}
				}
			}
...
* Read all item-related databases
*/
static void laphinedb_read(void) {
	laphine_creation_db.load();
	laphine_options_db.load();
}
...
bool itemdb_isrefable(item  *item, item_data *id){
	if(item && id){
		if(item->equip == 0 &&
			item->nameid > 0 && 
			item->identify == 1 && 
			!id->flag.no_refine && 
			!item->expire_time &&
			itemdb_saferef(id->type, item->refine, id->weapon_level) > 0
		){
			return true;
		}
	}
...
int itemdb_saferef(int item_type, int item_ref, int wlvl){
	int safe_refine = item_type == IT_ARMOR ? SAFE_REFINE_ARMOR : SAFE_REFINE_WEAPON - wlvl;
	return cap_value(safe_refine - item_ref, 0, safe_refine);
}
