...
DEFINE_PACKET_HEADER(ZC_ACK_TAKEOFF_EQUIP_ALL, 0x0bf5);
...
struct PACKET_CZ_STALL_CLOSE_FROM_GM {
	int16 packetType;
	int32 vender_id;
} __attribute__((packed));
DEFINE_PACKET_HEADER(CZ_STALL_CLOSE_FROM_GM, 0x0af9);
#endif
...
struct PACKET_ZC_STALL_CHAR_DETAILS {
	...
	char name[NAME_LENGTH];
	int16 body; // needed to display the correct class on 2024 clients
} __attribute__((packed));
