...
public:
	...
	struct s_state {
		...
		bool stall_ui_open;
		unsigned int noks : 3;
		unsigned autoattack : 1;		// [Zeph Kill Steal Protection]
		unsigned int changemap : 1;
		unsigned int callshop : 1; // flag to indicate that a script used callshop; on a shop
		short pmap; // Previous map on Map Change
		unsigned short equiphide;
		...
		uint16 laphine_option;
		uint16 laphine_optionidx;
		uint16 laphine_creation;
		...
		uint8 hidepet;
		unsigned int pvpmode : 1;	// PvP Mode
	} state;
	struct {
	...
	t_tick ks_floodprotect_tick;
	t_tick autoattack_delay;	// [Kill Steal Protection]
	...
	struct {
		struct s_battleground_ranking ranking;
		bool guild_notice;
		bool countdown_hidden;
		int listen;
		bool queue_silent;
		uint16 apply_error;
	} bge;
	...
	uint32 playerinfo_id;

	int goldpc_tid;

	// [RomuloSM]: Stall System
	struct {
		t_itemid nameid; // Extended Stall
		t_itemid itemid;
		int slots;
		short itemindex;
		int seconds;
...
static bool pc_cant_act2( map_session_data* sd ){
	return sd->state.vending || sd->state.buyingstore || (sd->sc.opt1 && sd->sc.opt1 != OPT1_BURNING)
		...
		|| sd->state.laphine_synthesis || sd->state.laphine_option || sd->state.laphine_creation || sd->state.laphine_upgrade
...
// [RomuloSM]: Count Item
int pc_countitem(map_session_data &sd, t_itemid nameid, bool ignore_expire = true, bool ignore_bound = true);
bool pc_delitem_sub(map_session_data &sd, t_itemid nameid, int amount, int type, short reason, e_log_pick_type log_type, bool ignore_expire = true, bool ignore_bound = true);
