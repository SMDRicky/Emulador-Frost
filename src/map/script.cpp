...
BUILDIN_FUNC(itemselectmenu)
{
	TBL_PC* sd;
	if (!script_rid2sd(sd))		// No associated players
		return SCRIPT_CMD_SUCCESS;

	int fd = sd->fd;
	if (!session_isActive(fd))	// Invalid web session
		return SCRIPT_CMD_SUCCESS;

	if (!script_hasdata(st, 2))	// No parameters
		return SCRIPT_CMD_SUCCESS;

	if (sd->menuskill_id)		// Interface has been opened
		return SCRIPT_CMD_SUCCESS;
	...
	if (!sd->state.menu_or_input) {		// The menu state has not been used

		int nameid, c, i;
		std::shared_ptr<item_data> itemid;

		// Start building a list of items
		c = script_lastdata(st) - 1;	// Calculate the number of prop parameters
		WFIFOHEAD(fd, 6 + c * 4);		// I have no idea
		WFIFOW(fd, 0) = 0x25a;			// Interface type
		WFIFOW(fd, 2) = 6 + c * 4;		// Data length

		// Start checking item numbers
		for (i = 0; i < c; i++) {
			nameid = script_getnum(st, i + 2);	// Get item number
			itemid = item_db.find(nameid);		// Check the item number
			if (itemid == nullptr) {			// Wrong item number
				ShowError("buildin_itemselectmenu: Nonexistant item %u requested.\n", nameid);
				return SCRIPT_CMD_FAILURE;
			}
			WFIFOL(fd, 6 + i * 4) = nameid;		// Items to add to the list
		}
		WFIFOSET(fd, WFIFOW(fd, 2));	// Output list content
		sd->menuskill_val2 = st->oid;	// Record NPC GIDs
		st->state = RERUNLINE;			// Set script execution status
		sd->state.menu_or_input = 1;	// Set menu usage status
	}
	else
	{
		sd->state.menu_or_input = 0;		// Set menu usage status
		script_pushint(st, sd->npc_amount);	// Return value(item number)
		sd->menuskill_val2 = 0;				// Removed NPC GID records
		st->state = RUN;					// Set script execution status
	}
	return SCRIPT_CMD_SUCCESS;
}
...
/*==========================================
 * Refine all inventory items to the safe level
 * saferefitems <char_id>
 *------------------------------------------*/
BUILDIN_FUNC(saferefitems) {
	short i, up = 1;
	TBL_PC *sd;
	int count = 0, saferef = 0;
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		...
		if(itemdb_isrefable(item, id)) {
			saferef = itemdb_saferef(id->type, item->refine, id->weapon_level);
			if(saferef > 0){

				log_pick_pc(sd, LOG_TYPE_SCRIPT, 1, item);

				item->refine += saferef;
				clif_refine(sd->fd, 0, i, item->refine);
				clif_additem(sd,i,0,0);
				count++;
			}
		}
	}
...
/*==========================================
 * Get cost for refinimg player current inventory to safe limit
 * Return by @type :
 *	REFINECOST_ZENY
 *	REFINECOST_ORIDECON
 *	REFINECOST_ELUNIUM
 *	REFINECOST_PHRACON
 *	REFINECOST_EMVERETARCON
 * saferefitemscost (<type>{,<char_id>})
 *------------------------------------------*/
BUILDIN_FUNC(saferefitemscost) {
	short i;
	TBL_PC *sd;
	int refcost = 0, type, finalref = 0;
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		...
		struct item_data *id = sd->inventory_data[i];
		if(itemdb_isrefable(item, id)) {
			finalref = itemdb_saferef(id->type, item->refine, id->weapon_level);
			if(finalref > 0){
				...
				std::shared_ptr<s_refine_cost> cost = util::umap_find( level_info->costs, (uint16)REFINE_COST_NORMAL );

				if( cost == nullptr ){
					continue;
				}

				if(type == REFINECOST_ZENY) {
					refcost += finalref * cost->zeny;
				} else {
					if(cost->nameid != type){
						continue;
					}
	...
	script_pushint( st, refcost );
	return SCRIPT_CMD_SUCCESS;
}

/*==========================================
 * Get the number of refinable items
 * saferefitemsamount <char_id>
 *------------------------------------------*/
BUILDIN_FUNC(saferefcnt) {
	int count = 0, i = 0;
	TBL_PC *sd;
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		if(itemdb_isrefable(&sd->inventory.u.items_inventory[i], sd->inventory_data[i])){
			count+= 1;
		}
	}
...
BUILDIN_FUNC(inviteguild) {
    int tsd_id = script_getnum(st, 2); // ID do jogador que receberá o convite
    int guild_id = script_getnum(st, 3); // ID da guilda

    TBL_PC *tsd;
    std::shared_ptr<MapGuild> g;


    if (!(tsd = map_charid2sd(tsd_id))) {
        ShowDebug("inviteguild: Jogado com ID %d não está online\n", tsd_id);
        return 0;
    }

    if (!(g = guild_search(guild_id))) {
        ShowDebug("inviteguild: Guild com ID %d não encontrada\n", guild_id);
        return 0;
    }

    const mmo_guild &guild_ref = g->guild;
    tsd->guild_invite = guild_id;
    tsd->guild_invite_account = 0; // Não precisa armazenar a conta de quem enviou o convite, deixei somente pra evitar bug

    // Envia o convite ao jogador
    clif_guild_invite(*tsd, guild_ref);
    return 0;
}
	...
	switch(id){
		case 1:	// Check if skill is available for auto buff
			...
			if (!skill)
				...
				&& (
						(type == CAST_NODAMAGE && skill->inf&(INF_SUPPORT_SKILL|INF_SELF_SKILL)
						&& skill_get_sc(skill->nameid) > 0)
						|| skill_id == MO_CALLSPIRITS
						|| skill_id == CH_SOULCOLLECT
						|| skill_id == SA_AUTOSPELL
					)
				)
				skill_status = true;
		...
		case 2:	// Check if skill is available for auto attack
			...
			if (!skill)
				return skill_status;
			else if (skill && skill->skill_type != BF_NONE && skill->inf != INF_PASSIVE_SKILL && !skill_get_nk(skill_id, NK_NODAMAGE) && (type != CAST_NODAMAGE || skill->inf & INF_SELF_SKILL))
				skill_status = true;
...
BUILDIN_FUNC( autoattack_fromitem ){
    ...
    if( sd->itemid == 0 ){
        ShowError( "autoattack_fromitem: Called outside of an item script without item id.\n" );
        return SCRIPT_CMD_SUCCESS;
    }

    if( sd->inventory.u.items_inventory[sd->itemindex].expire_time == 0 ){
        ShowError( "autoattack_fromitem: Called from item %u, which is not a consumed delayed.\n", sd->itemid );
        return SCRIPT_CMD_SUCCESS;
    }

    t_tick duration_ = DIFF_TICK(sd->inventory.u.items_inventory[sd->itemindex].expire_time,time(NULL));
...
BUILDIN_FUNC(getautoattackstring)
{
	...
	switch(id){
				...
				if(sd->aa.autopotion.size() > 0){
					for(auto &itAutopotion : sd->aa.autopotion){
						...
						if(itAutopotion.min_hp)
							buf += " - HP < " + std::to_string(itAutopotion.min_hp) + "%";
						if(itAutopotion.min_sp)
							buf += " - SP < " + std::to_string(itAutopotion.min_sp) + "%";
						buf += "\n";
					}
				}
			}
			...
			switch(index){
				case 0:
					if(sd->aa.autositregen.is_active)
						buf += std::string("^FF0000(Enabled)^000000 \n");
					else
						buf += std::string("^FF0000(Disabled)^000000 \n");
					break;
				case 1:
					if(sd->aa.autositregen.min_hp > 0)
						buf += "HP - Sit if hp < " + std::to_string(sd->aa.autositregen.min_hp) + "% - Stand if hp >= " + std::to_string(sd->aa.autositregen.max_hp) + "% \n";
					break;
				case 2:
					if(sd->aa.autositregen.min_sp)
						buf += "SP - Sit if sp < " + std::to_string(sd->aa.autositregen.min_sp) + "% - Stand if sp >= " + std::to_string(sd->aa.autositregen.max_sp) + "% \n";
					break;
			}
			...
			if(index == -1){ // Show all attack skills available
				for(i=0;i<MAX_SKILL;i++){
					if(sd->status.skill[i].id > 0 && sd->status.skill[i].lv > 0){
						...
						e_cast_type type = skill_get_casttype(sd->status.skill[i].id);
						//Debug skill shows
						/*if(sd->status.skill[i].id == MO_CHAINCOMBO){
							ShowError("skill id %d skill->desc %s \n",sd->status.skill[i].id, skill->desc);
							ShowError("skill->skill_type != BF_NONE  %d \n",skill->skill_type != BF_NONE );
							ShowError("skill->inf != INF_PASSIVE_SKILL   %d \n",skill->inf != INF_PASSIVE_SKILL );
							ShowError("!skill_get_nk(sd->status.skill[i].id, NK_NODAMAGE) %d \n",(!skill_get_nk(sd->status.skill[i].id, NK_NODAMAGE)) );
							ShowError("(type != CAST_NODAMAGE || skill->inf & INF_SELF_SKILL) %d \n",(type != CAST_NODAMAGE || skill->inf & INF_SELF_SKILL));
							ShowError("type %d \n",type);
						}*/
						if (skill 
							&& skill->skill_type != BF_NONE 
			...
			if(index > 0){
				...
				if(item_data->type == IT_USABLE){ // check if item is type usable
					...
					if(itAutobuffitem != sd->aa.autobuffitems.end()){
						...
						buf += std::string("[ON] - ^FF0000(") + std::to_string(index) + ")^000000 [" + item_data->name.c_str() + "] - Delay : " + std::to_string(autobuffitem.delay/1000) + "s";
					} else {
						buf += std::string("[OFF] - ^FF0000(") + std::to_string(index) + ")^000000 [" + item_data->name.c_str() + "]";					
					}
				}
			}
		...
		case 6:	// Melee attack
			if(index == 0){
				if(!sd->aa.stopmelee)
					buf += std::string("[ON] - Melee attack");
				else
					buf += std::string("[OFF] - Melee attack");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 7:	// Teleport
			if(index == 0){
				if(!sd->aa.teleport.use_teleport)
					buf += std::string("[ON] - Teleport Skill \n");
				else
					buf += std::string("[OFF] - Teleport Skill \n");
				
				if(!sd->aa.teleport.use_flywing)
					buf += std::string("[ON] - Fly Wing \n");
				else
					buf += std::string("[OFF] - Fly Wing \n");
				
				if(sd->aa.teleport.min_hp)
					buf += std::string("Emergency espace if hit when HP < ") + std::to_string(sd->aa.teleport.min_hp) + std::string("% \n");
				if(sd->aa.teleport.delay_nomobmeet)
					buf += std::string("TP if no monster attacked after ") + std::to_string(sd->aa.teleport.delay_nomobmeet) + std::string("s \n");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 9:	// Items selection
			if(index == 0){
				switch(sd->aa.prio_item_config){
					case 0:
						buf += std::string("Priorize fight \n");
						break;
					case 1:
						buf += std::string("Priorize loot \n");
						break;
				}
				switch(sd->aa.pickup_item_config){
					case 0:
						buf += std::string("Pick up every item on the floor \n");
						break;
					case 1:
						buf += std::string("Pick up only items on the list \n");
						break;
					case 2:
						buf += std::string("Don't pick up any item \n");
						break;	
				}
		...
		switch(id){
			...
			case 3:
				if(result.size() == 4){ // id = 3 - autobuffskills (is_active;skill_id;skill_lv)
					...
					skill = skill_db.find(skill_id);
					e_cast_type type = skill_get_casttype(skill_id);
					if ((!skill || type != CAST_NODAMAGE || !(skill->inf&(INF_SUPPORT_SKILL|INF_SELF_SKILL)) || skill_get_sc(skill->nameid) == 0) && (skill_id != 261 || skill_id != 401))
						return SCRIPT_CMD_SUCCESS;
				...
				if(result.size() == 3){ // id = 9 - item pickup (is_active;item_id)
					...
					switch(status){
						...
						case 2:
							...
							break;
						case 3:
							sd->aa.prio_item_config = (int)nameid;
							break;
					}
				}
				break;
...
BUILDIN_FUNC(monster)
{
	...
	for(i = 0; i < amount; i++) { //not optimised
		int mobid = mob_once_spawn(sd, m, x, y, str, class_, 1, event, size, ai, monster_chapion);
...
static int mob_count(struct block_list *bl,va_list ap)
{
    int mob_id = va_arg(ap, int);    
    struct mob_data *md = ((struct mob_data *)bl);	
    
    if(md->mob_id == mob_id ) return 1;
    
    return SCRIPT_CMD_SUCCESS;
}
/*==========================================
*    mobalive(mob id, map name)
*------------------------------------------*/
BUILDIN_FUNC(mobalive)    // Added by RoVeRT
{
    const char *mapname;
    int mob_id;
    int16 m;
    ...
    script_pushint(st,map_foreachinmap(mob_count, m, BL_MOB, mob_id));
    return SCRIPT_CMD_SUCCESS;
}
//======
...
// ------------------------------------------------------ -- 
// Função iconefalso
// iconefalso(CHAR_ID,ICON_ID,TIME,STATE)
// ------------------------------------------------------ -- 
BUILDIN_FUNC(iconefalso)
{
...
#define getunitdata_sub(idx__,var__) setd_sub_num(st,sd,name,(idx__),(var__),data->ref)
	...
	switch(bl->type) {
		case BL_MOB:
			...
			// [RomuloSM]: Player Bot
			getunitdata_sub(UMOB_DEADSIT, md->vd->dead_sit);
			getunitdata_sub(UMOB_PLAYER_BOT, md->player_bot);
			getunitdata_sub(UMOB_JOBLEVEL, md->job_level);
			break;
		...
		case BL_STALL: {
			...
			ShowWarning("buildin_getunitdata: Stall in work, wait development please!\n");
			} break;
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
		...
		switch (type) {
			...
			case UMOB_PLAYER_BOT: md->player_bot = value ? true : false; unit_refresh(bl,true); break;
			case UMOB_JOBLEVEL: md->job_level = (unsigned short)value; clif_name_area(&md->bl); break;
			default:
...
BUILDIN_FUNC(waitingroom2bg)
{
	...
	team.warp_x = script_getnum(st,4);
	team.warp_y = script_getnum(st,5);
	if (script_hasdata(st,6)) {
	...
	for (i = 0; i < cd->users; i++) { // Only add those who are in the chat room
		map_session_data *sd;
		if( (sd = cd->usersd[i]) != NULL && bg_team_join(bg_id, sd, false, true) ){
...
BUILDIN_FUNC(waitingroom2bg_single)
{
	...
	if( bg_team_join(bg_id, sd, false, true) && pc_setpos(sd, mapindex, x, y, CLR_TELEPORT) == SETPOS_OK)
	{
		script_pushint(st, true);
	}
	else
...
BUILDIN_FUNC(bg_create) {
	...
	teamId = script_getnum(st,5);

	if( teamId < 0 || teamId > 1 ) {
		ShowError("Invalid Team Id '%d'.\n", teamId);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	else {
...
BUILDIN_FUNC(bg_leave)
{
	...
	bg_team_leave(sd, false, deserter, (enum bg_team_leave_type)flag);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(bg_get_data)
{
	...
	if (bg) {
		...
		case 1:
			...
			break;
		case 2:
			script_pushint(st, bg->expansion.teamId);
			break;
		case 3:
			script_pushint(st, bg->expansion.master_id);
			break;
		default:
 ...
 * setticketuniqueid
 *------------------------------------------*/
BUILDIN_FUNC(setticketuniqueid)
{
	class map_session_data *sd;
	...
	if( sd->itemid == 0 ){
		ShowError( "setticketuniqueid: Chamado fora de um script de item sem id de item.\n" );
		return SCRIPT_CMD_FAILURE;
	}

	if( !sd->inventory_data[sd->itemindex]->flag.guid ){
		ShowError( "setticketuniqueid: Chamado do item %u, que não tem um id exclusivo.\n", sd->itemid );
		return SCRIPT_CMD_FAILURE;
	}
	
	sd->ticketuniqueid = sd->inventory.u.items_inventory[sd->itemindex].unique_id;
 ...
 * getticketuniqueid
 *------------------------------------------*/
BUILDIN_FUNC(getticketuniqueid)
{
	class map_session_data *sd;
	...
	script_pushint(st, sd->ticketuniqueid);
 ...
 * kickplayer
 *------------------------------------------*/
BUILDIN_FUNC(kickplayer)
{
	class map_session_data *sd, *tsd;

	if( !script_rid2sd(sd) )
		return SCRIPT_CMD_SUCCESS;
	
	tsd = map_charid2sd(script_getnum(st, 2));
	if( tsd != NULL && tsd->fd )
...
BUILDIN_FUNC(buyingstore)
{
	...
	if( battle_config.extended_buyingstore_enable )
		clif_buyingstore(sd, 1, script_getnum(st,2));
	else
...
// Checa se o Jogador est\E1 vivo ou morto.
//
// * getcharisdead(<name>)
...
BUILDIN_FUNC(bg_reward_list)
{
	int j = 0, type = -1, bg_id = script_getnum(st,2);
	std::shared_ptr<s_battleground_reward> bgr;

	if( (bgr = bg_reward_search(bg_id)) == nullptr ) {
		ShowWarning("script: bg_reward_list: Battleground Reward ID '%d' not found.\n", bg_id);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	if( type < BGRW_ALL || type > BGRW_DRAW ) {
		ShowWarning("script: bg_reward_list: invalid type '%d'.\n", type);
		return SCRIPT_CMD_SUCCESS;
	}

	for (const auto &rewards : bgr->reward_items) {
		if( type != BGRW_ALL && type != rewards.type )
			continue;

		setd_sub_num( st, NULL, ".@reward_nameid", j, rewards.nameid, NULL );
		setd_sub_num( st, NULL, ".@reward_type", j, rewards.type, NULL );
		setd_sub_str( st, NULL, ".@reward_name$", j, "", NULL );
		setd_sub_str( st, NULL, ".@reward_desc$", j, "", NULL );
		setd_sub_num( st, NULL, ".@reward_amount", j, rewards.amount, NULL );
		setd_sub_num( st, NULL, ".@reward_hamount", j, rewards.happy_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_max_amount", j, rewards.max_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_rate", j, rewards.rate, NULL );
		setd_sub_num( st, NULL, ".@reward_hrate", j, rewards.happy_rate, NULL );
		setd_sub_num( st, NULL, ".@reward_pamount", j, rewards.premium_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_phamount", j, rewards.premium_happy_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_pmax_amount", j, rewards.premium_max_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_prate", j, rewards.premium_rate, NULL );
		setd_sub_num( st, NULL, ".@reward_phrate", j, rewards.premium_happy_rate, NULL );
		j++;
	}

	for (const auto &rewards : bgr->reward_vars) {
		if( type != -1 && type != rewards.type )
			continue;

		setd_sub_num( st, NULL, ".@reward_nameid", j, 0, NULL );
		setd_sub_num( st, NULL, ".@reward_type", j, rewards.type, NULL );
		setd_sub_str( st, NULL, ".@reward_name$", j, rewards.name.c_str(), NULL );
		setd_sub_str( st, NULL, ".@reward_desc$", j, rewards.desc.c_str(), NULL );
		setd_sub_num( st, NULL, ".@reward_amount", j, rewards.amount, NULL );
		setd_sub_num( st, NULL, ".@reward_hamount", j, rewards.happy_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_max_amount", j, rewards.max_amount, NULL );
		setd_sub_num( st, NULL, ".@reward_rate", j, rewards.rate, NULL );
	...
	setd_sub_num( st, NULL, ".@bgreward_count", 0, j, NULL );
	return SCRIPT_CMD_SUCCESS;
}

// Battleground Reward
//
// * bg_reward <bg_unique_id>, <type>, <vip>{, <char_id>};
// * bg_reward <bg_unique_id>, <type>, <vip>{, "<char_name>"};
//
//	- bg_unique_id: This ID is unique to each arena, it is set in battleground_reward_db.yml and in the arena script.
//	- type:
//		0: Winner
//		1: Loser
//		2: Draw
//	- vip: Is the Player a VIP user?
//	- char_id: Character ID.
//	- char_name: Character Name.
//
BUILDIN_FUNC(bg_reward)
{
	int i, j = 0, bg_id = script_getnum(st,2), type = script_getnum(st,3), is_vip = script_getnum(st,4), happyhour = bg_happyhour_flag;
	TBL_PC *sd;
	std::shared_ptr<s_battleground_reward> bgr;
	...
	if( (bgr = bg_reward_search(bg_id)) == nullptr ) {
		script_pushint(st, 0);
		ShowWarning("script: bg_reward: Battleground Reward ID '%d' not found.\n", bg_id);
		return SCRIPT_CMD_SUCCESS;
	}

	if( type < BGRW_ALL || type > BGRW_DRAW ) {
		script_pushint(st, 0);
		ShowWarning("script: bg_reward: invalid type '%d'.\n", type);
		return SCRIPT_CMD_SUCCESS;
	}

	// MapFlag Happyhour
	if( map_getmapflag(sd->bl.m, MF_NOBGHAPPYHOUR) )
		happyhour = 0;
	else if( map_getmapflag(sd->bl.m, MF_BGHAPPYHOUR))
		happyhour = 1;
	...
	for (const auto &rewards : bgr->reward_items) {
		...
		t_itemid nameid = rewards.nameid;
		struct item it;
		int rate, amount, max_amount;
		std::shared_ptr<item_data> id = item_db.find(nameid);

		if( id == nullptr )
			continue;

		if( is_vip && happyhour && rewards.happy_amount ) {
			rate = rewards.premium_happy_rate;
			amount = rewards.premium_happy_amount;
			max_amount = rewards.premium_max_amount;
		}
		else if( is_vip && rewards.premium_amount ) {
			rate = rewards.premium_rate;
			amount = rewards.premium_amount;
			max_amount = rewards.premium_max_amount;
		}
		else if( happyhour && rewards.happy_amount ) {
			rate = rewards.happy_rate;
			amount = rewards.happy_amount;
		...
		if( max_amount > 0 ) {
			int count = script_countitem_sub(sd->inventory.u.items_inventory, id, MAX_INVENTORY, false, st, sd);
			if( (count+amount) >= max_amount )
				amount = max_amount-count;
		}

		if( amount <= 0 || (rate != 100 && rate < rnd()%100) )
			continue;
		...
		sprintf(output, msg_txt(sd,1679), amount, id->ename.c_str());
	...
	for (const auto &rewards : bgr->reward_vars) {
		...
		int val, rate, amount, max_amount;
		...
		val = static_cast<unsigned char>(pc_readglobalreg(sd, add_str(rewards.name.c_str())));
		if( max_amount > 0 && (val+amount) >= max_amount )
			amount = max_amount-val;

		if( amount <= 0 || (rate != 100 && rate < rnd()%100) )
			continue;

		amount += val;
		pc_setglobalreg(sd, add_str(rewards.name.c_str()), amount);

		char output[CHAT_SIZE_MAX];
		sprintf(output, msg_txt(sd,1679), (amount-val), rewards.desc.c_str());
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		j++;
	}
...
//	- flag: 1 On / 0 Off
//
BUILDIN_FUNC(bg_happyhour)
{
	int type = script_getnum(st,2);
	int flag = type ? bg_happyhour_start() : bg_happyhour_end();
...
// * bg_happyhour_status();
//
BUILDIN_FUNC(bg_happyhour_status)
{
	script_pushint(st,bg_happyhour_flag);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(bg_flagemblem)
{
	...
	} else if( bg_id && !(bg = util::umap_find(bg_team_db, bg_id)) ) {
		ShowError("script:bg_flagemblem: Battleground (%d) does not exist!\n", bg_id);
	} else {
		nd->bg_id = bg_id;
		clif_guild_emblem_area(&nd->bl);
	}
...
/// Shows a time counter to the players on the team.
///
/// * bg_setcountdown <bg_id>, <seconds>;
///
BUILDIN_FUNC(bg_setcountdown)
{
	int bg_id = script_getnum(st,2), second = script_getnum(st,3);
	std::shared_ptr<s_battleground_data> bg = util::umap_find(bg_team_db, bg_id);
...
/// Checks if there is duplicate IP or Gepard UniqueID in a queue or a Battle Arena.
///
/// * bg_check_dual({<char_id>});
///
BUILDIN_FUNC(bg_check_dual)
{
	TBL_PC *sd;
...
/// Gets current Battleground global ranking information.
///
/// * bg_ranking_get <type>{, <char_id>};
///
BUILDIN_FUNC(bg_ranking_get)
{
	TBL_PC* sd = NULL;
	...
	switch (type)
	{
	case BGR_WINS:             value = sd->status.bg_ranking.battle.wins; break;
	case BGR_LOSS:             value = sd->status.bg_ranking.battle.loss; break;
	case BGR_TIED:             value = sd->status.bg_ranking.battle.tied; break;
	case BGR_SCORE_WINS:       value = sd->status.bg_ranking.battle.score_wins; break;
	case BGR_SCORE_LOSS:       value = sd->status.bg_ranking.battle.score_loss; break;
	case BGR_SKILLSUP_SUCCESS: value = sd->status.bg_ranking.skill.support_success; break;
	case BGR_SKILLSUP_FAIL:    value = sd->status.bg_ranking.skill.support_fail; break;
	case BGR_SKILLUSE_SUCCESS: value = sd->status.bg_ranking.skill.success; break;
	case BGR_SKILLUSE_FAIL:    value = sd->status.bg_ranking.skill.fail; break;
	case BGR_SKILLDMG_GIVEN:   value = sd->status.bg_ranking.skill.damage_given; break;
	case BGR_SKILLDMG_TAKEN:   value = sd->status.bg_ranking.skill.damage_taken; break;
	case BGR_HEAL_HP:          value = sd->status.bg_ranking.heal.hp; break;
	case BGR_HEAL_SP:          value = sd->status.bg_ranking.heal.sp; break;
	case BGR_ITEMHEAL_HP:      value = sd->status.bg_ranking.heal.item_hp; break;
	case BGR_ITEMHEAL_SP:      value = sd->status.bg_ranking.heal.item_sp; break;
	case BGR_ITEMHEAL:         value = sd->status.bg_ranking.item.heal; break;
	case BGR_AMMO:             value = sd->status.bg_ranking.item.ammos; break;
	case BGR_POISON_BOTTLE:    value = sd->status.bg_ranking.item.poison_bottle; break;
	case BGR_FIRE_BOTTLE:      value = sd->status.bg_ranking.item.fire_bottle; break;
	case BGR_ACID_BOTTLE:      value = sd->status.bg_ranking.item.acid_bottle; break;
	case BGR_ORIDECON:         value = sd->status.bg_ranking.item.oridecon; break;
	case BGR_ELUNIUM:          value = sd->status.bg_ranking.item.elunium; break;
	case BGR_STEEL:            value = sd->status.bg_ranking.item.steel; break;
	case BGR_EMVERETARCON:     value = sd->status.bg_ranking.item.emveretarcon; break;
	case BGR_WOODEN_BLOCK:     value = sd->status.bg_ranking.item.wooden_block; break;
	case BGR_STONE:            value = sd->status.bg_ranking.item.stone; break;
	case BGR_YELLOW_GEMSTONE:  value = sd->status.bg_ranking.item.yellow_gemstone; break;
	case BGR_RED_GEMSTONE:     value = sd->status.bg_ranking.item.red_gemstone; break;
	case BGR_BLUE_GEMSTONE:    value = sd->status.bg_ranking.item.blue_gemstone; break;
	case BGR_PLAYER_KILL:      value = sd->status.bg_ranking.player.kills; break;
	case BGR_PLAYER_DEATH:     value = sd->status.bg_ranking.player.deaths; break;
	case BGR_PLAYER_DMG_GIVEN: value = sd->status.bg_ranking.player.damage_given; break;
	case BGR_PLAYER_DMG_TAKEN: value = sd->status.bg_ranking.player.damage_taken; break;
	case BGR_RUNESTONE_KILL:   value = sd->status.bg_ranking.runestone.kills; break;
	case BGR_RUNESTONE_DMG:    value = sd->status.bg_ranking.runestone.damage; break;
	case BGR_RUNESTONE_REPAIR: value = sd->status.bg_ranking.runestone.repair; break;
	case BGR_EMPERIUM_KILL:    value = sd->status.bg_ranking.emperium.kills; break;
	case BGR_EMPERIUM_DMG:     value = sd->status.bg_ranking.emperium.damage; break;
	case BGR_BARRICADE_KILL:   value = sd->status.bg_ranking.barricade.kills; break;
	case BGR_BARRICADE_DMG:    value = sd->status.bg_ranking.barricade.damage; break;
	case BGR_BARRICADE_REPAIR: value = sd->status.bg_ranking.barricade.repair; break;
	case BGR_OBJECTIVE_KILL:   value = sd->status.bg_ranking.objective.kills; break;
	case BGR_OBJECTIVE_DMG:    value = sd->status.bg_ranking.objective.damage; break;
	case BGR_FLAG_KILL:        value = sd->status.bg_ranking.flag.kills; break;
	case BGR_FLAG_DMG:         value = sd->status.bg_ranking.flag.damage; break;
	case BGR_FLAG_CAPTURE:     value = sd->status.bg_ranking.flag.capture; break;
	case BGR_FLAG_RECAPTURE:   value = sd->status.bg_ranking.flag.recapture; break;
	case BGR_CRYSTAL_KILL:     value = sd->status.bg_ranking.crystal.kills; break;
	case BGR_CRYSTAL_DMG:      value = sd->status.bg_ranking.crystal.damage; break;
	case BGR_GUARD_KILL:       value = sd->status.bg_ranking.guardian.kills; break;
	case BGR_GUARD_DEATH:      value = sd->status.bg_ranking.guardian.deaths; break;
	case BGR_GUARD_DMG_GIVEN:  value = sd->status.bg_ranking.guardian.damage_given; break;
	case BGR_GUARD_DMG_TAKEN:  value = sd->status.bg_ranking.guardian.damage_taken; break;
	default:
		script_pushint(st, 0);
		ShowError("script:bg_ranking_get2: Unknow type (%d)\n", type);
		return SCRIPT_CMD_SUCCESS;
	}
...
/// Gets current Battleground global ranking information.
///
/// * bg_ranking_set <type>,<value>{, <char_id>};
///
BUILDIN_FUNC(bg_ranking_set)
{
	TBL_PC* sd = NULL;
	...
	switch (type)
	{
	case BGR_WINS:             sd->status.bg_ranking.battle.wins = value; break;
	case BGR_LOSS:             sd->status.bg_ranking.battle.loss = value; break;
	case BGR_TIED:             sd->status.bg_ranking.battle.tied = value; break;
	case BGR_SCORE_WINS:       sd->status.bg_ranking.battle.score_wins = value; break;
	case BGR_SCORE_LOSS:       sd->status.bg_ranking.battle.score_loss = value; break;
	case BGR_SKILLSUP_SUCCESS: sd->status.bg_ranking.skill.support_success = value; break;
	case BGR_SKILLSUP_FAIL:    sd->status.bg_ranking.skill.support_fail = value; break;
	case BGR_SKILLUSE_SUCCESS: sd->status.bg_ranking.skill.success = value; break;
	case BGR_SKILLUSE_FAIL:    sd->status.bg_ranking.skill.fail = value; break;
	case BGR_SKILLDMG_GIVEN:   sd->status.bg_ranking.skill.damage_given = value; break;
	case BGR_SKILLDMG_TAKEN:   sd->status.bg_ranking.skill.damage_taken = value; break;
	case BGR_HEAL_HP:          sd->status.bg_ranking.heal.hp = value; break;
	case BGR_HEAL_SP:          sd->status.bg_ranking.heal.sp = value; break;
	case BGR_ITEMHEAL_HP:      sd->status.bg_ranking.heal.item_hp = value; break;
	case BGR_ITEMHEAL_SP:      sd->status.bg_ranking.heal.item_sp = value; break;
	case BGR_ITEMHEAL:         sd->status.bg_ranking.item.heal = value; break;
	case BGR_AMMO:             sd->status.bg_ranking.item.ammos = value; break;
	case BGR_POISON_BOTTLE:    sd->status.bg_ranking.item.poison_bottle = value; break;
	case BGR_FIRE_BOTTLE:      sd->status.bg_ranking.item.fire_bottle = value; break;
	case BGR_ACID_BOTTLE:      sd->status.bg_ranking.item.acid_bottle = value; break;
	case BGR_ORIDECON:         sd->status.bg_ranking.item.oridecon = value; break;
	case BGR_ELUNIUM:          sd->status.bg_ranking.item.elunium = value; break;
	case BGR_STEEL:            sd->status.bg_ranking.item.steel = value; break;
	case BGR_EMVERETARCON:     sd->status.bg_ranking.item.emveretarcon = value; break;
	case BGR_WOODEN_BLOCK:     sd->status.bg_ranking.item.wooden_block = value; break;
	case BGR_STONE:            sd->status.bg_ranking.item.stone = value; break;
	case BGR_YELLOW_GEMSTONE:  sd->status.bg_ranking.item.yellow_gemstone = value; break;
	case BGR_RED_GEMSTONE:     sd->status.bg_ranking.item.red_gemstone = value; break;
	case BGR_BLUE_GEMSTONE:    sd->status.bg_ranking.item.blue_gemstone = value; break;
	case BGR_PLAYER_KILL:      sd->status.bg_ranking.player.kills = value; break;
	case BGR_PLAYER_DEATH:     sd->status.bg_ranking.player.deaths = value; break;
	case BGR_PLAYER_DMG_GIVEN: sd->status.bg_ranking.player.damage_given = value; break;
	case BGR_PLAYER_DMG_TAKEN: sd->status.bg_ranking.player.damage_taken = value; break;
	case BGR_RUNESTONE_KILL:   sd->status.bg_ranking.runestone.kills = value; break;
	case BGR_RUNESTONE_DMG:    sd->status.bg_ranking.runestone.damage = value; break;
	case BGR_RUNESTONE_REPAIR: sd->status.bg_ranking.runestone.repair = value; break;
	case BGR_EMPERIUM_KILL:    sd->status.bg_ranking.emperium.kills = value; break;
	case BGR_EMPERIUM_DMG:     sd->status.bg_ranking.emperium.damage = value; break;
	case BGR_BARRICADE_KILL:   sd->status.bg_ranking.barricade.kills = value; break;
	case BGR_BARRICADE_DMG:    sd->status.bg_ranking.barricade.damage = value; break;
	case BGR_BARRICADE_REPAIR: sd->status.bg_ranking.barricade.repair = value; break;
	case BGR_OBJECTIVE_KILL:   sd->status.bg_ranking.objective.kills = value; break;
	case BGR_OBJECTIVE_DMG:    sd->status.bg_ranking.objective.damage = value; break;
	case BGR_FLAG_KILL:        sd->status.bg_ranking.flag.kills = value; break;
	case BGR_FLAG_DMG:         sd->status.bg_ranking.flag.damage = value; break;
	case BGR_FLAG_CAPTURE:     sd->status.bg_ranking.flag.capture = value; break;
	case BGR_FLAG_RECAPTURE:   sd->status.bg_ranking.flag.recapture = value; break;
	case BGR_CRYSTAL_KILL:     sd->status.bg_ranking.crystal.kills = value; break;
	case BGR_CRYSTAL_DMG:      sd->status.bg_ranking.crystal.damage = value; break;
	case BGR_GUARD_KILL:       sd->status.bg_ranking.guardian.kills = value; break;
	case BGR_GUARD_DEATH:      sd->status.bg_ranking.guardian.deaths = value; break;
	case BGR_GUARD_DMG_GIVEN:  sd->status.bg_ranking.guardian.damage_given = value; break;
	case BGR_GUARD_DMG_TAKEN:  sd->status.bg_ranking.guardian.damage_taken = value; break;
	default:
		script_pushint(st, 0);
		ShowError("script:bg_ranking_set: Unknow type (%d)\n", type);
		return SCRIPT_CMD_SUCCESS;
	}
...
/// Gets current Battleground temporary ranking information.
/// This temporary ranking is responsible for saving information from separate arenas.
///
/// * bg_ranking_get2 <type>{, <char_id>};
///
BUILDIN_FUNC(bg_ranking_get2)
{
	TBL_PC* sd = NULL;
	int type = script_getnum(st, 2), value = 0;
	...
	switch (type)
	{
	case BGR_WINS:             value = sd->bge.ranking.battle.wins; break;
	case BGR_LOSS:             value = sd->bge.ranking.battle.loss; break;
	case BGR_TIED:             value = sd->bge.ranking.battle.tied; break;
	case BGR_SCORE_WINS:       value = sd->bge.ranking.battle.score_wins; break;
	case BGR_SCORE_LOSS:       value = sd->bge.ranking.battle.score_loss; break;
	case BGR_SKILLSUP_SUCCESS: value = sd->bge.ranking.skill.support_success; break;
	case BGR_SKILLSUP_FAIL:    value = sd->bge.ranking.skill.support_fail; break;
	case BGR_SKILLUSE_SUCCESS: value = sd->bge.ranking.skill.success; break;
	case BGR_SKILLUSE_FAIL:    value = sd->bge.ranking.skill.fail; break;
	case BGR_SKILLDMG_GIVEN:   value = sd->bge.ranking.skill.damage_given; break;
	case BGR_SKILLDMG_TAKEN:   value = sd->bge.ranking.skill.damage_taken; break;
	case BGR_HEAL_HP:          value = sd->bge.ranking.heal.hp; break;
	case BGR_HEAL_SP:          value = sd->bge.ranking.heal.sp; break;
	case BGR_ITEMHEAL_HP:      value = sd->bge.ranking.heal.item_hp; break;
	case BGR_ITEMHEAL_SP:      value = sd->bge.ranking.heal.item_sp; break;
	case BGR_ITEMHEAL:         value = sd->bge.ranking.item.heal; break;
	case BGR_AMMO:             value = sd->bge.ranking.item.ammos; break;
	case BGR_POISON_BOTTLE:    value = sd->bge.ranking.item.poison_bottle; break;
	case BGR_FIRE_BOTTLE:      value = sd->bge.ranking.item.fire_bottle; break;
	case BGR_ACID_BOTTLE:      value = sd->bge.ranking.item.acid_bottle; break;
	case BGR_ORIDECON:         value = sd->bge.ranking.item.oridecon; break;
	case BGR_ELUNIUM:          value = sd->bge.ranking.item.elunium; break;
	case BGR_STEEL:            value = sd->bge.ranking.item.steel; break;
	case BGR_EMVERETARCON:     value = sd->bge.ranking.item.emveretarcon; break;
	case BGR_WOODEN_BLOCK:     value = sd->bge.ranking.item.wooden_block; break;
	case BGR_STONE:            value = sd->bge.ranking.item.stone; break;
	case BGR_YELLOW_GEMSTONE:  value = sd->bge.ranking.item.yellow_gemstone; break;
	case BGR_RED_GEMSTONE:     value = sd->bge.ranking.item.red_gemstone; break;
	case BGR_BLUE_GEMSTONE:    value = sd->bge.ranking.item.blue_gemstone; break;
	case BGR_PLAYER_KILL:      value = sd->bge.ranking.player.kills; break;
	case BGR_PLAYER_DEATH:     value = sd->bge.ranking.player.deaths; break;
	case BGR_PLAYER_DMG_GIVEN: value = sd->bge.ranking.player.damage_given; break;
	case BGR_PLAYER_DMG_TAKEN: value = sd->bge.ranking.player.damage_taken; break;
	case BGR_RUNESTONE_KILL:   value = sd->bge.ranking.runestone.kills; break;
	case BGR_RUNESTONE_DMG:    value = sd->bge.ranking.runestone.damage; break;
	case BGR_RUNESTONE_REPAIR: value = sd->bge.ranking.runestone.repair; break;
	case BGR_EMPERIUM_KILL:    value = sd->bge.ranking.emperium.kills; break;
	case BGR_EMPERIUM_DMG:     value = sd->bge.ranking.emperium.damage; break;
	case BGR_BARRICADE_KILL:   value = sd->bge.ranking.barricade.kills; break;
	case BGR_BARRICADE_DMG:    value = sd->bge.ranking.barricade.damage; break;
	case BGR_BARRICADE_REPAIR: value = sd->bge.ranking.barricade.repair; break;
	case BGR_OBJECTIVE_KILL:   value = sd->bge.ranking.objective.kills; break;
	case BGR_OBJECTIVE_DMG:    value = sd->bge.ranking.objective.damage; break;
	case BGR_FLAG_KILL:        value = sd->bge.ranking.flag.kills; break;
	case BGR_FLAG_DMG:         value = sd->bge.ranking.flag.damage; break;
	case BGR_FLAG_CAPTURE:     value = sd->bge.ranking.flag.capture; break;
	case BGR_FLAG_RECAPTURE:   value = sd->bge.ranking.flag.recapture; break;
	case BGR_CRYSTAL_KILL:     value = sd->bge.ranking.crystal.kills; break;
	case BGR_CRYSTAL_DMG:      value = sd->bge.ranking.crystal.damage; break;
	case BGR_GUARD_KILL:       value = sd->bge.ranking.guardian.kills; break;
	case BGR_GUARD_DEATH:      value = sd->bge.ranking.guardian.deaths; break;
	case BGR_GUARD_DMG_GIVEN:  value = sd->bge.ranking.guardian.damage_given; break;
	case BGR_GUARD_DMG_TAKEN:  value = sd->bge.ranking.guardian.damage_taken; break;
	default:
		script_pushint(st, 0);
		ShowError("script:bg_ranking_get2: Unknow type (%d)\n", type);
		return SCRIPT_CMD_SUCCESS;
...
/// Gets current Battleground temporary ranking information.
///
/// * bg_ranking_set2 <type>,<value>{, <char_id>};
///
BUILDIN_FUNC(bg_ranking_set2)
{
	TBL_PC* sd = NULL;
	int type = script_getnum(st, 2), value = script_getnum(st, 3);
	...
	switch (type)
	{
	case BGR_WINS:             sd->bge.ranking.battle.wins = value; break;
	case BGR_LOSS:             sd->bge.ranking.battle.loss = value; break;
	case BGR_TIED:             sd->bge.ranking.battle.tied = value; break;
	case BGR_SCORE_WINS:       sd->bge.ranking.battle.score_wins = value; break;
	case BGR_SCORE_LOSS:       sd->bge.ranking.battle.score_loss = value; break;
	case BGR_SKILLSUP_SUCCESS: sd->bge.ranking.skill.support_success = value; break;
	case BGR_SKILLSUP_FAIL:    sd->bge.ranking.skill.support_fail = value; break;
	case BGR_SKILLUSE_SUCCESS: sd->bge.ranking.skill.success = value; break;
	case BGR_SKILLUSE_FAIL:    sd->bge.ranking.skill.fail = value; break;
	case BGR_SKILLDMG_GIVEN:   sd->bge.ranking.skill.damage_given = value; break;
	case BGR_SKILLDMG_TAKEN:   sd->bge.ranking.skill.damage_taken = value; break;
	case BGR_HEAL_HP:          sd->bge.ranking.heal.hp = value; break;
	case BGR_HEAL_SP:          sd->bge.ranking.heal.sp = value; break;
	case BGR_ITEMHEAL_HP:      sd->bge.ranking.heal.item_hp = value; break;
	case BGR_ITEMHEAL_SP:      sd->bge.ranking.heal.item_sp = value; break;
	case BGR_ITEMHEAL:         sd->bge.ranking.item.heal = value; break;
	case BGR_AMMO:             sd->bge.ranking.item.ammos = value; break;
	case BGR_POISON_BOTTLE:    sd->bge.ranking.item.poison_bottle = value; break;
	case BGR_FIRE_BOTTLE:      sd->bge.ranking.item.fire_bottle = value; break;
	case BGR_ACID_BOTTLE:      sd->bge.ranking.item.acid_bottle = value; break;
	case BGR_ORIDECON:         sd->bge.ranking.item.oridecon = value; break;
	case BGR_ELUNIUM:          sd->bge.ranking.item.elunium = value; break;
	case BGR_STEEL:            sd->bge.ranking.item.steel = value; break;
	case BGR_EMVERETARCON:     sd->bge.ranking.item.emveretarcon = value; break;
	case BGR_WOODEN_BLOCK:     sd->bge.ranking.item.wooden_block = value; break;
	case BGR_STONE:            sd->bge.ranking.item.stone = value; break;
	case BGR_YELLOW_GEMSTONE:  sd->bge.ranking.item.yellow_gemstone = value; break;
	case BGR_RED_GEMSTONE:     sd->bge.ranking.item.red_gemstone = value; break;
	case BGR_BLUE_GEMSTONE:    sd->bge.ranking.item.blue_gemstone = value; break;
	case BGR_PLAYER_KILL:      sd->bge.ranking.player.kills = value; break;
	case BGR_PLAYER_DEATH:     sd->bge.ranking.player.deaths = value; break;
	case BGR_PLAYER_DMG_GIVEN: sd->bge.ranking.player.damage_given = value; break;
	case BGR_PLAYER_DMG_TAKEN: sd->bge.ranking.player.damage_taken = value; break;
	case BGR_RUNESTONE_KILL:   sd->bge.ranking.runestone.kills = value; break;
	case BGR_RUNESTONE_DMG:    sd->bge.ranking.runestone.damage = value; break;
	case BGR_RUNESTONE_REPAIR: sd->bge.ranking.runestone.repair = value; break;
	case BGR_EMPERIUM_KILL:    sd->bge.ranking.emperium.kills = value; break;
	case BGR_EMPERIUM_DMG:     sd->bge.ranking.emperium.damage = value; break;
	case BGR_BARRICADE_KILL:   sd->bge.ranking.barricade.kills = value; break;
	case BGR_BARRICADE_DMG:    sd->bge.ranking.barricade.damage = value; break;
	case BGR_BARRICADE_REPAIR: sd->bge.ranking.barricade.repair = value; break;
	case BGR_OBJECTIVE_KILL:   sd->bge.ranking.objective.kills = value; break;
	case BGR_OBJECTIVE_DMG:    sd->bge.ranking.objective.damage = value; break;
	case BGR_FLAG_KILL:        sd->bge.ranking.flag.kills = value; break;
	case BGR_FLAG_DMG:         sd->bge.ranking.flag.damage = value; break;
	case BGR_FLAG_CAPTURE:     sd->bge.ranking.flag.capture = value; break;
	case BGR_FLAG_RECAPTURE:   sd->bge.ranking.flag.recapture = value; break;
	case BGR_CRYSTAL_KILL:     sd->bge.ranking.crystal.kills = value; break;
	case BGR_CRYSTAL_DMG:      sd->bge.ranking.crystal.damage = value; break;
	case BGR_GUARD_KILL:       sd->bge.ranking.guardian.kills = value; break;
	case BGR_GUARD_DEATH:      sd->bge.ranking.guardian.deaths = value; break;
	case BGR_GUARD_DMG_GIVEN:  sd->bge.ranking.guardian.damage_given = value; break;
	case BGR_GUARD_DMG_TAKEN:  sd->bge.ranking.guardian.damage_taken = value; break;
	default:
		script_pushint(st, 0);
		ShowError("script:bg_ranking_set2: Unknow type (%d)\n", type);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(bg_ranking_save)
{
	...
	if( script_hasdata(st,3) )
		clear = script_getnum(st,3) ? true : false;
	...
	script_pushint(st, bg_ranking_save(sd, bg_id, clear));
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(bg_clotherscolor) {
	map_session_data *sd;
	int team = script_getnum(st,2);
...
BUILDIN_FUNC(bg_queue_join) {
	...
	if( script_hasdata(st, 3) && script_isstring(st, 3) ) { // Character Name
		if (!script_nick2sd(3, sd)) {
			script_pushint(st, BG_APPLY_NONE);
			return SCRIPT_CMD_FAILURE;
		}
	} else { // Account ID
		if( !script_accid2sd(3, sd) ) {
	...
	if( sd->bg_queue_id > 0 ) {
		script_pushint(st, BG_APPLY_DUPLICATE);
		return SCRIPT_CMD_SUCCESS;
	}

	std::shared_ptr<s_battleground_type> bg;
	if( script_isstring(st, 2) ) {
		bg = bg_search_name(script_getstr(st,2));
	} else {
		bg = bg_search_id(script_getnum(st,2));
	}

	if( bg == nullptr ) {
		script_pushint(st, BG_APPLY_INVALID_NAME);
		return SCRIPT_CMD_SUCCESS;
	}

	if (!bg->solo) {
		script_pushint(st, BG_APPLY_INVALID_APP);
		return SCRIPT_CMD_SUCCESS;
	}

	sd->bge.queue_silent = true;
	int result = 0;
	if( (result = bg_queue_join_multi(bg->name.c_str(), sd, { sd })) )
...
BUILDIN_FUNC(bg_queue_leave) {
	...
	if( script_hasdata(st,2) )
		apply_sc = script_getnum(st,2) ? true : false;
	...
	if( sd->bg_queue_id == 0 )
		...
		script_pushint(st, bg_queue_leave(sd,apply_sc));

	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(bg_queue_check_join) {
	map_session_data *sd;
	...
	bool silent = sd->bge.queue_silent;
	sd->bge.queue_silent = true;
	if( !bg_queue_check_joinable(bg, sd, bg->name.c_str()) ) {
		sd->bge.queue_silent = silent;
		script_pushint(st, sd->bge.apply_error);
		return SCRIPT_CMD_SUCCESS;
	}

	sd->bge.queue_silent = silent;
	script_pushint(st, BG_APPLY_ACCEPT);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(bg_queue_check_delay) {
	map_session_data *sd;
	bool display = true;

	if( script_hasdata(st, 2) )
		display = script_getnum(st, 2) ? true : false;
...
BUILDIN_FUNC(bg_queue_list) {
	int i=0;
	for (const auto &queue : bg_queues) {
		std::shared_ptr<s_battleground_type> bg = bg_search_id(queue->id);
		if( bg != nullptr ) {
			mapreg_setreg(reference_uid(add_str("$@arena_qid"), i), queue->queue_id);
			mapreg_setreg(reference_uid(add_str("$@arena_bgid"), i), queue->id);
			mapreg_setreg(reference_uid(add_str("$@arena_state"), i), queue->state);
			mapreg_setreg(reference_uid(add_str("$@arena_maxplayers"), i), queue->max_players);
			mapreg_setreg(reference_uid(add_str("$@arena_teama_count"), i), queue->teama_members.size());
			mapreg_setreg(reference_uid(add_str("$@arena_teamb_count"), i), queue->teamb_members.size());
			mapreg_setreg(reference_uid(add_str("$@arena_accepted_players"), i), queue->accepted_players);
			mapreg_setreg(reference_uid(add_str("$@arena_required_players"), i), queue->required_players);
			mapreg_setregstr(reference_uid(add_str("$@arena_name$"), i), bg->name.c_str());
			i++;
		}
	}

	mapreg_setreg(reference_uid(add_str("$@arena_count"), 0), i);
	script_pushint(st,i);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(bg_queue_info) {
	...
	for(const auto &queue : bg_queues) {
		...
		switch(type) {
			case BG_QUEUE_ID:
				script_pushint(st,queue->queue_id);
				break;
			case BG_QUEUE_NAME:
				script_pushstrcopy(st,bg->name.c_str());
				break;
			case BG_QUEUE_ARENA_NAME:
				script_pushstrcopy(st,bg->display_name.c_str());
				break;
			case BG_QUEUE_REQUIRED_PLAYERS:
				script_pushint(st,queue->required_players);
				break;
			case BG_QUEUE_ACCEPTED_PLAYERS:
				script_pushint(st,queue->accepted_players);
				break;
			case BG_QUEUE_MAX_PLAYERS:
				script_pushint(st,bg->max_players);
				break;
			case BG_QUEUE_MIN_LEVEL:
				script_pushint(st, bg->min_lvl);
				break;
			case BG_QUEUE_MAX_LEVEL:
				script_pushint(st, bg->max_lvl);
				break;
			case BG_QUEUE_STATE:
				script_pushint(st,queue->state);
				break;
			case BG_QUEUE_TEAMA_SIZE:
				script_pushint(st,queue->teama_members.size());
				break;
			case BG_QUEUE_TEAMB_SIZE:
				script_pushint(st,queue->teamb_members.size());
				break;
			default:
				ShowError("buildin_queue_info: Unknow Type %d\n", type);
				return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(bg_announce)
{
	...
	int         fontType  = script_hasdata(st,4) ? script_getnum(st,4) : FW_NORMAL; // default fontType (FW_NORMAL)
...
BUILDIN_FUNC(bg_listen)
{
	...
	sd->bge.listen = value;
	pc_setglobalreg(sd, add_str(BG_LISTEN_VAR), value);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(bg_setmobdesc)
{
	struct block_list* bl;
	TBL_MOB* md;
	const char *desc = script_getstr(st,3);
	...
	if( bl->type != BL_MOB ) {
		ShowError("buildin_bg_setmobdesc: Invalid mob object!\n");
		return SCRIPT_CMD_FAILURE;
	}
	
	if ((md = map_id2md(bl->id)) == NULL) {
		ShowError("buildin_bg_setmobdesc: Monster not found!\n");
		return SCRIPT_CMD_FAILURE;
	}

	safestrncpy(md->bg_desc, desc, sizeof(md->bg_desc));
	return SCRIPT_CMD_SUCCESS;
}

// [RomuloSM]: Get Char ID
// * getchardir(<char_id>);
// * getchardir("<name>");
BUILDIN_FUNC(getchardir)
{
	TBL_PC *sd;
	...
	script_pushint(st, sd == NULL ? 0 : sd->ud.dir);
	return SCRIPT_CMD_SUCCESS;
}
...
// [RomuloSM]: Stuff Items
BUILDIN_FUNC(getstuffitem)
{
	int get_count, i, type, subtype = 0;
	t_itemid nameid;
	unsigned short amount;
	struct item it;
	unsigned char flag = 0;
	...
	if( !script_mapid2sd(5,sd) ) // no target
		return SCRIPT_CMD_SUCCESS;
	...
	if( script_isstring(st, 2) ) {// "<item name>"
		...
		if( id == nullptr ){
			ShowError("buildin_getstuffitem: Nonexistant item %s requested.\n", name);
			return SCRIPT_CMD_FAILURE; //No item created.
		}
		nameid = id->nameid;
	} else {// <item id>
		...
		if( id == nullptr ){
			ShowError("buildin_getstuffitem: Nonexistant item %u requested.\n", nameid);
			return SCRIPT_CMD_FAILURE; //No item created.
		}
	}
	
	if( id->type == IT_PETEGG || id->type == IT_CARD ) {
		ShowError("buildin_getstuffitem: it is not possible to create pet eggs or cards (item %u).\n", nameid);
		return SCRIPT_CMD_FAILURE; //No item created.
	}
	...
	if( id->type == IT_ARMOR || id->type == IT_WEAPON || id->type == IT_PETARMOR )
		subtype = 1;
	...
	if( subtype ) {
		if( type == 0 )
			random_option_db.option_get_id("STUFF_BATTLEGROUND", option_id);
		else if( type == 1 )
			random_option_db.option_get_id("STUFF_GVG", option_id);
		else if( type == 2 )
			random_option_db.option_get_id("STUFF_PVP", option_id);

		if( option_id <= 0 ) {
			ShowError("buildin_getstuffitem: Unknow option, %d (item %u).\n", option_id, nameid);
			return SCRIPT_CMD_FAILURE; //No item created.
		}
	}
	else {
		switch(type) {
			...
			case 2:
				...
				break;
			default:
				ShowError("buildin_getstuffitem: Unknow type %d (item %u).\n", type, nameid);
				return SCRIPT_CMD_FAILURE; //No item created.
		}
	}
...
// setitemshopcurrency "<npc name>",<itemid>;
// setitemshopcurrency "<npc name>","<item aegis name>";
BUILDIN_FUNC(setitemshopcurrency)
{
	t_itemid nameid;
	struct npc_data *nd;
	const char *name = script_getstr(st,2);

	if( (nd = npc_name2id(name)) == NULL ) {
		ShowError("buildin_setitemshopcurrency: NPC %s not found.\n", name);
		script_pushint(st,0);
		return SCRIPT_CMD_FAILURE;
	}

	if( nd->subtype != NPCTYPE_ITEMSHOP ) {
		ShowError("buildin_setitemshopcurrency: NPC %s is not a item shop.\n", name);
		script_pushint(st,0);
		return SCRIPT_CMD_FAILURE;
	}

	if( script_isstring(st,3) ) {// "<item name>"
		const char *name = script_getstr(st,3);

		std::shared_ptr<item_data> id = item_db.searchname( name );

		if( id == nullptr ){
			ShowError("buildin_setitemshopcurrency: Nonexistant item %s requested.\n", name);
			script_pushint(st,0);
			return SCRIPT_CMD_FAILURE;
		}
		nameid = id->nameid;
		...
		if( item_db.find(nameid) == nullptr ){
			ShowError("buildin_setitemshopcurrency: Nonexistant item %u requested.\n", nameid);
			script_pushint(st,0);
			return SCRIPT_CMD_FAILURE;
		}
	}
...
BUILDIN_FUNC(getpvpmode){
	...
	if( sd && sd->state.pvpmode )
		result = sd->state.pvpmode;

	script_pushint(st, result);
	return SCRIPT_CMD_SUCCESS;
}

// [RomuloSM]: Restock System
BUILDIN_FUNC(addrestock)
{
	...
	nameid = script_getnum(st, 2);
	if ((item_data = item_db.find(nameid)) == nullptr ) {
...
/*
  [RomuloSM]: Open Stall
  open_stall type, <slots> {, <consume>{, <expire_time>{, <account_id>}}}
*/
BUILDIN_FUNC(open_stall) {
	TBL_PC *sd = nullptr;
	bool consume = false;
	int type = script_getnum(st,2), slots = script_getnum(st,3), seconds = 0, idx = -1;
	...
	if( sd->state.stall_ui_open != 0 ){
		ShowError( "buildin_open_stall: Stall window was already open. Player %s (AID: %u, CID: %u).\n", sd->status.name, sd->status.account_id, sd->status.char_id);
		script_pushint(st,0);
		return SCRIPT_CMD_FAILURE;
	}
	...
	idx = sd->itemindex;
	if( script_hasdata(st,4) ) {
		consume = script_getnum(st,4) ? true: false;
		if( consume && idx == -1 ) {
			ShowError( "buildin_open_stall: The function was not called by an item. Player %s (AID %u, CID: %u).\n", sd->status.name, sd->status.account_id, sd->status.char_id);
			script_pushint(st,0);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	if( script_hasdata(st,5) )
		seconds = script_getnum(st,5);
	else if( idx != -1 && sd->inventory.u.items_inventory[idx].expire_time ) {
		seconds = (unsigned int)(sd->inventory.u.items_inventory[idx].expire_time - time(nullptr));
		if( seconds <= 0 ) {
			ShowError( "buildin_open_stall: Called from item %u, which is not a consumed delayed.\n", sd->itemid );
			script_pushint(st,0);
	        return SCRIPT_CMD_FAILURE;
		}
	}

	if( consume && (sd->inventory.u.items_inventory[idx].nameid <= 0 || sd->inventory.u.items_inventory[idx].amount <= 0) ){
		ShowError( "buildin_open_stall: Called item is not in inventory. Player %s (AID %u, CID: %u).\n", sd->status.name, sd->status.account_id, sd->status.char_id);
		script_pushint(st,0);
		return SCRIPT_CMD_FAILURE;
	}

	sd->stall.slots = slots;
	sd->stall.seconds = seconds;
	sd->stall.itemindex = consume ? idx : -1;
	sd->stall.itemid = consume ? sd->inventory.u.items_inventory[idx].nameid : 0;
	if( battle_config.extended_stall_enable )
		clif_stall(sd, type, slots);
	else
		stall_ui_open(sd, type);
	script_pushint(st,1);
	return SCRIPT_CMD_SUCCESS;
}
...
/**
 * Check player's stall
 * check_stall({<char_id>})
 */
BUILDIN_FUNC(check_stall) {
	int char_id = script_getnum(st,2);
...
/**
 * Gets the number of players with open stalls in an area.
 * getareastalls("<map name>",<x1>,<y1>,<x2>,<y2>
 */
BUILDIN_FUNC(getareastalls)
{
	const char *str;
	...
	map_foreachinallarea(buildin_getareausers_sub,
		m,x0,y0,x1,y1,BL_STALL,&users);
	script_pushint(st,users);
	return SCRIPT_CMD_SUCCESS;
}

/// Creates a waiting room (chat room) for this monster.
///
/// mobwaitingroom <unit_id>,"<title>",<limit>{,"<event>"{,<trigger>{,<zeny>{,<minlvl>{,<maxlvl>}}}}};
BUILDIN_FUNC(mobwaitingroom)
{
	const char* title = script_getstr(st, 3);
	int limit = script_getnum(st, 4);
	const char* ev = script_hasdata(st,5) ? script_getstr(st,5) : "";
	int trigger =  script_hasdata(st,6) ? script_getnum(st,6) : limit;
	int zeny =  script_hasdata(st,7) ? script_getnum(st,7) : 0;
	int minLvl =  script_hasdata(st,8) ? script_getnum(st,8) : 1;
	int maxLvl =  script_hasdata(st,9) ? script_getnum(st,9) : MAX_LEVEL;
	struct block_list* bl;
	...
	if( md != nullptr )
		chat_createmobchat(md, title, limit, 1, trigger, ev, zeny, minLvl, maxLvl);

	return SCRIPT_CMD_SUCCESS;
}

/// Removes the waiting room of the current or target monster.
///
/// mobdelwaitingroom <unit_id>;
BUILDIN_FUNC(mobdelwaitingroom)
{
	struct block_list* bl;
	...
	mob_data* md = reinterpret_cast<mob_data*>( bl );
	if( md != nullptr )
		chat_deletemobchat(md);
	return SCRIPT_CMD_SUCCESS;
}

/// Set Monster Fake Guild
///
/// mobfakeguild <unit_id>,<guild_id>,<emblem_id>,"<Guild Name>","<Position>";
BUILDIN_FUNC(mobfakeguild)
{
	int guild_id = script_getnum(st,3);
	int emblem_id = script_getnum(st,4);
	const char* name = script_getstr(st,5);
	const char* position = script_getstr(st,6);
	...
	mob_data* md = reinterpret_cast<mob_data*>( bl );
	if( md != nullptr ) {
		md->fake_guild.guild_id = guild_id;
		md->fake_guild.emblem_id = emblem_id;
		safestrncpy(md->fake_guild.name, name, sizeof(md->fake_guild.name));
		safestrncpy(md->fake_guild.position, position, sizeof(md->fake_guild.position));
	}
...
/// Get Guild Emblem Id
///
/// getguildemblemid <guild_id>";
BUILDIN_FUNC(getguildemblemid)
{
	int guild_id;
	...
	auto g = guild_search(guild_id);
	if (g)
		script_pushint(st,g->guild.emblem_id);
	else 
		script_pushint(st,0);
	return SCRIPT_CMD_SUCCESS;
}
...
// (^~_~^) Gepard Shield End
	...
	BUILDIN_DEF(saferefitems,"?"),
	BUILDIN_DEF(saferefitemscost,"i?"),
	BUILDIN_DEF(saferefcnt,"?"),
	...
	BUILDIN_DEF(autoattack_fromitem,""),
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(setticketuniqueid,""),
	BUILDIN_DEF(getticketuniqueid,""),
	BUILDIN_DEF(kickplayer,"i"),
	...
	// BattleGround
	BUILDIN_DEF(waitingroom2bg,"isii???"),
	...
	// [RomuloSM]: Restock System
	BUILDIN_DEF(addrestock,"ii?"),
	BUILDIN_DEF(delrestock,"i?"),
	BUILDIN_DEF(getrestocklist,""),

	// [RomuloSM]: Battleground Expansion
	BUILDIN_DEF(bg_reward_list, "i?"),
	BUILDIN_DEF(bg_reward, "iii?"),
	BUILDIN_DEF(bg_happyhour, "i"),
	BUILDIN_DEF(bg_happyhour_status, ""),
	BUILDIN_DEF(bg_flagemblem, "i"),
	BUILDIN_DEF(bg_setcountdown, "ii"),
	BUILDIN_DEF(bg_check_dual, "?"),
	BUILDIN_DEF(bg_ranking_get, "i?"),
	BUILDIN_DEF(bg_ranking_set, "ii?"),
	BUILDIN_DEF(bg_ranking_get2, "i?"),
	BUILDIN_DEF(bg_ranking_set2, "ii?"),
	BUILDIN_DEF(bg_ranking_save, "i??"),
	BUILDIN_DEF(bg_clotherscolor, "i?"),
	BUILDIN_DEF(bg_queue_join, "v?"),
	BUILDIN_DEF(bg_queue_leave, "??"),
	BUILDIN_DEF(bg_queue_check_join, "v?"),
	BUILDIN_DEF(bg_queue_check_delay, "??"),
	BUILDIN_DEF(bg_queue_list, ""),
	BUILDIN_DEF(bg_queue_info, "ii"),
	BUILDIN_DEF(bg_announce,"s?????"),
	BUILDIN_DEF(bg_listen, "i?"),
	BUILDIN_DEF(bg_setmobdesc, "is"),

	// [RomuloSM]: Common Script Commands
	BUILDIN_DEF(getchardir, "?"),
	BUILDIN_DEF(itemselectmenu, "*"),

	// [RomuloSM]: Stuff Items
	BUILDIN_DEF(getstuffitem, "vii?"),
	BUILDIN_DEF(setitemshopcurrency,"sv"),

	BUILDIN_DEF(getpvpmode, ""),

	// [RomuloSM]: Stall System
	BUILDIN_DEF(open_stall, "ii???"),
	BUILDIN_DEF(close_stall, "i"),
	BUILDIN_DEF(check_stall,"i"),
	BUILDIN_DEF(getareastalls,"siiii"),

	// [RomuloSM]: Player Bot
	BUILDIN_DEF(mobwaitingroom,"isi?????"),
	BUILDIN_DEF(mobdelwaitingroom,"i"),
	BUILDIN_DEF(mobfakeguild,"iiiss"),
	BUILDIN_DEF(getguildemblemid,"i"),
