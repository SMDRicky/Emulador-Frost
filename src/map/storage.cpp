...
static int storage_additem(map_session_data* sd, struct s_storage *stor, struct item *it, int amount)
{
	...
	if( battle_config.stuff_enable ) {
		int stuffType = itemdb_stuffType(it);
		if( (stuffType == IS_TYPE_BG  && !(battle_config.stuff_enable_storage&0x01)) ||
			(stuffType == IS_TYPE_GVG && !(battle_config.stuff_enable_storage&0x02)) ||
			(stuffType == IS_TYPE_PVP && !(battle_config.stuff_enable_storage&0x04)) ){
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return 1;
		}
	}
...
bool storage_guild_additem(map_session_data* sd, struct s_storage* stor, struct item* item_data, int amount)
{
	...
	// [RomuloSM]: Stuff Items
	if( battle_config.stuff_enable ) {
		int stuffType = itemdb_stuffType(item_data);
		if( (stuffType == IS_TYPE_BG  && !(battle_config.stuff_enable_guild_storage&0x01)) ||
			(stuffType == IS_TYPE_GVG && !(battle_config.stuff_enable_guild_storage&0x02)) ||
			(stuffType == IS_TYPE_PVP && !(battle_config.stuff_enable_guild_storage&0x04)) ) {
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return false;
		}
	}
