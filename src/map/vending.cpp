...
void vending_vendinglistreq(map_session_data* sd, int id)
{
	...
	if( battle_config.extended_vending_enable && vsd->extended_vend.nameid ) {
		std::shared_ptr<s_item_vend_db> iv = item_vend_db.find(vsd->extended_vend.nameid);
		if( iv != nullptr ) {
			char output[1024];
			sprintf(output, msg_txt(sd,1751), vsd->status.name, itemdb_ename(vsd->extended_vend.nameid));
...
void vending_purchasereq(map_session_data* sd, int aid, int uid, const uint8* data, int count)
{
		...
		if( battle_config.extended_vending_enable && vsd->extended_vend.nameid ) {
			if( vsd->extended_vend.nameid == battle_config.item_zeny ) {
				...
				if( pc_inventoryblank(vsd) <= 0 ) {
					clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd,1760), false, SELF);
					return;
				}

				vsd_w += itemdb_weight(vsd->extended_vend.nameid) * amount;
				if( (vsd_w + vsd->weight) > vsd->max_weight ) {
					clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd,1761), false, SELF);
					return;
				}
			}
 		}
	...
	// [RomuloSM]: Extended Vending Message
	if( battle_config.extended_vending_enable && vsd->extended_vend.nameid ) {
		memset(&msg, 0, sizeof(struct mail_message));
		msg.dest_id = vsd->status.char_id;
		msg.zeny = 0;
		safestrncpy(msg.send_name, "Server", NAME_LENGTH);
		safestrncpy(msg.title, msg_txt(vsd,1762), MAIL_TITLE_LENGTH);
	}
	...
	for( i = 0; i < count; i++ ) {
		...
		if( battle_config.extended_vending_enable && vsd->extended_vend.nameid ) {
			if( i == 0 ) {
				sprintf(mailBody, msg_txt(sd,1763), sd->status.name);
				sprintf(mailBody, "%s\r\n\r\n", mailBody);
			}
#if PACKETVER >= 20150513 // Old mail box is too shorter
			if( i >= 0 && strlen(mailBody) < 180 ) {
				if( battle_config.extended_vending_enable ) {
					sprintf(mailBody + strlen(mailBody), msg_txt(vsd,1764), amount, itemdb_ename(itemID), (int)z, vsd->extended_vend.nameid?itemdb_ename(vsd->extended_vend.nameid):"Zeny");
				}
				else {
					sprintf(mailBody + strlen(mailBody), msg_txt(vsd,1764), amount, itemdb_ename(itemID), (int)z, "Zeny");
				}
				sprintf(mailBody, "%s\r\n", mailBody);
			}
	...
	// [RomuloSM]: Extended Vending Messages
	if( battle_config.extended_vending_enable && vsd->extended_vend.nameid ) {
	...
	if( battle_config.extended_vending_enable && vsd->extended_vend.nameid ) {
		vending_update_bank(vsd->status.char_id, VENDING_BANK_SELL, vsd->extended_vend.nameid, (int64)total);
		if( vsd && !vsd->state.autotrade )
			npc_event(vsd, "Vending System#bank::OnPayment", 0);
	}
...
/**
* [RomuloSM]: Extended Vending / Stall
* Update Bank Items
* @param char_id: char id
* @param type : enum e_vend_bank
* @param nameid: item nameid
* @param value: value
*/
bool vending_update_bank(uint32 char_id, enum e_vend_bank type, t_itemid nameid, int64 value) {
	if (Sql_Query(mmysql_handle, "INSERT INTO `vending_bank` (`char_id`, `type`, `nameid`, `value`) VALUES ('%d', '%d', '%u', '%" PRId64 "') ON DUPLICATE KEY UPDATE `value`=`value`+'%" PRId64 "'",
		char_id, type, nameid, value, value) != SQL_SUCCESS) {
		Sql_ShowDebug(mmysql_handle);
		return false;
	}
...
bool vending_searchall(map_session_data* sd, const struct s_search_store_search* s)
{
	...
	for( idx = 0; idx < s->item_count; idx++ ) {
		...
		if( battle_config.extended_vending_enable && sd->extended_vend.nameid ) {
			char out_msg[1024];
...
void vending_reopen( map_session_data* sd )
{
	...
	if ((at = (struct s_autotrader *)uidb_get(vending_autotrader_db, sd->status.char_id)) && at->count && at->entries) {
		...
		if( (fail = vending_openvending(sd, at->title, data, count, at)) == 0 ) {
			...
			// [RomuloSM]: Extended Vending
			sd->extended_vend.nameid = at->extended_vending;
	...
	if (battle_config.feature_autotrade) {
		...
		if( Sql_NumRows(mmysql_handle) > 0 ) {
			...
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				...
				Sql_GetData(mmysql_handle, 8, &data, nullptr); at->extended_vending = atoi(data);	// [RomuloSM] Extended Vending
				at->count = 0;
