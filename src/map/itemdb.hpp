...
enum item_itemid : t_itemid
{
	...
	ITEMID_NEUTRAL_REACTOR				= 51008,
	ITEMID_FIRE_REACTOR					= 51009,
	ITEMID_WATER_REACTOR				= 51010,
	ITEMID_WIND_REACTOR					= 51011,
	ITEMID_EARTH_REACTOR				= 51012,
...
// ======================
// Laphine forging system
// ======================
struct s_laphine_creation_requirement{
	t_itemid item_id;
	uint16 amount;
	uint16 type;
};
...
struct s_laphine_creation{
	t_itemid item_id;
	uint16 requiredRequirements;
	std::unordered_map<t_itemid, std::shared_ptr<s_laphine_creation_requirement>> requirements;
	std::vector<uint16> randomOptionGroups;
};

class LaphineCreationDatabase : public TypesafeYamlDatabase<t_itemid, s_laphine_creation>{
public:
	LaphineCreationDatabase() : TypesafeYamlDatabase( "LAPHINE_CREATE_DB", 1 ){}

	const std::string getDefaultLocation();
	uint64 parseBodyNode( const ryml::NodeRef& node );
};
extern LaphineCreationDatabase laphine_creation_db;
...
class LaphineOptionsDatabase : public TypesafeYamlDatabase<uint16, s_laphine_options>{
public:
	LaphineOptionsDatabase() : TypesafeYamlDatabase( "LAPHINE_OPTIONS_DB", 1 ){}

	const std::string getDefaultLocation();
	uint64 parseBodyNode( const ryml::NodeRef& node );
};
extern LaphineOptionsDatabase laphine_options_db;
// =========================
...
// Check Stuff Items
inline int itemdb_stuffType(struct item *it)
{
	// Healing, Usable, Etc, Card, Delay Consume and Cash
	if( battle_config.stuff_enable ) {
		if( it->card[0] == CARD0_CREATE && MakeDWord(it->card[2],it->card[3]) == battle_config.stuff_bg_reserved_id )
			return IS_TYPE_BG;
		else if( it->card[0] == CARD0_CREATE && MakeDWord(it->card[2],it->card[3]) == battle_config.stuff_gvg_reserved_id )
			return IS_TYPE_GVG;
		else if( it->card[0] == CARD0_CREATE && MakeDWord(it->card[2],it->card[3]) == battle_config.stuff_pvp_reserved_id )
			return IS_TYPE_PVP;
	}

	// Armor, Weapon, Pet Armor and Shadow Equipament
	for( int i=0; i < MAX_ITEM_RDM_OPT; i++ ) {
		int id;
		if( (id = it->option[i].id) ) {
			std::shared_ptr<s_random_opt_data> opt = random_option_db.find(static_cast<uint16>(id));

			if( battle_config.stuff_enable ) {
				if( opt->name.compare("STUFF_BATTLEGROUND") == 0 )
					return IS_TYPE_BG;
				else if( opt->name.compare("STUFF_GVG") == 0 )
					return IS_TYPE_GVG;
				else if( opt->name.compare("STUFF_PVP") == 0 )
					return IS_TYPE_PVP;
			}
		}
	}
