...
int chat_joinchat(map_session_data* sd, int chatid, const char* pass)
{
	...
	if( cd == NULL || cd->bl.type != BL_CHAT || cd->bl.m != sd->bl.m || sd->state.vending || sd->state.buyingstore || sd->chatID || ((cd->owner->type == BL_NPC || cd->owner->type == BL_MOB) ? cd->users+1 : cd->users) >= cd->limit ) {
	...
	if( cd->owner->type != BL_NPC && cd->owner->type != BL_MOB && idb_exists(cd->kick_list,sd->status.char_id) ) {
...
// [RomuloSM]: Player Bot
/**
 * Creates a chat room for a Mob.
 * @param md : Mob requesting
 * @param title : title of chat room
 * @param limit : limit of users in chat room
 * @param pub : public or private
 * @param trigger : event trigger
 ...
 * @return 0
 */
int chat_createmobchat(struct mob_data* md, const char* title, int limit, bool pub, int trigger, const char* ev, int zeny, int minLvl, int maxLvl)
{
	struct chat_data* cd;
	...
	if( md->chat_id ) {
		ShowError("chat_createmobchat: npc '%s' already has a chatroom, cannot create new one!\n", md->name);
		return 0;
	}

	if( zeny > MAX_ZENY || maxLvl > MAX_LEVEL ) {
		ShowError("chat_createmobchat: npc '%s' has a required lvl or amount of zeny over the max limit!\n", md->name);
		return 0;
	}

	cd = chat_createchat(&md->bl, title, "", limit, pub, trigger, ev, zeny, minLvl, maxLvl);
...
/**
 * Removes a chat room for a Mob.
 * @param md : Mob requesting
 */
int chat_deletemobchat(struct mob_data* md)
{
	struct chat_data *cd;

	nullpo_ret(md);

	cd = (struct chat_data*)map_id2bl(md->chat_id);
