...
void trade_tradeadditem(map_session_data *sd, short index, short amount)
{
	...
	if (item->unique_id && item->nameid == 35002){
		StringBuf buf;
		uint32 origin_charid, tmp_origin_charid;
		uint32 origin_accountid, tmp_origin_accountid;
		int count = 0;
		char char_name[NAME_LENGTH], tmp_char_name[NAME_LENGTH];

		StringBuf_Init(&buf);

		StringBuf_AppendStr(&buf, "SELECT `origin_charid`, `origin_accountid`, `name`");
		StringBuf_Printf(&buf, "FROM `transfer_char` INNER JOIN `char` ON `char`.`char_id` = `transfer_char`.`origin_charid` WHERE `unique_id`='%" PRIu64 "'",item->unique_id );
		...
		SqlStmt_BindColumn(stmt, 0, SQLDT_UINT,       &origin_charid,    0, NULL, NULL);
		SqlStmt_BindColumn(stmt, 1, SQLDT_UINT,       &origin_accountid, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, 2, SQLDT_STRING,     &char_name, sizeof(char_name), NULL, NULL);

		while( SQL_SUCCESS == SqlStmt_NextRow(stmt) ){
			tmp_origin_charid = origin_charid;
			tmp_origin_accountid = origin_accountid;
			strcpy(tmp_char_name, char_name);
			count++;
		}
		...
		if(count < 1 || origin_accountid != sd->status.account_id){
			clif_displaymessage (sd->fd, "You can't trade a ticket unbound or not bound to your current account, deal has been canceled");
			return;
		}
		/*if(origin_charid != sd->status.char_id || count < 1){
			clif_displaymessage (sd->fd, "You can't trade a ticket unbound or not bound to your current char, deal has been canceled");
			return;
		}*/
		char message[256];
		sprintf(message, "The char %s is bound to the ticket traded.", tmp_char_name );
		clif_displaymessage(target_sd->fd, message);
	}
	...
	if( battle_config.stuff_enable && pc_get_partner(sd) != target_sd ) {
		int stuffType = itemdb_stuffType(item);
		if( (stuffType == IS_TYPE_BG  && !(battle_config.stuff_enable_trade&0x01)) ||
			(stuffType == IS_TYPE_GVG && !(battle_config.stuff_enable_trade&0x02)) ||
			(stuffType == IS_TYPE_PVP && !(battle_config.stuff_enable_trade&0x04)) ) {
			clif_displaymessage(sd->fd, msg_txt(sd,260));
			clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
			return;
		}
