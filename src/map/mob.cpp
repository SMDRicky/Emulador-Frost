...
struct mob_data* mob_spawn_dataset(struct spawn_data *data)
{
	...
	// [RomuloSM]: Player Bot
	md->fake_guild.guild_id = 0;
	md->fake_guild.emblem_id = 0;
	memset(md->fake_guild.name, '\0', NAME_LENGTH);
	memset(md->fake_guild.position, '\0', NAME_LENGTH);
...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, bool flag)
{
	...
	int adjusted_drop_rate = drop_rate;
	if (sd) {
		std::shared_ptr<item_data> id = item_db.find(ditem->item_data.nameid);
		if (id) {
			if (sd->custom_rate.state && sd->custom_rate.drop > 0) {
				bool is_card = id->type == IT_CARD;
				bool is_boss = md->get_bosstype() == BOSSTYPE_MVP || md->get_bosstype() == BOSSTYPE_MINIBOSS;
				int bonus = (is_card && is_boss) ? sd->custom_rate.drop / 5 : sd->custom_rate.drop;
				adjusted_drop_rate = (int)(adjusted_drop_rate * (100 + bonus) / 100.0);
			}
			if (sd->sc.getSCE(SC_ITEMBOOST))
				adjusted_drop_rate = (int)(adjusted_drop_rate * (100 + sd->sc.getSCE(SC_ITEMBOOST)->val1) / 100.0);
			if (pc_isvip(sd))
				adjusted_drop_rate = (int)(adjusted_drop_rate * (100 + battle_config.vip_drop_increase) / 100.0);
			if (adjusted_drop_rate > 10000)
				adjusted_drop_rate = 10000;
		}
	}

	test_autoloot = sd 
		&& (adjusted_drop_rate <= sd->state.autoloot || pc_isautolooting(sd, ditem->item_data.nameid))
 ...
 */
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, mob_data* md, t_itemid nameid)
{
	int drop_rate = base_rate;

	if (md && battle_config.mob_size_influence) {
	...
	if (src) {
		// Corrige src para garantir que estamos lidando com o jogador real
		if (src->type != BL_PC)
		...
		if (src->type == BL_PC) {
			...
			if (sd->custom_rate.state && sd->custom_rate.drop > 1 && nameid > 0) {
				struct item_data* id = itemdb_search(nameid);
				if (id) {
					bool is_card = (id->type == IT_CARD);
					bool is_boss = mob->get_bosstype() == BOSSTYPE_MINIBOSS || mob->get_bosstype() == BOSSTYPE_MVP;

					if (is_card && is_boss)
						drop_rate = (int)(0.5 + drop_rate * (1. + (sd->custom_rate.drop / 500.))); // dividido por 5
					else
						drop_rate = (int)(0.5 + drop_rate * (1. + (sd->custom_rate.drop / 100.)));
				}
			}

			if (sd->state.pvpmode && battle_config.pvpmode_dropbonus)
				drop_rate_bonus += battle_config.pvpmode_dropbonus;
			...
			int cap = pc_isvip(sd) ? battle_config.drop_rate_cap_vip : battle_config.drop_rate_cap;
			if (pc_isvip(sd))
				drop_rate_bonus += battle_config.vip_drop_increase;

			// SC_ITEMBOOST (goma) como multiplicador adicional
			if (sd->sc.getSCE(SC_ITEMBOOST)) {
				int booster = sd->sc.getSCE(SC_ITEMBOOST)->val1;
				drop_rate += (int)(0.5 + drop_rate * booster / 100.);
			}

			// Aplica bônus de raça/classe como multiplicador direto (após SC_ITEMBOOST)
			...
			if (race_bonus != 0 || class_bonus != 0) {
				drop_rate = (int)(0.5 + drop_rate * (1. + (race_bonus + class_bonus) / 100.0));
			}
			// Aplica cap final
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
			...
			if(flag) {
				if(base_exp || job_exp) {
					if( md->dmglog[i].flag != MDLF_PET || battle_config.pet_attack_exp_to_master ) {
						...
						if(md->bl.champion_monster){
							if(md->bl.champion_monster == 1){ // Furioso
								base_exp += base_exp * 10 / 100;
								job_exp  += job_exp  * 10 / 100;
							}
							if(md->bl.champion_monster == 3){ // Experiente
								base_exp += base_exp * 100 / 2;
								job_exp  += job_exp  * 100 / 2;
							}
						}
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if (mvp_sd) {
				if (it->type == IT_CARD && (md->get_bosstype() == BOSSTYPE_MVP || md->get_bosstype() == BOSSTYPE_MINIBOSS)) {
					char message[128];
					...
					intif_broadcast2(message, strlen(message) + 1, 0xF6FF00, 0x190, 12, 0, 0);
					if ( SQL_ERROR == Sql_Query(mmysql_handle, "INSERT INTO dropped_mvp_card_log (account_id, char_name, mvp_id, mvp_name, card_id, card_name, drop_map) VALUES ('%d', '%s', '%d', '%s', '%d', '%s', '%s')", mvp_sd->status.account_id, mvp_sd->status.name, md->mob_id, md->db->sprite.c_str(), it->nameid, it->ename.c_str(),  mapindex_id2name(mvp_sd->mapindex)))
...
for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
	...
	if (sd->custom_rate.state && sd->custom_rate.drop > 1) {
		...
		if (is_card && is_boss)
			bonus = sd->custom_rate.drop / 5;
		else
			bonus = sd->custom_rate.drop;
	}

	temp = (int)(0.5 + base_rate * (100 + bonus) / 100.0);
...
// [RomuloSM]: Player Bot
int mob_playerbot_search_nick(const char *name, int16 map_id) {
	int id = 0;
