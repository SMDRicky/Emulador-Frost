...
int8 buyingstore_create( map_session_data* sd, int zenylimit, unsigned char result, const char* storename, const struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub* itemlist, unsigned int count, struct s_autotrader *at ){
	...
	if( Sql_Query( mmysql_handle, "INSERT INTO `%s`(`id`, `account_id`, `char_id`, `sex`, `map`, `x`, `y`, `title`, `limit`, `autotrade`, `body_direction`, `head_direction`, `sit`, `extended_vending_item`) "
		"VALUES( %d, %d, %d, '%c', '%s', %d, %d, '%s', %d, %d, '%d', '%d', '%d' );",
		buyingstores_table, sd->buyer_id, sd->status.account_id, sd->status.char_id, sd->status.sex == 0 ? 'F' : 'M', map_getmapdata(sd->bl.m)->name, sd->bl.x, sd->bl.y, message_sql, sd->buyingstore.zenylimit, sd->state.autotrade, at ? at->dir : sd->ud.dir, at ? at->head_dir : sd->head_dir, at ? at->sit : pc_issit(sd), at ? at->extended_vending : sd->extended_vend.nameid ) != SQL_SUCCESS ){
...
void buyingstore_open(map_session_data* sd, uint32 account_id)
{
	...
	if( battle_config.extended_buyingstore_enable && pl_sd->extended_vend.nameid ) {
		char output[1024];
		sprintf(output, msg_txt(sd,1752), sd->message, itemdb_ename(pl_sd->extended_vend.nameid));
...
void buyingstore_trade( map_session_data* sd, uint32 account_id, unsigned int buyer_id, const struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub* itemlist, unsigned int count ){
	...
	// [RomuloSM]: buyer lost money in the mean time? fix the limit
	if( battle_config.extended_buyingstore_enable && pl_sd->extended_vend.nameid ) {
		if( !pl_sd->extended_vend.nameid || pl_sd->extended_vend.nameid == battle_config.item_zeny ) {
			if( pl_sd->status.zeny < pl_sd->buyingstore.zenylimit )
				pl_sd->buyingstore.zenylimit = pl_sd->status.zeny;
		}
		else if( pl_sd->extended_vend.nameid == battle_config.item_cash ) {
			if( pl_sd->cashPoints < pl_sd->buyingstore.zenylimit )
				pl_sd->buyingstore.zenylimit = pl_sd->cashPoints;
		}
		else {
			int loot_count = pc_countitem(*pl_sd, pl_sd->extended_vend.nameid, true, false);
			if( loot_count < pl_sd->buyingstore.zenylimit )
				pl_sd->buyingstore.zenylimit = loot_count;
		}
	}
	else
	...
	if( battle_config.extended_buyingstore_enable && pl_sd->extended_vend.nameid ) {
		if( !pl_sd->extended_vend.nameid  || pl_sd->extended_vend.nameid == battle_config.item_zeny )
			pc_payzeny(pl_sd, total, LOG_TYPE_BUYING_STORE, sd->status.char_id);
		else if( battle_config.item_cash && pl_sd->extended_vend.nameid == battle_config.item_cash )
			pc_paycash(pl_sd, total, 0, LOG_TYPE_BUYING_STORE);
		else
			pc_delitem_sub(*pl_sd, pl_sd->extended_vend.nameid, total, 1, 0, LOG_TYPE_BUYING_STORE, false, true);

		if( vending_update_bank(sd->status.char_id, VENDING_BANK_BUYINGSOTORE, pl_sd->extended_vend.nameid, total) )
...
bool buyingstore_searchall(map_session_data* sd, const struct s_search_store_search* s)
{
	...
	for( idx = 0; idx < s->item_count; idx++ )
	{
		...
		if( battle_config.extended_buyingstore_enable && sd->extended_vend.nameid ) {
			char out_msg[1024];
...
void do_init_buyingstore_autotrade( void ) {
	if(battle_config.feature_autotrade) {
		if (Sql_Query(mmysql_handle,
			"SELECT `id`, `account_id`, `char_id`, `sex`, `title`, `limit`, `body_direction`, `head_direction`, `sit`, `extended_vending_item` "
			"FROM `%s` "
		...
		if( Sql_NumRows(mmysql_handle) > 0 ) {
			...
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				...
				Sql_GetData(mmysql_handle, 9, &data, nullptr); at->extended_vending = atoi(data);	// [RomuloSM] Extended Buying Store
				at->count = 0;
				...
				if (battle_config.autotrade_monsterignore)
					...
					at->sd->state.block_action &= ~PCBLOCK_IMMUNE;
				// [RomuloSM]: Extended Buyingstore
				at->sd->extended_vend.nameid = at->extended_vending;
