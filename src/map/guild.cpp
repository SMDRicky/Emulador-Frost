 ...
 **/
short guild_skill_get_index(uint16 skill_id) {
	if (!SKILL_CHK_GUILD(skill_id))
		return -1;
...
// Retrieve skill_lv learned by guild
int guild_fake_checkskill(struct guild *g, int id) {
...
// Guild skill check - from jA [Komurka]
bool guild_fake_check_skill_require( struct guild *g, uint16 id ){
	if( g == nullptr ){
		return false;
	}
	...
	for( const auto& pair : skill->need ){
		if( pair.second->lv > guild_fake_checkskill( g, pair.second->id ) ){
			return false;
		}
	}
