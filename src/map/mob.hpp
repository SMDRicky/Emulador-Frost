...
enum MOBID {
	...
	MOBID_SACERDOTEA = 20608,
	MOBID_SACERDOTEB = 20609,
	MOBID_PALAA = 20610,
	MOBID_PALAB = 20611,
	MOBID_PROFESSORA = 3971,
	MOBID_PROFESSORB = 3972,
	MOBID_BARDOA = 3489,
	MOBID_BARDOB = 20638,
};
...
struct mob_data {
	...
	char bg_desc[NAME_LENGTH]; // [RomuloSM]: Battleground Expansion (desc)
...
struct mob_data *mob_once_spawn_sub(struct block_list *bl, int16 m, int16 x, int16 y, const char *mobname, int mob_id, const char *event, unsigned int size, enum mob_ai ai, int champion = 0);

int mob_once_spawn(map_session_data* sd, int16 m, int16 x, int16 y,
	const char* mobname, int mob_id, int amount, const char* event, unsigned int size, enum mob_ai ai, int champion = 0);

int mob_once_spawn_area(map_session_data* sd, int16 m,
	int16 x0, int16 y0, int16 x1, int16 y1, const char* mobname, int mob_id, int amount, const char* event, unsigned int size, enum mob_ai ai,int champion = 0);
...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, mob_data* md = nullptr, t_itemid nameid = 0);
...
// [RomuloSM]: Player Bot
int mob_playerbot_search_nick(const char *name, int16 map_id = 0);
inline bool mob_fake_guild( mob_data *md ) {
	if( md->fake_guild.name[0] != '\0' && strlen(md->fake_guild.name) > 0 && md->fake_guild.position[0] != '\0' && strlen(md->fake_guild.position) > 0 )
		return true;
	return false;
}
