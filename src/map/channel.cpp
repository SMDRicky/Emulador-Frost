...
struct Channel* channel_create_simple(char *name, char *pass, enum Channel_Type chantype, unsigned int owner) {
	...
	switch (chantype) {
		...
		case CHAN_TYPE_PRIVATE:
			...
			break;
		// [RomuloSM]: Battlground Expansion
		case CHAN_TYPE_BG:
			safestrncpy(tmp_chan.name, name+1, sizeof(tmp_chan.name));
			tmp_chan.pass[0] = '\0';
			safestrncpy(tmp_chan.alias, name, sizeof(tmp_chan.name));
			tmp_chan.type = CHAN_TYPE_BG;
			tmp_chan.bg_id = owner;
			tmp_chan.char_id = 0;
			break;
		default:
			return NULL;
...
int channel_send(struct Channel *channel, map_session_data *sd, const char *msg) {
	...
	// [RomuloSM]: Battleground Expansion
	if( !battle_config.bg_channel_open && channel->type == CHAN_TYPE_BG && sd->bg_id != channel->bg_id )
		return -1;
...
int channel_pcjoin(map_session_data *sd, char *chname, char *pass){
	...
	// [RomuloSM]: Battleground Expansion
	case CHAN_TYPE_BG:
		if( sd->bg_id != channel->bg_id ) {
			clif_displaymessage(sd->fd, msg_txt(sd,1699));
			return -1;
		}
	default: //private and public atm
