...
int npc_cashshop_buylist( map_session_data *sd, int points, std::vector<s_npc_buy_list>& item_list ){
	...
	if (nd->master_nd) //Script-based shops.
		return npc_buylist_sub(sd,item_list,vt,nd->master_nd);
 ...
 */
static int npc_buylist_sub(map_session_data* sd, std::vector<s_npc_buy_list>& item_list, int zeny, struct npc_data* nd) {
	char npc_ev[EVENT_NAME_LENGTH];
	int key_nameid = 0, key_amount = 0, key_zeny = 0;
	...
	script_setarray_pc( sd, "@bought_cost", 0, zeny, &key_zeny );
...
e_purchase_result npc_buylist( map_session_data* sd, std::vector<s_npc_buy_list>& item_list ){
	...
	if (nd->master_nd){ //Script-based shops.
		npc_buylist_sub(sd,item_list,(int)z,nd->master_nd);
