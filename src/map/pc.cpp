...
uint8 pc_isequip(map_session_data *sd,int n)
{
	...
	// [RomuloSM]: Stuff Items equip restricted
	if( battle_config.stuff_enable ) {
		int stuffType = itemdb_stuffType(&sd->inventory.u.items_inventory[n]);
		if( stuffType == IS_TYPE_BG && !map_getmapflag(sd->bl.m,MF_BG_CONSUME) ) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1822),false,SELF);
			return ITEM_EQUIP_ACK_FAIL;
		}

		if( stuffType == IS_TYPE_GVG && !map_getmapflag(sd->bl.m,MF_GVG_CONSUME) ) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1824),false,SELF);
			return ITEM_EQUIP_ACK_FAIL;
		}

		if( stuffType == IS_TYPE_PVP && !map_getmapflag(sd->bl.m,MF_PVP_CONSUME) ) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1826),false,SELF);
			return ITEM_EQUIP_ACK_FAIL;
		}
	}
...
bool pc_authok(map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	// [RomuloSM]: Extended Vending & Buying/Search Store
	sd->extended_vend.nameid = 0;
	sd->extended_vend.level = 0;

	// [RomuloSM]: Extended Stall
	sd->stall.itemindex = -1;
	sd->stall.itemid = 0;
	sd->stall.seconds = 0;
	sd->stall.slots = 0;
...
// (^~_~^) Color Nicks End
	...
	// [RomuloSM]: Battleground Expansion
	sd->bge.listen = static_cast<int>(pc_readglobalreg(sd, add_str(BG_LISTEN_VAR)));
...
void pc_bonus2(map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_SUBELE: // bonus2 bSubEle,e,x;
		...
		if(sd->indexed_bonus.subele_script[type2] > 90)
			sd->indexed_bonus.subele_script[type2] = 90;
	}
	break;
	case SP_SUBRACE: // bonus2 bSubRace,r,x;
		...
		if(sd->indexed_bonus.subrace[type2] > 90)
			sd->indexed_bonus.subrace[type2] = 90;
	}
	break;
	...
	case SP_SUBSIZE: // bonus2 bSubSize,s,x;
		...
		if(sd->state.lr_flag != 2) {
			...
			if(sd->indexed_bonus.subsize[type2] > 90)
				sd->indexed_bonus.subsize[type2] = 90;
	}
	break;
...
void pc_bonus3(map_session_data *sd,int type,int type2,int type3,int val)
{
	...
	if (sd->state.lr_flag != 2) {
		...
		if (sd->indexed_bonus.subele[type2] > 90)
			sd->indexed_bonus.subele[type2] = 90;
	}
	break;
...
/**
	* Verify if a item on player's inventory is refinable
	@param item
	@param item_data
	@return is refinable returns 
*/
int pc_item_saferefine_value(item *item, item_data *id){
	int safe_refine = id->type == IT_ARMOR ? SAFE_REFINE_ARMOR : SAFE_REFINE_WEAPON - id->weapon_level;
	...
	if(item->nameid > 0 && 
			...
			!id->flag.no_refine && 
			!item->expire_time &&
			!item->refine < safe_refine
	){
		return cap_value(safe_refine - item->refine, 0, safe_refine); 
	}
	return -1;
}
...
char pc_delitem(map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
...
#ifdef STUFF_SYSTEM
		int stuffType = IS_TYPE_NONE;
#else
		int stuffType = 0;
#endif
		bool packguild = false;
		bool costume = false;

#ifdef STUFF_SYSTEM
		if( battle_config.stuff_enable )
			stuffType = itemdb_stuffType(&sd->inventory.u.items_inventory[n]);
		
		if( battle_config.costumeitem_enable )
			costume = itemdb_iscostume(&sd->inventory.u.items_inventory[n]);
			
		if( battle_config.packguild_enable )
			packguild = itemdb_ispackguild(&sd->inventory.u.items_inventory[n]);
#endif

		if( sd->inventory.u.items_inventory[n].amount <= 0 && restock_check_zone(sd->bl.m) )
			restock_get_item(sd, sd->inventory.u.items_inventory[n].nameid, stuffType, costume, packguild);
	}
	
	if( sd->inventory.u.items_inventory[n].amount <= 0 ){
		if(sd->inventory.u.items_inventory[n].equip)
			pc_unequipitem(sd,n,2|(!(type&4) ? 1 : 0));
		item_id = sd->inventory_data[n]->type; // charms system
	...
	if (item_id == IT_CHARM) status_calc_pc(sd, SCO_NONE);
	return 0;
}
...
int pc_useitem(map_session_data *sd,int n)
{
	...
	if( battle_config.stuff_enable ) {
		...
		// BG Stuff
		if( stuffType == IS_TYPE_BG && !map_getmapflag(sd->bl.m, MF_BG_CONSUME) ) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1823),false,SELF);
			return 0;
		}

		// GvG Stuff
		if( stuffType == IS_TYPE_GVG && !map_getmapflag(sd->bl.m, MF_GVG_CONSUME) ) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1825),false,SELF);
			return 0;
		}

		// PvP Stuff
		if( stuffType == IS_TYPE_PVP && !map_getmapflag(sd->bl.m, MF_PVP_CONSUME) ) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1827),false,SELF);
			return 0;
		}
	}
	...
	// [RomuloSM]: Battleground Expansion
	if( battle_config.bg_expansion_enable && sd->bg_id && map_getmapflag(sd->bl.m, MF_BATTLEGROUND) )
		bg_score_del_item(sd, nameid, 1);
...
enum e_additem_result pc_cart_additem(map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type)
{
	...
	// [RomuloSM]: Stuff Items
	if( battle_config.stuff_enable ) {
		int stuffType = itemdb_stuffType(item);
		// BG Stuff
		if( stuffType == IS_TYPE_BG && !(battle_config.stuff_enable_cart&0x01) ) {
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return ADDITEM_INVALID;
		}

		// GvG Stuff
		if( stuffType == IS_TYPE_GVG && !(battle_config.stuff_enable_cart&0x02) ) {
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return ADDITEM_INVALID;
		}

		// PvP Stuff
		if( stuffType == IS_TYPE_PVP && !(battle_config.stuff_enable_cart&0x04) ) {
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return ADDITEM_INVALID;
		}
	}
...
enum e_setpos pc_setpos(map_session_data* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	if( sd->state.changemap ) { // Misc map-changing settings
		...
		if (sd->bg_id && mapdata && !mapdata->getMapFlag(MF_BATTLEGROUND)) // Moving to a map that isn't a Battlegrounds
			bg_team_leave(sd, false, true, BGTL_LEFT);
		...
		// [RomuloSM]: Battleground Warfrare
	...
	if( m < 0 )
	{
		...
		if (sd->bg_id) // Switching map servers, remove from bg
			bg_team_leave(sd, false, true, BGTL_QUIT);
...
void pc_damage(map_session_data *sd,struct block_list *src,unsigned int hp, unsigned int sp, unsigned int ap)
{
	...
	{
		map_session_data *pk = (TBL_PC*)src;
...
int pc_dead(map_session_data *sd,struct block_list *src)
{
	...
	if(sd->status.pet_id > 0 && sd->pd) {
		...
		std::shared_ptr<item_data> ps_item = item_db.find(battle_config.petsecurity_item);
		if(ps_item != nullptr) {
			int i = 0;
			ARR_FIND(0, MAX_INVENTORY, i, sd->inventory.u.items_inventory[i].nameid == ps_item->nameid);
			is_secured = i != MAX_INVENTORY;
		}

		if( !mapdata->getMapFlag(MF_NOEXPPENALTY) && !is_secured) {
	...
	if(sd && sd->state.pvpmode){
		...
		if(ssd && sd != ssd && ssd->state.pvpmode){
			t_exp base_penalty = 0;
			t_exp job_penalty = 0;
			base_penalty = job_penalty = battle_config.pvpmode_exppenalty;

			if(!pc_is_maxbaselv(sd) && base_penalty > 0) {
				base_penalty = (t_exp) ( pc_nextbaseexp(sd) * ( base_penalty / 100. ) );
			}else{
				base_penalty = 0;
			}
			if(!pc_is_maxjoblv(sd) && job_penalty > 0) {
				job_penalty = (t_exp) ( pc_nextjobexp(sd) * ( job_penalty / 100. ) );
			}else{
				job_penalty = 0;
			}

			pc_lostexp(sd, base_penalty, job_penalty);
			if(battle_config.pvpmode_expreward)
				pc_gainexp(ssd, NULL, base_penalty, job_penalty, 2);
		}
		pc_pvpmodeoff(sd, 1, 1);
		sc_start(&sd->bl,&sd->bl,SC_NOCHAT,100,0,battle_config.pvpmode_mutepenalty);
	}
...
void pc_revive(map_session_data *sd,unsigned int hp, unsigned int sp, unsigned int ap) {
	...
	if ( battle_config.bg_expansion_enable && sd->bg_id) {
		...
		if (bg != nullptr && bg->expansion.g != NULL) {
			guild_guildaura_refresh(sd, GD_LEADERSHIP, guild_fake_checkskill(bg->expansion.g, GD_LEADERSHIP));
			guild_guildaura_refresh(sd, GD_GLORYWOUNDS, guild_fake_checkskill(bg->expansion.g, GD_GLORYWOUNDS));
			guild_guildaura_refresh(sd, GD_SOULCOLD, guild_fake_checkskill(bg->expansion.g, GD_SOULCOLD));
			guild_guildaura_refresh(sd, GD_HAWKEYES, guild_fake_checkskill(bg->expansion.g, GD_HAWKEYES));
		}
	}
}
...
bool pc_candrop(map_session_data *sd, struct item *item)
{
	...
	if( !pc_can_give_items(sd) || sd->sc.cant.drop) //check if this GM level can drop items
		return false;
	// [RomuloSM]: Stuff Item
	if( item && battle_config.stuff_enable ) {
		int stuffType = itemdb_stuffType(item);
		// BG Stuff
		if( stuffType == IS_TYPE_BG && !(battle_config.stuff_enable_drop&0x01) )
			return false;
		// GvG Stuff
		if( stuffType == IS_TYPE_GVG && !(battle_config.stuff_enable_drop&0x02) )
			return false;
		// PvP Stuff
		if( stuffType == IS_TYPE_PVP && !(battle_config.stuff_enable_drop&0x04) )
			return false;
	}
...
bool pc_equipitem(map_session_data *sd,short n,int req_pos,bool equipswitch)
{
	...
	if (id) {
		...
		if(itemdb_isspecial(sd->inventory.u.items_inventory[n].card[0]))
			; //No cards
		// [RomuloSM]: Stuff Items
		else if(battle_config.stuff_enable && !battle_config.stuff_enable_cards && itemdb_stuffType(&sd->inventory.u.items_inventory[n]) != IS_TYPE_NONE)
			; //No cards
		else {
			for( i = 0; i < MAX_SLOTS; i++ ) {
				if (!sd->inventory.u.items_inventory[n].card[i])
...
static void pc_unequipitem_sub(map_session_data *sd, int n, int flag) {
	...
	bool packguild = itemdb_ispackguild(&sd->inventory.u.items_inventory[n]);
#else
	bool packguild = false;
#endif
	...
	if (sd->inventory_data[n]) {
		...
		if (itemdb_isspecial(sd->inventory.u.items_inventory[n].card[0]) && stuffType != IS_TYPE_NONE && packguild == false)
			; // No cards
		// [RomuloSM]: Stuff Items
		else if(battle_config.stuff_enable && !battle_config.stuff_enable_cards && itemdb_stuffType(&sd->inventory.u.items_inventory[n]) != IS_TYPE_NONE)
			; //No cards
...
// [RomuloSM]: Stuff Items
short pc_search_inventory_stuff(map_session_data *sd, t_itemid nameid) {
	short i, stuffType = IS_TYPE_NONE;
	nullpo_retr(-1, sd);

	if( battle_config.stuff_enable ) {
		if( map_getmapflag(sd->bl.m,MF_BG_CONSUME) )
			stuffType = IS_TYPE_BG;
		else if( map_getmapflag(sd->bl.m,MF_GVG_CONSUME) )
			stuffType = IS_TYPE_GVG;
		else if( map_getmapflag(sd->bl.m,MF_PVP_CONSUME) )
			stuffType = IS_TYPE_PVP;
		else
			return -1;
	
		ARR_FIND( 0, MAX_INVENTORY, i, sd->inventory.u.items_inventory[i].nameid == nameid && itemdb_stuffType(&sd->inventory.u.items_inventory[i]) == stuffType);
		if (i < MAX_INVENTORY)
			return i;
	}
	return -1;
...
// PvP Mode
void pc_pvpmode(map_session_data* sd)
{
	struct map_data *mapdata = map_getmapdata(sd->bl.m);
	...
	if( battle_config.pvpmode_city_only && !mapdata->getMapFlag(MF_TOWN)){
		clif_displaymessage(sd->fd, "[Você so pode ativer/desativar o modo PK em cidades]");
		return; // Ativar/desativar apenas em cidades
	}

	if( mapdata_nopvpmode(mapdata) ){
		clif_displaymessage(sd->fd, "[Você não pode entrar no modo PK nesse mapa]");
		return; // Não pode em alguns mapas
	}

	if( sd->duel_group > 0 )
		return; // N?pode em duelo

	if( sd->disguise )
		return; // N?pode em disguise

	if( sd->idlepvp >= last_tick - battle_config.pvpmode_enable_delay && !mapdata->getMapFlag(MF_FORCED_PVPMODE))
	{
		char pkinfo[100];
		unsigned int tick = (unsigned int)(sd->idlepvp - last_tick + battle_config.pvpmode_enable_delay);
		sprintf(pkinfo, "[Modo PK - espere %d segundos para entrar novamente no Modo PK]", tick);
		clif_displaymessage(sd->fd, pkinfo);
		return;
	}
  ...
  auto it = util::vector_get(sd->hatEffects, HAT_EF_MOD_PK);
  if (it == sd->hatEffects.end()) {
    sd->hatEffects.push_back(HAT_EF_MOD_PK);
  }
  if (!sd->state.connect_new) {
	  clif_hat_effect_single(sd, HAT_EF_MOD_PK, 0);
  }

	sc_start(&sd->bl,&sd->bl, SC_PVPMODE, 100, 0, INFINITE_TICK);
	...
	clif_displaymessage(sd->fd, "[Entrando no Modo PK - Você pode matar e ser morto no Modo PK]");
	return;
}

void pc_pvpmodeoff(map_session_data* sd, short force, short flag)
{
	char pkinfo[100];
	...
	if(mapdata->getMapFlag(MF_FORCED_PVPMODE)){
		clif_displaymessage(sd->fd, "[Você não pode sair do modo PK nesse mapa]");
		return;
	}
	...
	if( !force && sd->idlepvp >= last_tick - battle_config.pvpmode_disable_delay )
	{
		...
		sprintf(pkinfo, "[Modo PK - %d segundos para desabilitar]", tick);
		clif_displaymessage(sd->fd, pkinfo);
		return;
	}
  ...
  auto it = util::vector_get(sd->hatEffects, HAT_EF_MOD_PK);
  if (it != sd->hatEffects.end()) {
    util::vector_erase_if_exists(sd->hatEffects, HAT_EF_MOD_PK);
  }
  if (!sd->state.connect_new) {
	  clif_hat_effect_single(sd, HAT_EF_MOD_PK, 1);
  }

	status_change_end(&sd->bl, SC_PVPMODE, INVALID_TIMER);
	...
	sprintf(pkinfo, "[Modo PK desabilitado]");
	clif_displaymessage(sd->fd, pkinfo);
...
// [RomuloSM]: Count Item
int pc_countitem(map_session_data &sd, t_itemid nameid, bool ignore_expire, bool ignore_bound) {
	int i, c = 0;
	for( i = 0; i < MAX_INVENTORY; i++ ) {
		if( !ignore_bound && sd.inventory.u.items_inventory[i].bound )
			continue;
		if( !ignore_expire && sd.inventory.u.items_inventory[i].expire_time > 0 )
			continue;
		if( sd.inventory.u.items_inventory[i].nameid == nameid )
			c += sd.inventory.u.items_inventory[i].amount;
	}

	return c;
}

// [RomuloSM]: Del Item Sub
bool pc_delitem_sub(map_session_data &sd, t_itemid nameid, int amount, int type, short reason, e_log_pick_type log_type, bool ignore_expire, bool ignore_bound) {
	int remaing = amount;
	for( int i = 0; i < MAX_INVENTORY && remaing > 0; i++ ) {
		if( !ignore_bound && sd.inventory.u.items_inventory[i].bound )
			continue;
		if( !ignore_expire && sd.inventory.u.items_inventory[i].expire_time > 0 )
			continue;
		...
		int get_items = 0;
		if( sd.inventory.u.items_inventory[i].amount > remaing )
			get_items = sd.inventory.u.items_inventory[i].amount - remaing;
		else
			get_items = sd.inventory.u.items_inventory[i].amount;

		if( !pc_delitem(&sd, i, get_items, type, reason, log_type) )
			remaing -= get_items;
	}

	return (remaing > 0 ? false : true);
}
