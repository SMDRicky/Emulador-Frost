...
int status_calc_mob_(struct mob_data* md, uint8 opt)
{
	...
	if(flag&4) { // Strengthen Guardians - custom value +10% / lv
		...
		// [RomuloSM]: Battleground Expansion
		if (battle_config.bg_expansion_enable && mapdata->getMapFlag(MF_BATTLEGROUND) && md->mob_id == MOBID_EMPERIUM) {
#ifdef RENEWAL
			status->max_hp += 50 * (battle_config.bg_castle_defense/5);
#else
			status->max_hp += 1000 * battle_config.bg_castle_defense;
#endif
			status->hp = status->max_hp;
			status->def += (battle_config.bg_castle_defense+2)/3;
			status->mdef += (battle_config.bg_castle_defense+2)/3;
		}
		else
		if (gc == nullptr)
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (bl->type == BL_PC) {
			...
			if ((skill_lv = pc_checkskill(sd, NV_TRANSCENDENCE)) > 0)
				bonus += 350 * skill_lv + (skill_lv > 4 ? 250 : 0);
#ifndef HP_SP_TABLES
			if ((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE && sd->status.base_level >= 50)
				bonus += 4000; // Supernovice lvl99 hp bonus.
			if ((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE && sd->status.base_level >= 99)
				bonus += 5000; // Supernovice lvl150 hp bonus.
#endif
		}
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (bl->type == BL_PC) {
			...
			if ((skill_lv = pc_checkskill(sd,SL_KAINA)) > 0)
				bonus += 30 * skill_lv;
			if ((skill_lv = pc_checkskill(sd,MG_SRECOVERY)) > 0)
				bonus += 30 * skill_lv;
		...
		if (sc) {
			...
			if (sc->getSCE(SC_ID_MAXSP)) //[Easycore]
				bonus -= sc->getSCE(SC_ID_MAXSP)->val1;
			if (sc->getSCE(SC_2011RWC_SCROLL))
				bonus -= 10;
			if (sc->getSCE(SC_INFINITY_DRINK))
				bonus += 5;
...
static unsigned int status_calc_maxhpsp_pc(map_session_data* sd, unsigned int stat, bool isHP) {
	...
	if (isHP) { //Calculates MaxHP
		...
		dmax = job->base_hp[level-1] * (1 + (umax(stat,1) * 0.01)) * ((sd->class_&JOBL_UPPER)?1.25:(pc_is_taekwon_ranker(sd))?4:1);
		...
		double equip_bonus = 0, item_bonus = 0;
		dmax = job->base_sp[level-1] * (1 + (umax(stat,1) * 0.01)) * ((sd->class_&JOBL_UPPER)?1.25:(pc_is_taekwon_ranker(sd))?4:1);
...
int status_calc_pc_sub(map_session_data* sd, uint8 opt)
{
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]) {
			...
			if( battle_config.stuff_enable && !battle_config.stuff_enable_cards && itemdb_stuffType(&sd->inventory.u.items_inventory[index]) != IS_TYPE_NONE )
				continue;
...
// ----- STATS CALCULATION -----
	...
	if((((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE && (sd->status.job_level >= 70  || sd->class_&JOBL_THIRD)) && sd->die_counter == 0) || pc_is_taekwon_ranker(sd)) {
		base_status->str += 10;
		base_status->agi += 10;
		base_status->vit += 10;
		base_status->int_+= 10;
...
static unsigned short status_calc_str(struct block_list *bl, status_change *sc, int str)
{
	...
	if(sc->getSCE(SC_ID_ALLSTATS)) //[Easycore]
		str -= sc->getSCE(SC_ID_ALLSTATS)->val1;
...
static unsigned short status_calc_agi(struct block_list *bl, status_change *sc, int agi)
{
	...
	if(sc->getSCE(SC_INCREASING))
		agi += 10; // Added based on skill updates [Reddozen]
	...
	if(sc->getSCE(SC_ID_ALLSTATS)) //[Easycore]
		agi -= sc->getSCE(SC_ID_ALLSTATS)->val1;
	if(sc->getSCE(SC_KYOUGAKU))
...
static unsigned short status_calc_vit(struct block_list *bl, status_change *sc, int vit)
{
	...
	if(sc->getSCE(SC_ID_ALLSTATS)) //[Easycore]
		vit -= sc->getSCE(SC_ID_ALLSTATS)->val1;
	if(sc->getSCE(SC_KYOUGAKU))
...
static unsigned short status_calc_int(struct block_list *bl, status_change *sc, int int_)
{
	...
	if(sc->getSCE(SC_CHASEWALK2))
		int_ += sc->getSCE(SC_CHASEWALK2)->val1;
	...
	if(sc->getSCE(SC_ID_ALLSTATS)) //[Easycore]
		int_ -= sc->getSCE(SC_ID_ALLSTATS)->val1;
	if(sc->getSCE(SC_KYOUGAKU))
...
static unsigned short status_calc_dex(struct block_list *bl, status_change *sc, int dex)
{
	...
	if(sc->getSCE(SC_INCREASING))
		dex += 10; // Added based on skill updates [Reddozen]
	if(sc->getSCE(SC_MARIONETTE))
	...
	if(sc->getSCE(SC_ID_ALLSTATS)) //[Easycore]
		dex -= sc->getSCE(SC_ID_ALLSTATS)->val1;
	if(sc->getSCE(SC_KYOUGAKU))
...
static unsigned short status_calc_luk(struct block_list *bl, status_change *sc, int luk)
{
	...
	if(sc->getSCE(SC_ID_ALLSTATS)) //[Easycore]
		luk -= sc->getSCE(SC_ID_ALLSTATS)->val1;
	if(sc->getSCE(SC_KYOUGAKU))
...
static unsigned short status_calc_batk(struct block_list *bl, status_change *sc, int batk)
{
	...
	if(sc->getSCE(SC_ID_ATK)) //[Easycore]
		batk -= batk * sc->getSCE(SC_ID_ATK)->val1/100;
	if (sc->getSCE(SC_ALMIGHTY))
		batk += 30;
	if (sc->getSCE(SC_ULTIMATECOOK))
		batk += 30;
...
static unsigned short status_calc_ematk(struct block_list *bl, status_change *sc, int matk)
{
	...
	if (sc->getSCE[SC_ID_MATK]) //[Easycore]
		matk -= matk * sc->getSCE[SC_ID_MATK]->val1 / 100;
	if(sc->getSCE(SC_ALMIGHTY))
		matk += 30;
	if(sc->getSCE(SC_ULTIMATECOOK))
		matk += 30;
...
static signed short status_calc_hit(struct block_list *bl, status_change *sc, int hit)
{
	...
	if(sc->getSCE(SC_ADJUSTMENT))
		hit -= 30;
	if(sc->getSCE(SC_INCREASING))
		hit += 30; // RockmanEXE; changed based on updated [Reddozen]
...
static signed short status_calc_flee(struct block_list *bl, status_change *sc, int flee)
{
	if( bl->type == BL_PC ) {
		...
		// PvP Mode
		else if( ((TBL_PC*)bl)->state.pvpmode )
			flee -= flee * battle_config.pvpmode_flee_penalty/100;
	}
...
static defType status_calc_def(struct block_list *bl, status_change *sc, int def)
{
	...
	if( sc->getSCE(SC_SOLID_SKIN_OPTION) )
		def += def * sc->getSCE(SC_SOLID_SKIN_OPTION)->val2 / 185;
...
static unsigned short status_calc_speed(struct block_list *bl, status_change *sc, int speed)
{
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER && (pc_checkskill(sd,SA_FREECAST) > 0 || sd->ud.skill_id == LG_EXEEDBREAK) ) {
		...
		if( sd && sc->getSCE(SC_HIDING) && pc_checkskill(sd,RG_TUNNELDRIVE) > 0 )
			val = 120 - 10 * pc_checkskill(sd,RG_TUNNELDRIVE);
			...
			if( sc->getSCE(SC_DECREASEAGI) )
				val = max( val, 25 );
			if( sc->getSCE(SC_ID_SPEED) ) //[Easycore]
				val = max( val, sc->getSCE(SC_ID_SPEED)->val1 );
			if( sc->getSCE(SC_ID_SPEED) ) //[Easycore]
				val = max( val, sc->getSCE(SC_ID_SPEED)->val1 );
...
static short status_calc_aspd(struct block_list *bl, status_change *sc, bool fixed)
{
	...
	if (fixed) {
		...
		if (bonus < 10 && sc->getSCE(SC_MADNESSCANCEL))
			bonus = 20;
		...
		if (sc->getSCE[SC_ID_ASPD]) //[Easycore]
			bonus -= sc->getSCE[SC_ID_ASPD]->val1;
		if( sc->getSCE(SC_2011RWC_SCROLL) )
			bonus += 5;
		if( sc->getSCE(SC_SPARKCANDY) )
			bonus += 25;
...
void status_calc_slave_mode(struct mob_data *md, struct mob_data *mmd)
{
	switch (battle_config.slaves_inherit_mode) {
		...
		case 4: // Overwrite with slave mode
			sc_start4(NULL, &md->bl, SC_MODECHANGE, 100, 1, MD_CANMOVE|MD_NORANDOMWALK|MD_CANATTACK|MD_CANATTACKMOB, 0, 0, 0);
			break;
		default: //Copy master
		...
		case BL_MOB:
			{
				...
				if (md->fake_guild.guild_id )
					return md->fake_guild.guild_id;
			}
			break;
		case BL_HOM:
			if (((TBL_HOM*)bl)->master)
		...
		case BL_MOB:
			{
				...
				if (md->fake_guild.emblem_id)
					return md->fake_guild.emblem_id;
			}
			break;
		case BL_HOM:
			if (((TBL_HOM*)bl)->master)
 ...
 * @param bl: Object whose view data to get [PC|MOB|PET|HOM|MER|ELEM|NPC|STALL]
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_POISONREACT:
			...
			val2= (val1 + 1) / 2;
#else
			val2 = 5 + val1; // Number of counters [Skotlex]
#endif
...
TIMER_FUNC(status_change_timer){
	...
	case SC_AUTOATTACK:
		if (--(sce->val4) > 0) {
            ...
            //reset target
            if((sd->aa.itempick_id || sd->aa.target_id) && move_ > 5000 && attack_ > 5000){
                sd->aa.itempick_id = 0;
                sd->aa.target_id = 0;
                skip = false;
            }
			...
			//ShowError("5 - timer \n");
			sce->timer = add_timer(200 + tick, status_change_timer, bl->id, data);
		return 0;
	} else {
    ...
    if (type == SC_SIGHTBLASTER) {
        ...
        if (sce->val4 % 2)
            sce->val4--;
        map_foreachinallrange(status_change_timer_sub, bl, sce->val3, BL_CHAR, bl, sce, type, tick); // Removido o BL_SKILL, que inclui armadilhas.
		} else {
        map_foreachinallrange(status_change_timer_sub, bl, sce->val3, BL_CHAR, bl, sce, type, tick); // Mesma modificação, sem BL_SKILL.
        // Removida a linha que revela armadilhas:
...
int status_change_timer_sub(struct block_list* bl, va_list ap)
{
    ...
    {
        if (sce) {
            if (bl->type != BL_SKILL) { // Ignore traps
...
bool aa_teleport(map_session_data *sd){
	int i_;
	bool flywing_ = false, item_used = false;

	// Early return if auto-attack teleport is disabled globally or by feature configuration
    if (!sd->sc.getSCE(SC_AUTOATTACK) || !battle_config.feature_autoattack_teleport) {
        return flywing_;
    }
	...
	if(!sd->aa.teleport.use_flywing && flywing_ == false){
		i_ = pc_search_inventory(sd, 23029);
        ...
        if (i_ >= 0){
            if(item_used == true)
                pc_delitem(sd, i_, 1, 0, 0, LOG_TYPE_OTHER);
            pc_randomwarp(sd, CLR_TELEPORT);
            flywing_ = true;
        }
	}
