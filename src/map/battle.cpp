...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	// PvP Mode
	if (sd && bl->type == BL_PC && src->type == BL_PC && damage && sd->state.pvpmode) {
		if (flag & BF_SKILL) { //Skills get a different reduction than non-skills. [Skotlex]
			if (flag&BF_WEAPON)
				damage = damage * battle_config.pvpmode_weapon_damage_rate / 100;
			if (flag&BF_MAGIC)
				damage = damage * battle_config.pvpmode_magic_damage_rate / 100;
			if (flag&BF_MISC)
				damage = damage * battle_config.pvpmode_misc_damage_rate / 100;
		} else { //Normal attacks get reductions based on range.
			if (flag & BF_SHORT)
				damage = damage * battle_config.pvpmode_short_damage_rate / 100;
			if (flag & BF_LONG)
				damage = damage * battle_config.pvpmode_long_damage_rate / 100;
		}
		damage = i64max(damage,1);
	}
	...
	if( sstatus->cri )
	{
		...
		if(wd->type == DMG_MULTI_HIT){	//Multiple Hit Attack Skills.
			...
			if(pc_checkskill(sd,TF_DOUBLE) && !skill_get_nk(TF_DOUBLE,NK_CRITICAL)) //Double Attack
				return false;
			if(pc_checkskill(sd,AC_CHARGEARROW) && !skill_get_nk(AC_CHARGEARROW,NK_CRITICAL)) //Double Attack
				return false;
		}
	...
	switch (skill_id) {	//Calc base damage according to skill
		...
		case ML_SPIRALPIERCE:
			if (sd) {
				...
				ATK_ADDRATE(wd->damage, wd->damage2, 75*skill_lv); //Skill modifier applies to weight only.
			} else {
				wd->damage = battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc, tstatus->size, 0); //Monsters have no weight and use ATK instead
			}
			...
			switch (tstatus->size) { //Size-fix. Is this modified by weapon perfection?
				case SZ_SMALL: //Small: 140%
					ATK_RATE(wd->damage, wd->damage2, 140);
					break;
				case SZ_MEDIUM: //Medium: 115%
					ATK_RATE(wd->damage, wd->damage2, 115);
					break;
				case SZ_BIG: //Large: 100%
					ATK_RATE(wd->damage, wd->damage2, 100);
					break;
			}
#endif
			break;
...
//					ATK_ADDRATE(wd->damage, wd->damage2, 20 + 15 * skill);
//					RE_ALLATK_ADDRATE(wd, 20 + 15 * skill);
//				}
//#endif
			}
//#ifndef RENEWAL
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case NPC_DARKCROSS:
		case CR_HOLYCROSS:
			if(sd && (sd->status.weapon == W_2HSPEAR || sd->status.weapon == W_2HSWORD))
				skillratio += 50 + 100 * skill_lv;
			else
...
#else
			skillratio += 140 + 60 * skill_lv + sstatus->str;
#endif
		...
		case ASC_METEORASSAULT:
			...
			RE_LVL_DMOD(100);
#else
			skillratio += -60 + 40 * skill_lv + sstatus->str + sstatus->luk;
#endif
			break;
		case SN_SHARPSHOOTING:
			if (src->type == BL_MOB) { // TODO: Did these formulas change in the renewal balancing?
				if (wd->miscflag & 4) // Splash damage bonus
					skillratio += -100 + 190 * skill_lv;
				else
					skillratio += 100 + 100 * skill_lv;
				break;
			}
...
#else
			skillratio += 200 + (sstatus->str/2 + sstatus->int_/3) * skill_lv;
#endif
			if(sd)
		...
		case ASC_BREAKER:
			...
			skillratio += 170 + 170 * skill_lv;
#endif
			break;
		case PA_SACRIFICE:
			skillratio += -11 + 11 * skill_lv;
			break;
		case PA_SHIELDCHAIN:
#ifdef RENEWAL
		...
		case TK_DOWNKICK:
		case TK_STORMKICK:
			skillratio += 60 + 60 * skill_lv;
			break;
		case TK_TURNKICK:
		case TK_COUNTER:
			skillratio += 90 + 70 * skill_lv;
			break;
		case TK_JUMPKICK:
	...
	if( sc ) {
		//SC_FUSION hp penalty [Komurka]
			if (sc->getSCE(SC_FUSION)) {
				...
				hp = 2*hp/1000; //2% hp loss per hit
				status_zap(src, hp, 0);
			}
	...
	if(!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
		...
		if (sd) {
			...
			if (sd->status.weapon == W_KATAR && (katar_skill = pc_checkskill(sd, ASC_KATAR)) > 0) // Adv. Katar Mastery applied after calculate with skillratio.
				ATK_ADDRATE(wd.damage, wd.damage2, (10 + 5 * katar_skill));
		}
	...
	switch (skill_id) {
		...
		case TK_COUNTER:
			if(sd && sd->weapontype1 == W_FIST && sd->weapontype2 == W_FIST)
				ATK_ADD(wd.damage, wd.damage2, 20 * pc_checkskill(sd, TK_RUN));
			break;
		case SR_TIGERCANNON:
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		switch (skill_id) {
			...
			case NPC_EARTHQUAKE:
				...
				MATK_RATE(100 + 50 * skill_lv + 50 * (skill_lv / 2) + ((skill_lv > 4) ? 100 : 0));
				...
				switch(skill_id) {
					...
					case AL_HOLYLIGHT:
						...
						if (sd && sd->sc.getSCE(SC_SPIRIT) && sd->sc.getSCE(SC_SPIRIT)->val2 == SL_PRIEST)
							skillratio *= 3; //Does 5x damage include bonuses from other skills?
						break;
					case AL_RUWACH:
						skillratio += 45;
						break;
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch (skill_id) {
		...
		case HT_LANDMINE:
		case MA_LANDMINE:
			md.damage = static_cast<decltype(md.damage)>(skill_lv * (sstatus->agi + 1.0) * (1.0 + sstatus->int_) / 10.0);
			break;
		case HT_BLASTMINE:
			md.damage = static_cast<decltype(md.damage)>(skill_lv * (sstatus->agi / 10.0 + 5.0) * (3.0 * sstatus->int_) / 2.0);
			break;
		case HT_CLAYMORETRAP:
			md.damage = static_cast<decltype(md.damage)>(skill_lv * (sstatus->agi / 10.0 + 5.0) * (2.0 * sstatus->int_) / 2.0);
			break;
#else
		case HT_LANDMINE:
		case MA_LANDMINE:
		...
		case SN_FALCONASSAULT:
			{
				...
				RE_LVL_MDMOD(100);
#else
				md.damage = (sstatus->batk + sstatus->watk) * (1 + (sstatus->agi / 100.0) + (sstatus->luk / 150.0));
...
#ifndef RENEWAL
		case BA_DISSONANCE: {
			double matk = (sstatus->matk_min + sstatus->matk_max) / 2.0;
			md.damage = matk * (1.0 + (sstatus->int_ / 100.0)); // 100 INT dobra o dano
			break;
		}
		...
		case NPC_EVILLAND:
			...
			break;
		case ASC_BREAKER:
			md.damage = 500 + rnd()%1000 + 5 * skill_lv * ((sstatus->dex + sstatus->str)/3);
			nk.set(NK_IGNOREFLEE);
			break;
#ifndef RENEWAL
		case HW_GRAVITATION:{
			double matk = (sstatus->matk_min + sstatus->matk_max) / 1.0;
			md.damage = matk * (1.0 + (sstatus->int_ / 50.0)); // 100 INT dobra o dano
			break;
		}
		case PA_PRESSURE:
...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,uint16 skill_id,uint16 skill_lv,int flag)
{
	...
	if( sd && battle_config.bg_expansion_enable && map_getmapflag(bl->m,MF_BATTLEGROUND) && sd->bg_id ) {
		map_session_data* tsd = BL_CAST(BL_PC, target);
		if( tsd && tsd->bg_id ) {
			int64 damage = d.damage + d.damage2;
			if( skill_id && sd->bg_id == tsd->bg_id || damage <= 0 ) {
				increment_limit(sd->bge.ranking.skill.fail, 1, USHRT_MAX);
				increment_limit(sd->status.bg_ranking.skill.fail, 1, USHRT_MAX);
			}
			else {
				increment_limit(sd->bge.ranking.skill.success, 1, USHRT_MAX);
				increment_limit(sd->status.bg_ranking.skill.success, 1, USHRT_MAX);
				increment_limit(sd->bge.ranking.skill.damage_given, (int)damage, USHRT_MAX);
				increment_limit(sd->status.bg_ranking.skill.damage_given, (int)damage, USHRT_MAX);
				increment_limit(tsd->bge.ranking.skill.damage_taken, (int)damage, USHRT_MAX);
				increment_limit(tsd->status.bg_ranking.skill.damage_taken, (int)damage, USHRT_MAX);
			}
		}
	}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC: {
			...
			// PvP Mode
			if( sd->state.pvpmode && s_bl->type == BL_PC && (!((TBL_PC*)s_bl)->state.pvpmode || mapdata->getMapFlag(MF_TOWN)) )				return 0; // Cannot target
		...
		case BL_MOB:
		{
			...
			if( battle_config.bg_expansion_enable && mapdata->getMapFlag(MF_BATTLEGROUND) )
				; // [RomuloSM]: Battleground Expansion
			else
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(mapdata->getMapFlag(MF_PVP) && mapdata->getMapFlag(MF_PVP_NOPARTY)) && !(mapdata_flag_gvg(mapdata) && mapdata->getMapFlag(MF_GVG_NOPARTY)) && (!mapdata->getMapFlag(MF_BATTLEGROUND) || sbg_id == tbg_id) )
				state |= BCT_PARTY;
			else if ( battle_config.bg_expansion_enable && mapdata->getMapFlag(MF_BATTLEGROUND) && map_getcell(s_bl->m, s_bl->x, s_bl->y, CELL_CHKNOBGBATTLE))
				state |= BCT_PARTY; // [RomuloSM]: BattleGround Expansion
			else
				state |= BCT_ENEMY;
		}
...
// rAthena
	...
	// [RomuloSM]: Ro Commerce
//	{ "rocommerce_enable",                  &battle_config.rocommerce_enable,               1,      0,      1,              },
	...
	// [RomuloSM]: Battleground Expansion
	{ "bg_expansion_enable",               &battle_config.bg_expansion_enable,             1,      0,       1,              },
	{ "bg_channel",                        &battle_config.bg_channel,                      1,      0,       1,              },
	{ "bg_channel_open",                   &battle_config.bg_channel_open,                 1,      0,       1,              },
	{ "bg_announce",                       &battle_config.bg_announce,                     1,      0,       1,              },
	...
	{ "bg_reward_enable",                  &battle_config.bg_reward_enable,                1,      0,       1,              },
	{ "bg_check_dual",                     &battle_config.bg_check_dual,                   0x00,   0x00,    0x03,           },
	{ "bg_ranking_rows",                   &battle_config.bg_ranking_rows,                 1,      1,       INT_MAX,        },
	{ "bg_castle_defense",                 &battle_config.bg_castle_defense,               1,      1,       100,            },
	{ "bg_interface",                      &battle_config.bg_interface,                    1,      0,       1,              },
	{ "bg_skip_accept",                    &battle_config.bg_skip_accept,                  1,      0,       1,              },
	{ "bg_clothes1color",                  &battle_config.bg_clothes1color,                2,      0,       INT_MAX,        },
	{ "bg_clothes2color",                  &battle_config.bg_clothes2color,                3,      0,       INT_MAX,        },

	// [RomuloSM]: Player Info
	{ "playerinfo_equip_item",             &battle_config.playerinfo_equip_item,       41007,      0,       INT32_MAX,      },
	{ "playerinfo_guild_item",             &battle_config.playerinfo_guild_item,       41008,      0,       INT32_MAX,      },
	{ "playerinfo_bg_item",                &battle_config.playerinfo_bg_item,          41009,      0,       INT32_MAX,      },
	{ "playerinfo_ranked_item",            &battle_config.playerinfo_ranked_item,      41010,      0,       INT32_MAX,      },

	// [RomuloSM]: Force Unequip Items
	{ "unequip_prohibited_items",           &battle_config.unequip_prohibited_items,      1,      1,               1,       },

	// [RomuloSM]: Stuff Items
	{ "stuff_enable",                       &battle_config.stuff_enable,                      1,       0,          1,       },
	{ "stuff_enable_cards",                 &battle_config.stuff_enable_cards,                1,       0,          1,       },
	...
	// PvP Mode
	{ "pvpmode_expbonus",                   &battle_config.pvpmode_expbonus,               10,      0,      100,            },
	{ "pvpmode_dropbonus",                  &battle_config.pvpmode_expbonus,               10,      0,      100,            },
	{ "pvpmode_mutepenalty",                &battle_config.pvpmode_mutepenalty,            60000,   0,      INT_MAX,        },
	{ "pvpmode_exppenalty",                 &battle_config.pvpmode_exppenalty,             10,      0,      100,            },
	{ "pvpmode_expreward",                  &battle_config.pvpmode_expreward,               1,      0,      1,              },
	{ "pvpmode_nowarp_cmd",                 &battle_config.pvpmode_nowarp_cmd,              1,      0,      1,              },
	...
	{ "pvpmode_short_damage_rate",   		&battle_config.pvpmode_short_damage_rate,      80,      0,      INT_MAX,        },
	{ "pvpmode_long_damage_rate",    		&battle_config.pvpmode_long_damage_rate,       80,      0,      INT_MAX,        },
	{ "pvpmode_weapon_damage_rate",  		&battle_config.pvpmode_weapon_damage_rate,     60,      0,      INT_MAX,        },
	{ "pvpmode_magic_damage_rate",   		&battle_config.pvpmode_magic_damage_rate,      60,      0,      INT_MAX,        },
	{ "pvpmode_misc_damage_rate",    		&battle_config.pvpmode_misc_damage_rate,       60,      0,      INT_MAX,        },
	{ "pvpmode_flee_penalty",               &battle_config.pvpmode_flee_penalty,           20,      0,      INT_MAX,        },
	{ "pvpmode_city_only",                  &battle_config.pvpmode_city_only,               1,      0,      1,              },
	{ "petsecurity_item",               	&battle_config.petsecurity_item,           		0,      0,      INT32_MAX,      },
	...
	{ "extended_stall_enable",			    &battle_config.extended_stall_enable,		1,		0,		     1, },
	{ "extended_stall_broadcast",			&battle_config.extended_stall_broadcast,	1,		0,		     1, },
	{ "extended_stall_showboard",			&battle_config.extended_stall_showboard,	1,		0,		     1, },
	{ "extended_stall_showtotal",			&battle_config.extended_stall_showtotal,	1,		0,		     1, },

	// [RomuloSM]: Extended Searchstore
	{ "extended_searchstore_enable",		&battle_config.extended_searchstore_enable,	1,		0,		     1, },

	// [RomuloSM]: Extended Buingstore
	{ "extended_buyingstore_enable",		&battle_config.extended_buyingstore_enable,	    1,	0,		     1, },
	{ "extended_buyingstore_showboard",		&battle_config.extended_buyingstore_showboard,	1,	0,		     1, },

	// [RomuloSM]: Player Bot
	{ "player_bot_hpbar_green",             &battle_config.player_bot_hpbar_green,          1,      0,      1,              },
