...
void map_reqnickdb(map_session_data * sd, int charid)
{
	...
	if( battle_config.stuff_enable ) {
		// Battleground
		if( charid == battle_config.stuff_bg_reserved_id ) {
			clif_solved_charname(sd->fd, charid, msg_txt(sd,1770));
			return;
		}
	
		// GvG
		if( charid == battle_config.stuff_gvg_reserved_id ) {
			clif_solved_charname(sd->fd, charid, msg_txt(sd,1772));
			return;
		}
	
		// PvP
		if( charid == battle_config.stuff_pvp_reserved_id ) {
			clif_solved_charname(sd->fd, charid, msg_txt(sd,1771));
			return;
		}
	}
...
int map_quit(map_session_data *sd) {
	...
	if (sd->bg_id)
		bg_team_leave(sd, true, true, BGTL_QUIT);
...
int map_getcellp(struct map_data* m,int16 x,int16 y,cell_chk cellchk)
{
	...
	switch(cellchk)
	{
		...
		case CELL_CHKNOBGBATTLE:
			return (cell.bg_battle);

		case CELL_CHKNOBGSKILLCALL:
			return (cell.bg_skillcall);
...
void map_setcell(int16 m, int16 x, int16 y, cell_t cell, bool flag)
{
	...
	switch( cell ) {
		...
		// [RomuloSM]: Battleground Expansion
		case CELL_NOBGBATTLE:    mapdata->cell[j].bg_battle = flag;		  break;
		case CELL_NOBGSKILLCALL: mapdata->cell[j].bg_skillcall = flag;    break;
		default:
