...
/*==========================================
 * @market - Warps player to vendarea
 *------------------------------------------*/
ACMD_FUNC(market)
{
    char map_name[MAP_NAME_LENGTH_EXT];  // Declaração do map_name
    short x = 0, y = 0;
    int16 m = -1;
    ...
    // Definir o mapa fixo como vendarea
    safestrncpy(map_name, "vendarea", sizeof(map_name));

    // Verifica se há coordenadas fornecidas na mensagem
    if (message && *message && sscanf(message, "%6hd %6hd", &x, &y) < 2) {
        clif_displaymessage(fd, "Uso: @market <x> <y> (opcional).");
        return -1;
    }
    ...
    // Verifica permissões de entrada e saída
    ...
    // Move o jogador para Prontera nas coordenadas especificadas ou aleatórias
    if (pc_setpos(sd, mapindex, x, y, CLR_TELEPORT) != SETPOS_OK) {
        clif_displaymessage(fd, msg_txt(sd, 1)); // Map not found.
        return -1;
    }

    clif_displaymessage(fd, "Você foi teleportado para o mercado de Vendarea.");
    return 0;
}
...
ACMD_FUNC(mapmove)
{
	...
	{
		clif_displaymessage(sd->fd,"Você não pode usar @warp enquanto estiver no modo PvP.");
		return -1;
	}
	if (pc_setpos(sd, mapindex, x, y, CLR_TELEPORT) != SETPOS_OK) {
		clif_displaymessage(fd, msg_txt(sd,1)); // Map not found.
...
ACMD_FUNC(who) {
	...
	for(;;)
	{
		TBL_MOB* md = (TBL_MOB*)mapit_next(iter);
		if( md == nullptr )
			break;// no more mobs
		...
		if( map_id > 0 && md->bl.m != map_id )
			continue;

		switch (display_type) {
			case 2: {
				StringBuf_Printf(&buf, msg_txt(sd,343), md->name); // "Name: %s "
				StringBuf_Printf(&buf, msg_txt(sd,347), md->level, md->job_level,
					job_name(md->vd->class_)); // "| Lv:%d/%d | Job: %s"
				break;
			}
			case 3: {
				struct map_data *mapdata = map_getmapdata(md->bl.m);
				StringBuf_Printf(&buf, msg_txt(sd,343), md->name); // "Name: %s "
				StringBuf_Printf(&buf, msg_txt(sd,348), mapdata->name, md->bl.x, md->bl.y); // "| Location: %s %d %d"
				break;
			}
			default: {
				StringBuf_Printf(&buf, msg_txt(sd,343), md->name); // "Name: %s "
...
ACMD_FUNC(item2)
{
    ...
    if (!(item_data->equip & EQP_HEAD_LOW) &&
        ...
        !(item_data->equip & EQP_GARMENT) &&
        !(item_data->equip & EQP_COSTUME_GARMENT))
...
ACMD_FUNC(go)
{
	...
	if( battle_config.pvpmode_nowarp_cmd && sd->state.pvpmode )
	{
		clif_displaymessage(sd->fd,"Você não pode usar @go enquanto estiver no modo PvP.");
		return -1;
	}
	...
	if (strncmp(map_name, "prontera", 3) == 0) {
		...
		town = 36;
	} else if (strncmp(map_name, "moc_para01", 3) == 0) {
		town = 37;
	}
...
ACMD_FUNC(equipoff) {
	...
	if (strcmp(parent_cmd, "equipoff") == 0) {
		if ((sscanf(message, "%s", &type)) < 1) {
			clif_displaymessage(fd, msg_txt(sd, 1834)); // Syntax: @equipoff <type>
			clif_displaymessage(fd, msg_txt(sd, 1835)); // Type: all - all players | others - other players
			return -1;
		}
		...
		if ((strncmp(type, "others", 3) == 0))
			value += 1;

		sd->state.equiphide = value;
		clif_refresh(sd);
		clif_status_change(&sd->bl, EFST_EQUIP_HIDE, 1, INFINITE_TICK, 0, 0, 0);

		if ((strncmp(message, "others", 3) == 0) || (value&2)) {
		...
		clif_displaymessage(fd,  msg_txt(sd, 1833)); // Use @equipon to disable the system.
		clif_displaymessage(fd, msg_txt(sd, 1831)); // EquipOff activated!

	} else if (strcmp(parent_cmd, "equipon") == 0) {
		sd->state.equiphide = 0;
		clif_refresh(sd);
		clif_status_change(&sd->bl, EFST_EQUIP_HIDE, 0, INVALID_TIMER, 0, 0, 0);
		clif_displaymessage(fd, msg_txt(sd, 1832)); // EquipOff disabled!
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_TOP, sd->status.head_top, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_MID, sd->status.head_mid, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_BOTTOM, sd->status.head_bottom, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_ROBE, sd->status.robe, SELF);
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		for (auto &bg : bg_queues) {
				for (auto &bg_sd : bg->teama_members)
					bg_team_leave(bg_sd, false, false, BGTL_KICK); // Kick Team A from battlegrounds
				for (auto &bg_sd : bg->teamb_members)
					bg_team_leave(bg_sd, false, false, BGTL_KICK); // Kick Team B from battlegrounds
...
ACMD_FUNC(mapinfo) {
	...
	if (restock_check_zone(m_id))
		strcat(atcmd_output, " RestockOn |");
	else
		strcat(atcmd_output, " RestockOff |");	
 ...
 *------------------------------------------*/
ACMD_FUNC(disguisevirousegredo)
{
	int id = 0;
	nullpo_retr(-1, sd);
 ...
 * DisguiseAll
 *------------------------------------------*/
ACMD_FUNC(disguiseallvirousegredo)
{
	int mob_id=0;
	map_session_data *pl_sd;
	struct s_mapiterator* iter;
...
ACMD_FUNC(hideslave)
{
	...
	if (!message || !*message || (option = atoi(message)) < 0 || option > 7) {
		clif_displaymessage(fd, "Escolha um valor de op  o (usage: @hideslave <0=disable,1=pet,2=homun,4=abr and summon>)");
		clif_displaymessage(fd, "Voc  pode adicionar op  es para ocultar v rios tipos : @hideslave 3 would hide pet and homun, @hideslave 7 everything");
		return -1;
	}

	sd->state.hideslave = option;
	clif_refresh(sd);
	clif_displaymessage(fd, "@hideslave a op  o foi modificada");
	return 0;
}
...
ACMD_FUNC(mobinfo)
{
	...
	for (uint16 k = 0; k < count; k++) {
		...
		if (sd->custom_rate.state){
			base_exp = (base_exp * sd->custom_rate.base);
			job_exp = (job_exp * sd->custom_rate.job);
		}
       ...
       // Exibe bônus ativo se existir
       if (sd->custom_rate.state && sd->custom_rate.drop > 0) {
               sprintf(atcmd_output, " [Drop Bonus Ativo]: +%d%% (%s)", sd->custom_rate.drop,
                       sd->custom_rate.drop == 1 ? "Drop Personalizado" : "Multiplicador Personalizado");
               clif_displaymessage(fd, atcmd_output);
       }
       if (sd->sc.getSCE(SC_ITEMBOOST)) {
       	int itemboost = sd->sc.getSCE(SC_ITEMBOOST)->val1;
       	sprintf(atcmd_output, " [Drop Booster]: +%d%% (Status Effect)", itemboost);
	       clif_displaymessage(fd, atcmd_output);
       }
		strcpy(atcmd_output, " ");
		unsigned int j = 0;
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			int droprate = mob->dropitem[i].rate;

			if (sd->custom_rate.state && sd->custom_rate.drop > 0) {
	std::shared_ptr<item_data> id = item_db.find(mob->dropitem[i].nameid);
				if (id) {
		bool is_card = id->type == IT_CARD;
		bool is_boss = mob->get_bosstype() == BOSSTYPE_MVP || mob->get_bosstype() == BOSSTYPE_MINIBOSS;
		int bonus = (is_card && is_boss) ? sd->custom_rate.drop / 5 : sd->custom_rate.drop;

		droprate = (int)(droprate * (100 + bonus) / 100.0);
				}
			}

	// Aplica bônus de raça e classe
	int race_bonus = sd->indexed_bonus.dropaddrace[mob->status.race] + sd->indexed_bonus.dropaddrace[RC_ALL];
	int class_bonus = sd->indexed_bonus.dropaddclass[mob->status.class_] + sd->indexed_bonus.dropaddclass[CLASS_ALL];
	if (race_bonus != 0 || class_bonus != 0) {
		droprate = (int)(0.5 + droprate * (1. + (race_bonus + class_bonus) / 100.0));
	}

// Aplica bônus do status effect SC_ITEMBOOST
			if (sd->sc.getSCE(SC_ITEMBOOST)) {
	droprate = (int)(droprate * (100 + sd->sc.getSCE(SC_ITEMBOOST)->val1) / 100.0);
			}
	...
	droprate = 10000;
		...
		if( mob->get_bosstype() == BOSSTYPE_MVP ){
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				int mvp_rate = mob_getdroprate(&sd->bl, mob, mob->mvpitem[i].rate, drop_modifier);
				if (mvp_rate > 10000)
                       mvp_rate = 10000; // Cap visual MVP
				mvppercent = (float)mvp_rate / 100.0f;
...
ACMD_FUNC(rates)
{
	...
	if(sd->custom_rate.state)
	{
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1298), // Experience rates: Base %.2fx / Job %.2fx
			((battle_config.base_exp_rate * sd->custom_rate.base) + (pc_isvip(sd) ? (battle_config.vip_base_exp_increase * battle_config.base_exp_rate) / 100 : 0)) / 100.,
			((battle_config.job_exp_rate * sd->custom_rate.job) + (pc_isvip(sd) ? (battle_config.vip_job_exp_increase * battle_config.job_exp_rate) / 100 : 0)) / 100.);
		...
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			((battle_config.item_rate_common) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config.item_rate_common) / 100) / 100.,
			((battle_config.item_rate_heal) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_heal) / 100) / 100.,
			((battle_config.item_rate_use) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_use) / 100) / 100.,
			((battle_config.item_rate_equip) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_equip) / 100) / 100.,
			((battle_config.item_rate_card) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_card) / 100) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			((battle_config.item_rate_common_boss) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_common_boss) / 100) / 100.,
			((battle_config.item_rate_heal_boss) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_heal_boss) / 100) / 100.,
			((battle_config.item_rate_use_boss) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_use_boss) / 100) / 100.,
			((battle_config.item_rate_equip_boss) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_equip_boss) / 100) / 100.,
			((battle_config.item_rate_card_boss) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_card_boss) / 100) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			((battle_config.item_rate_common_mvp) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_common_mvp) / 100) / 100.,
			((battle_config.item_rate_heal_mvp) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_heal_mvp) / 100) / 100.,
			((battle_config.item_rate_use_mvp) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_use_mvp) / 100) / 100.,
			((battle_config.item_rate_equip_mvp) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_equip_mvp) / 100) / 100.,
			((battle_config.item_rate_card_mvp) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_card_mvp) / 100) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
			((battle_config.item_rate_mvp) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_mvp) / 100) / 100.,
			((battle_config.item_rate_adddrop) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_adddrop) / 100) / 100.,
			((battle_config.item_rate_treasure) + (pc_isvip(sd) ? battle_config.vip_drop_increase : 0 + (sd->custom_rate.drop > 1 ? sd->custom_rate.drop : 0) * battle_config .item_rate_treasure) / 100) / 100.);
		clif_displaymessage(fd, buf);
	}
	else
	{
...
ACMD_FUNC(sacerdotea)
{
    int x = 0, y = 0, i = 0;

    do {
        x = sd->bl.x + rnd_value(-5, 5); // Gera coordenadas x aleat rias pr ximas ao jogador
        y = sd->bl.y + rnd_value(-5, 5); // Gera coordenadas y aleat rias pr ximas ao jogador
    } while (map_getcell(sd->bl.m, x, y, CELL_CHKNOPASS) && i++ < 10); // Repete se a c lula est  bloqueada e n o ultrapassou 10 tentativas

    if (i >= 10) {
        x = sd->bl.x; // Se ultrapassou 10 tentativas, define x como a coordenada x do jogador
        y = sd->bl.y; // Define y como a coordenada y do jogador
    }

    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_SACERDOTEA; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Sacerdote Rank A", class_, "", SZ_SMALL, ai); // Nome do mob definido como "Sacerdote Rank A"

        if (md) {
            md->master_id = sd->bl.id; // Define o ID do mestre do mob (jogador que invocou)
            md->special_state.ai = ai; // Define a AI especial do mob

            // Define o tempo de vida do mob para 1 minuto (60 segundos)
            if (md->deletetimer != INVALID_TIMER)
                delete_timer(md->deletetimer, mob_timer_delete);
            md->deletetimer = add_timer(gettick() + 900000, mob_timer_delete, md->bl.id, 0);

            mob_spawn(md); // Prepara para o mob aparecer no mapa
            clif_displaymessage(fd, "Clone invocado com sucesso."); // Mensagem de sucesso para o jogador
            return 0; // Finaliza a fun  o
        }
    }
    clif_displaymessage(fd, "Você não pode invocar o clone.."); // Mensagem de falha para o jogador
    return 0;
}

ACMD_FUNC(sacerdoteb)
{
	int x = 0, y = 0, i = 0;
	...
	// Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
	{
		int class_ = MOBID_SACERDOTEB; // Define o ID do mob planta a ser invocado
		enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

		struct mob_data* md;

		// Invoca o mob no mapa nas coordenadas x, y
		md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Sacerdote Rank B", class_, "", SZ_SMALL, ai); // Nome do mob definido como "Sacerdote Rank B"
...
ACMD_FUNC(palaa)
{
    ...
    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_PALAA; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Paladino Rank A", class_, "", SZ_SMALL, ai); // Nome do mob definido como "Sacerdote Rank B"
...
ACMD_FUNC(palab)
{
    ...
    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_PALAB; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Paladino Rank B", class_, "", SZ_SMALL, ai); // Nome do mob definido como "Sacerdote Rank B"
...
ACMD_FUNC(professora)
{
    ...
    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_PROFESSORA; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Professor Rank A", class_, "", SZ_SMALL, ai); // Nome do mob definido como "Sacerdote Rank B"
...
ACMD_FUNC(professorb)
{
    ...
    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_PROFESSORB; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Professor Rank B", class_, "", SZ_SMALL, ai); // Nome do mob definido como "PROFESSOR RANK B"
...
ACMD_FUNC(bardoa)
{
    ...
    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_BARDOA; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Menestrel Rank A", class_, "", SZ_SMALL, ai); // Nome do mob definido como "PROFESSOR RANK B"
...
ACMD_FUNC(bardob)
{
    ...
    // Aqui come a a parte adaptada da skill AM_CANNIBALIZE para invocar o mob planta
    {
        int class_ = MOBID_BARDOB; // Define o ID do mob planta a ser invocado
        enum mob_ai ai = AI_NONE; // Define o tipo de AI do mob planta (no caso, nenhuma AI)

        struct mob_data *md;

        // Invoca o mob no mapa nas coordenadas x, y
        md = mob_once_spawn_sub(&sd->bl, sd->bl.m, x, y, "Menestrel Rank B", class_, "", SZ_SMALL, ai); // Nome do mob definido como "PROFESSOR RANK B"
...
int buildin_autoattack_sub2(struct block_list* bl, va_list ap)
{
	int* target_id = va_arg(ap, int*);
	*target_id = bl->id; // Captura o ID do alvo
	return 1;
}

void autoattack_motion(class map_session_data* sd)
{
	int i, target_id;

	for (i = 0; i <= 9; i++) // Varre a área ao redor do jogador
	{
		target_id = 0;
		map_foreachinarea(buildin_autoattack_sub2,
			sd->bl.m,
			sd->bl.x - i, sd->bl.y - i,
			sd->bl.x + i, sd->bl.y + i,
			BL_MOB, &target_id);
		if (target_id) // Se um alvo for encontrado, ataca
		{
	...
	if (!target_id) // Se nenhum alvo for encontrado, movimenta o jogador aleatoriamente
	{
		int rand_x = sd->bl.x + (rand() % 2 == 0 ? -1 : 1) * (rand() % 10);
		int rand_y = sd->bl.y + (rand() % 2 == 0 ? -1 : 1) * (rand() % 10);
		unit_walktoxy(&sd->bl, rand_x, rand_y, 0);
	}
}

int autoattack_timer(int tid, t_tick tick, int id, intptr_t data)
{
	class map_session_data* sd = NULL;

	sd = map_id2sd(id); // Obtém os dados da sessão pelo ID
	if (sd == NULL) // Valida se o jogador ainda existe
		return 0;

	if (sd->sc.option & OPTION_AUTOATTACK) // Verifica se o modo auto-attack está ativo
	{
		autoattack_motion(sd); // Executa a lógica de ataque automático
		add_timer(gettick() + 2000, autoattack_timer, sd->bl.id, 0); // Configura o próximo ataque
	}
	return 0;
}
...
{
	nullpo_retr(-1, sd); // Valida a sessão

	if (sd->sc.option & OPTION_AUTOATTACK) // Verifica se o auto-attack está ativado
	{
		clif_displaymessage(fd, "Auto-attack OFF"); // Mensagem para o jogador
		sd->sc.option &= ~OPTION_AUTOATTACK; // Desativa o auto-attack
		unit_stop_attack(&sd->bl); // Para qualquer ataque em andamento
	}
	else
	{
		clif_displaymessage(fd, "Auto-attack ON"); // Mensagem para o jogador
		sd->sc.option |= OPTION_AUTOATTACK; // Ativa o auto-attack
		add_timer(gettick() + 200, autoattack_timer, sd->bl.id, 0); // Inicia o temporizador
	}

	clif_changeoption(&sd->bl); // Atualiza a opção do jogador
	return 0;
}
...
ACMD_FUNC(ele_resist) {
	...
	struct {
		...
		{ "   [ %d ] de Resist ncia ao Elemento Neutro", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento  gua", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Terra", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Fogo", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Vento", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Veneno", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Sagrado", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Sombrio", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Fantasma", 0 },
		{ "   [ %d ] de Resist ncia ao Elemento Maldito", 0 },
//		{ "   [ %d ] de Resist ncia   todos os Elementos", 0 },
		{ NULL, 0 }
	};
	memset(output, '\0', sizeof(output));
	clif_displaymessage(sd->fd, "======== Resist ncia Elemental em % ========");
...
/*=========================================
 * Verifica os valores de Resist ncia  s ra as
 * Adapta  o livre do comando resist feito pela [ Keitenai ] e dos ajustes feitos por [ DietmarRuiz ]
...
ACMD_FUNC(race_resist) {
	...
	struct {
		...
		{ "   [ %d ] de Resist ncia a ra a Anjo", 0 },
		{ "   [ %d ] de Resist ncia a ra a Bruto", 0 },
		{ "   [ %d ] de Resist ncia a ra a Demi-Humano", 0 },
		{ "   [ %d ] de Resist ncia a ra a Demonio", 0 },
		{ "   [ %d ] de Resist ncia a ra a Drag o", 0 },
		{ "   [ %d ] de Resist ncia a ra a Peixe", 0 },
		{ "   [ %d ] de Resist ncia a ra a Amorfo", 0 },
		{ "   [ %d ] de Resist ncia a ra a Inseto", 0 },
		{ "   [ %d ] de Resist ncia a ra a Planta", 0 },
		{ "   [ %d ] de Resist ncia a ra a Humanoide", 0 },
		{ "   [ %d ] de Resist ncia a ra a Morto-Vivo", 0 },
		{ "   [ %d ] de Resist ncia a ra a Doram", 0 },
// 	{ "   [ %d ] de Resist ncia   todas as Ra as ", 0 },
		{ NULL, 0 }
	};
	memset(output, '\0', sizeof(output));
	clif_displaymessage(sd->fd, "======== Resist ncia   Ra a em % ========");
...
ACMD_FUNC(afk) {
	...
	if (map_getmapflag(sd->bl.m, MF_AUTOTRADE) == battle_config.autotrade_mapflag && !map_getmapflag(sd->bl.m, MF_NOAFK)) {
		...
		if (map_getmapflag(sd->bl.m, MF_PVP) || map_getmapflag(sd->bl.m, MF_GVG)) {
			clif_displaymessage(fd, msg_txt(sd, 1839));
			return -1;
		}
		...
		clif_displaymessage(fd, msg_txt(sd, 1840));
		return -1;
	}
...
// [RomuloSM]: Battleground Expansion
// Reload Battleground DB
ACMD_FUNC(bgreloadrewarddb) {
	nullpo_retr(-1,sd);

	bg_reward_reload();
	clif_displaymessage(sd->fd, msg_txt(sd,1680));
	return 0;
}

// Start Battlegound Happy Hour
ACMD_FUNC(bghappyhourstart) {
	nullpo_retr(-1,sd);

	if( bg_happyhour_start() )
		clif_displaymessage(sd->fd, msg_txt(sd,1681));
	else
		clif_displaymessage(sd->fd, msg_txt(sd,1682));
	return 0;
}

// End Battlegound Happy Hour
ACMD_FUNC(bghappyhourend) {
	nullpo_retr(-1,sd);

	if( bg_happyhour_end() )
		clif_displaymessage(sd->fd, msg_txt(sd,1683));
	else
		clif_displaymessage(sd->fd, msg_txt(sd,1684));
	return 0;
}
...
ACMD_FUNC(bgorder)
{
	...
	if( !message || !*message )
	{
		clif_displaymessage(fd, msg_txt(sd,1687));
		return -1;
	}
	...
	if( map_getmapflag(sd->bl.m,MF_BATTLEGROUND) && sd->bg_id )
	{
		...
		if( bg == nullptr || bg->expansion.master_id != sd->status.char_id )
		{
			clif_displaymessage(fd, msg_txt(sd,1886));
			return -1;
		}
		clif_broadcast2(&sd->bl, message, (int)strlen(message)+1, BGE_COLOR, 0x190, 12, 0, 0, BG);
		return 0;
	}

	clif_displaymessage(fd, msg_txt(sd,1885));
	return -1;
}
...
ACMD_FUNC(bgleader)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 1688), command);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	...
	if (!sd->bg_id) {
		clif_displaymessage(fd, msg_txt(sd, 1685));
		return -1;
	}
	...
	if( sd->bg_id ) {
		...
		if( bg == nullptr || bg->expansion.master_id != sd->status.char_id )
			clif_displaymessage(fd, msg_txt(sd,1886));
		else if( bg->expansion.master_id == pl_sd->status.char_id )
			clif_displaymessage(fd, msg_txt(sd,1689));
		else if( pl_sd->bg_id != sd->bg_id )
			clif_displaymessage(fd, msg_txt(sd,1690));
		else if( sd->ud.skilltimer != INVALID_TIMER || bg_check_skills_time(bg) == false )
			clif_displaymessage(fd, msg_txt(sd,1691));
		else {
			bg->expansion.master_id = pl_sd->status.char_id;
			bg_player_data(sd);
			bg_player_data(pl_sd);
			clif_displaymessage(fd, msg_txt(sd,1692));
			sprintf(atcmd_output, msg_txt(sd,1693), pl_sd->status.name);
			clif_broadcast2(&sd->bl, atcmd_output, (int)strlen(atcmd_output)+1, BGE_COLOR, 0x190, 20, 0, 0, BG);
			return 0;
		}
		return -1;
	}
...
ACMD_FUNC(bgcountdown)
{
	...
	if( sd->bge.countdown_hidden == false ) {
		sd->bge.countdown_hidden = true;
		clif_showdigit(sd, (unsigned char)3, 0);
		clif_displaymessage(fd, msg_txt(sd, 1696));
	}
	else {
		sd->bge.countdown_hidden = false;
		clif_displaymessage(fd, msg_txt(sd, 1695));
	}
	return 0;
}
...
// Ranking
ACMD_FUNC(bgranking) {
	struct npc_data *nd = npc_name2id("Battleground#ranking");

	nullpo_retr(-1, sd);

	if( nd == NULL ) {
		clif_displaymessage(sd->fd, msg_txt(sd, 1697));
		return -1;
	}
...
// Reset SC Entry
ACMD_FUNC(bgresetentry)
{
	nullpo_retr(-1,sd);
	sc_start(nullptr, &sd->bl, SC_ENTRY_QUEUE_APPLY_DELAY, 100, 1, 0);
	sc_start(nullptr, &sd->bl, SC_ENTRY_QUEUE_NOTIFY_ADMISSION_TIME_OUT, 100, 1, 0);
	clif_displaymessage(fd, msg_txt(sd, 1700));
	return 0;
}
...
ACMD_FUNC(bglisten)
{
	if( sd->bge.listen ) {
		...
		sd->bge.listen = 1;
		clif_displaymessage(fd, msg_txt(sd, 7002));
	}

	pc_setglobalreg(sd, add_str(BG_LISTEN_VAR), sd->bge.listen);
	return 0;
}
...
ACMD_FUNC(whosell){
	...
	if (!*message) {
		clif_displaymessage(fd, "Use: @whosell <+refine> <Nome ou Id>[card id]");
		return -1;
	}
	...
	if ((item_data = item_db.searchname(item_name)) == NULL){
		clif_displaymessage(fd, "N o foi encontrado item com este nome");
		return -1;
	}
		item_id = item_data->nameid;
	} else {
		clif_displaymessage(fd, "Use: @whosell <item_id> or @whosell <Nome>");
		return -1;
	}

	//check card
	if(s_type & 2 && ((item_data = item_db.find(card_id)) == NULL || item_data->type != IT_CARD)){
		clif_displaymessage(fd, "N o foi encontrado uma carta com esse id");
		return -1;
	}
	//check item
	if(s_type & 1 && (item_data = item_db.find(item_id)) == NULL){
		clif_displaymessage(fd, "N o foi encontrado um item com esse id");
		return -1;
	}
	//check refine
	if(s_type & 4){
		if (refine<0 || refine>10){
			clif_displaymessage(fd, "Refine fora dos limites: 0 - 10");
			return -1;
		}
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
		if (pl_sd->vender_id ) {	 // check if player is vending
			for (j = 0; j < pl_sd->vend_num; j++) {
				...
				snprintf(atcmd_output, CHAT_SIZE_MAX, "%s - Pre o: %uz - Quantidade: %d - %s %d,%d - %s"
	...
	mapit_free(iter);
	if(count > 0) {
		snprintf(atcmd_output, CHAT_SIZE_MAX, "%d Lojas vendendo.", count);
		clif_displaymessage(fd, atcmd_output);
	} else
		clif_displaymessage(fd, "Ninguem esta vendendo agora.");
...
ACMD_FUNC(stuffitem)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, msg_txt(sd,1773)); // Please enter an item name or ID (usage: @stuffitem <item name/ID> <amount> <quantity>).
		clif_displaymessage(fd, msg_txt(sd,1774));
		clif_displaymessage(fd, msg_txt(sd,1775));
		clif_displaymessage(fd, msg_txt(sd,1776));
		clif_displaymessage(fd, msg_txt(sd,1777));
		return -1;
	}

	if( type < 1 || type > 3 ) {
		clif_displaymessage(fd, msg_txt(sd,1778));
		clif_displaymessage(fd, msg_txt(sd,1773)); // Please enter an item name or ID (usage: @stuffitem <item name/ID> <amount> <quantity>).
		clif_displaymessage(fd, msg_txt(sd,1774));
		clif_displaymessage(fd, msg_txt(sd,1775));
		clif_displaymessage(fd, msg_txt(sd,1776));
	...
	while( itemlist != nullptr ){
		...
		if (item->type == IT_ARMOR || item->type == IT_WEAPON || item->type == IT_PETARMOR) {
			if( type == 1 )
			...
			if( option_id <= 0 ) {
				char output[128];
				sprintf(output, msg_txt(sd,1779), msg_txt(sd,1768+type));
				clif_displaymessage(fd, output);
				return 0;
			}
		}
	...
	for( const auto& item : items ){
		...
		std::shared_ptr<item_data> i_data = item;
		if( i_data->type == IT_ARMOR || i_data->type == IT_WEAPON || i_data->type == IT_PETARMOR ) {
			subtype = 1;
			option_id = 0;
			if( type == 1 )
				random_option_db.option_get_id("STUFF_BATTLEGROUND", option_id);
		...
		for( int i = 0; i < number; i += get_count ){
			...
			if (!pet_create_egg(sd, item_id)) {
				...
				if( subtype ) {
					item_tmp.option[0].id = option_id;
					item_tmp.option[0].param = 0;
					item_tmp.option[0].value = 0;
				}
				else {
					item_tmp.card[0] = CARD0_CREATE;
...
ACMD_FUNC(pvpmode)
{
	...
	{
    	clif_displaymessage(fd, "Você não pode entrar em modo PvP em um duelo.");
    	return -1;
    }
...
ACMD_FUNC(restockadd)
{
	...
	if( !battle_config.restock_enable ) {
		clif_displaymessage(fd, msg_txt(sd,1786));
		return 0;
	}
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, msg_txt(sd,1787));
		clif_displaymessage(fd, msg_txt(sd,1788));
		return -1;
	}

	if( number <= 0 || number > battle_config.restock_max_stack ) {
		sprintf(output, msg_txt(sd,1789), battle_config.restock_max_stack);
		clif_displaymessage(fd, output);
		return -1;
	}
	...
	if (id->type != IT_HEALING && id->type != IT_USABLE && id->type != IT_ETC && id->type != IT_AMMO && id->type != IT_DELAYCONSUME) {
		clif_displaymessage(fd, msg_txt(sd,1846));
		clif_displaymessage(fd, msg_txt(sd,1847));
		clif_displaymessage(fd, msg_txt(sd,1848));
		return -1;
	}
	...
	if( zeny && sd->status.zeny < zeny ) {
		clif_displaymessage(fd, msg_txt(sd,1799));
		return -1;
	}
	...
	if( restock_add_item(sd, id->nameid, number) ) {
		...
		sprintf(output, msg_txt(sd,1794), id->ename.c_str());
		clif_displaymessage(fd, output);
	}
	else {
		clif_displaymessage(fd, msg_txt(sd,1787));
		clif_displaymessage(fd, msg_txt(sd,1792));
		clif_displaymessage(fd, msg_txt(sd,1793));
	}
	return 0;
}
...
ACMD_FUNC(restockrem)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, msg_txt(sd,1790));
		clif_displaymessage(fd, msg_txt(sd,1791));
		return -1;
	}
	...
	if( restock_del_item(sd, id->nameid) ) {
		sprintf(output, msg_txt(sd,1795), id->ename.c_str());
		clif_displaymessage(fd, output);
	}
	else {
		clif_displaymessage(fd, msg_txt(sd,1790));
...
ACMD_FUNC(restocklist)
{
	...
	if( !count )
		clif_displaymessage(fd, msg_txt(sd,1797));
	else {
		clif_displaymessage(fd, msg_txt(sd,1796));
		for( i=0; i < max; i++ ) {
			if( sd->status.restock[i].nameid == 0 || sd->status.restock[i].amount == 0 )
				continue;

			std::shared_ptr<item_data> id = item_db.find(sd->status.restock[i].nameid);
			if( id == NULL )
				continue;
			sprintf(output, " - (ID: %d) %dx %s", sd->status.restock[i].nameid, sd->status.restock[i].amount, id->ename.c_str());
			clif_displaymessage(fd, output);
			n++;
		}

		sprintf(output, msg_txt(sd,1798), n);
		clif_displaymessage(fd, output);
	}
	return 0;
}
...
ACMD_FUNC(restock)
{
	...
	if (nd == NULL)
	{
		clif_displaymessage(sd->fd, msg_txt(sd, 1800));
		return -1;
	}
...
#include <custom/atcommand_def.inc>
		...
		ACMD_DEF(disguiseallvirousegredo),
		ACMD_DEF(changelook),
		ACMD_DEF(autoloot),
		...
		// [RomuloSM]: Battleground Expansion
		ACMD_DEF(bgreloadrewarddb),
		ACMD_DEF(bghappyhourstart),
		ACMD_DEF2("bghappystart", bghappyhourstart),
		ACMD_DEF(bghappyhourend),
		ACMD_DEF2("bghappyend", bghappyhourend),
		ACMD_DEF(bgorder),
		ACMD_DEF(bgleader),
		ACMD_DEF(bgcountdown),
		ACMD_DEF(bgranking),
		ACMD_DEF(bgresetentry),
		ACMD_DEF(bglisten),

		// [RomuloSM]: Stuff Items
		ACMD_DEF(stuffitem),

		ACMD_DEF(pvpmode),	// PvP Mode
