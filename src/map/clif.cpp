...
static inline uint16 laphine_index( uint16 client_index ){
	return client_index - 3;
}
...
static int clif_send_sub(struct block_list *bl, va_list ap)
{
	...
	case AREA_WOSC:
	{
		...
		// [RomuloSM]: Player Bot
			...
			struct mob_data *md = (struct mob_data *)src_bl;
			if (md && sd->chatID && (sd->chatID == md->chat_id))
			return 0;
		}
	}
	break;
...
int clif_send(const void* buf, int len, struct block_list* bl, enum send_target type)
{
	...
	case BG_LISTEN: // [RomuloSM]: Battleground Expansion
		iter = mapit_getallusers();
		while( ( tsd = (map_session_data*)mapit_next( iter ) ) != nullptr ){
			if( session_isActive( fd = tsd->fd ) ){
				if( type == BG_LISTEN && !(tsd->bge.listen || tsd->bg_queue_id) ) // [RomuloSM]: Battleground Expansion
					continue;
				WFIFOHEAD( fd, len );
				memcpy( WFIFOP( fd, 0 ), buf, len );
				WFIFOSET( fd, len );
...
int clif_spawn( struct block_list *bl, bool walking ){
	...
	if( bl->type == BL_MOB && vd->dead_sit == 2 )
		clif_spawn_unit( bl, AREA_WOS );
	else if( bl->type == BL_NPC && !vd->dead_sit ){
...
//			clif_send_colornicks(sd);
			...
			if (map_getmapflag(sd->bl.m, MF_EQUIP_HIDE)) {
				map_foreachinallrange(clif_equiphide, &sd->bl, AREA_SIZE, BL_PC, &sd->bl);
				clif_status_change(&sd->bl, EFST_EQUIP_HIDE, 1, INFINITE_TICK, 0, 0, 0);
			}
			else {
				clif_status_change(&sd->bl, EFST_EQUIP_HIDE, 0, INVALID_TIMER, 0, 0, 0);
			...
			if( battle_config.bg_expansion_enable && sd->bg_id )
				bg_player_data(sd);
	...
	case BL_MOB:
		{
			...
			if( md->player_bot ) {
				if( md->chat_id ) {
					...
					if( (cd = (struct chat_data*)map_id2bl(md->chat_id)) )
						clif_dispchat(cd,0);
				}
...
void clif_selllist(map_session_data *sd)
{
	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] )
		{
			...
			// [RomuloSM]: Stuff Items & Pack Guild
			if( battle_config.stuff_enable && !(battle_config.stuff_enable_sell&0x1) && itemdb_stuffType(&sd->inventory.u.items_inventory[i]) )
				continue;
...
void clif_dispchat(struct chat_data* cd, int fd)
{
	...
	WBUFW(buf,14) = (cd->owner->type == BL_NPC || cd->owner->type == BL_MOB) ? cd->users+1 : cd->users;
	WBUFB(buf,16) = type;
...
void clif_joinchatok(map_session_data *sd,struct chat_data* cd)
{
	...
	if (!session_isActive(fd))
		return;
	t = (int)(cd->owner->type == BL_NPC || cd->owner->type == BL_MOB);
	...
	}else if(cd->owner->type == BL_MOB) { // [RomuloSM]: Player Bot
		WFIFOL(fd, 30) = 1;
		WFIFOL(fd, 8) = 0;
		safestrncpy(WFIFOCP(fd, 12), ((struct mob_data *)cd->owner)->name, NAME_LENGTH);
		for (i = 0; i < cd->users; i++) {
			WFIFOL(fd, 8+(i+1)*28) = 1;
			safestrncpy(WFIFOCP(fd, 8+(i+t)*28+4), cd->usersd[i]->status.name, NAME_LENGTH);
		}
...
void clif_getareachar_unit( map_session_data* sd,struct block_list *bl ){
	...
	case BL_STALL:
		{
			struct s_stall_data* stl = map_id2st(bl->id);
	...
	case BL_NPC:
		{
			...
			clif_hat_effect_npc(&sd->bl, bl, true, SELF);
		}
		break;
	case BL_MOB:
		{
			...
			// [RomuloSM]: Battleground Expansion
			if (battle_config.bg_expansion_enable && md->bg_id && md->bg_id != sd->bg_id && map_getmapflag(md->bl.m,MF_BATTLEGROUND))
				clif_sendbgmobemblem_single(sd->fd, md);

			// [RomuloSM]: Player Bot
			if( md->player_bot && pc_has_permission(sd, PC_PERM_VIEW_HPMETER) ) {
				if( battle_config.player_bot_hpbar_green )
			...
			if( md->chat_id )
				clif_dispchat((struct chat_data*)map_id2bl(md->chat_id),sd->fd);
			// [RomuloSM]: Mob Hat Effects
		...
		case BL_STALL:
			{
				...
				struct s_stall_data* stl = map_id2st(bl->id);
				clif_stall_showunit(ssd,stl);
				if(stl->type == 0)
					clif_showstallboard(&ssd->bl,stl->vender_id,stl->message);
				else
					clif_buyingstall_entry(&ssd->bl,stl->vender_id,stl->message);
			}
			break;
		default:
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->getMapFlag(MF_PVP) || (sd && sd->duel_group > 0) || (battle_config.bg_expansion_enable && mapdata->getMapFlag(MF_BATTLEGROUND)) || bl->type == BL_PC && ((TBL_PC *)bl)->state.pvpmode)<<0)| // PARTY - Show attack cursor on non-party members (PvP)
...
void clif_goldpc_info( map_session_data& sd ){
	...
	if( battle_config.feature_goldpc_active ){
		...
		p.active = map_getmapflag(sd.bl.m, MF_BATTLEGROUND) || map_getmapflag(sd.bl.m, MF_PVP) || map_getmapflag(sd.bl.m, MF_GVG) ? false:true;
...
void clif_parse_dynamic_npc( int fd, map_session_data* sd ){
	...
	if (sd->npc_id == 0)
	npc_event_do_id("Kalliope::OnClick", sd->status.account_id);
...
void clif_openvendingreq(map_session_data* sd, int num)
{
	...
	if( battle_config.extended_vending_enable && sd->extended_vend.nameid == 0 ) {
		sd->state.prevend = 0;
		return;
	}
...
void clif_showvendingboard( map_session_data& sd, enum send_target target, struct block_list* tbl ){
	...
	if( battle_config.extended_vending_enable && battle_config.extended_vending_showboard && sd.extended_vend.nameid ) {
		...
		strcat(strcat(strcat(strcat(out_msg, "["), item->ename.c_str()), "] "), sd.message);
		safestrncpy( p.storeName, out_msg, sizeof( p.storeName ) );
	}
	else
...
void clif_showstallboard(struct block_list* bl, uint32 stall_id, const char* message)
{
	...
	struct s_stall_data* stl = map_id2st(stall_id);
	if( stl && battle_config.extended_stall_enable && battle_config.extended_stall_showboard && stl->extended_vending ) {
		char out_msg[1024];
		...
		safestrncpy( p.storeName, out_msg, MESSAGE_SIZE );
	}
	else
...
void clif_guild_belonginfo( map_session_data& sd ){
	...
	// [RomuloSM]: Battleground Expansion
	if( battle_config.bg_expansion_enable && sd.bg_id && battle_config.bg_name_position ) {
		if( clif_bg_belonginfo(&sd) )
			return;
	}
...
void clif_guild_positionnamelist(map_session_data *sd)
{
	...
	// [RomuloSM]: Battleground Expansion
	if( battle_config.bg_expansion_enable && sd->bg_id && battle_config.bg_name_position ) {
		if( clif_bg_positionnamelist(sd) )
			return;
	}
...
void clif_guild_positioninfolist(map_session_data *sd)
{
	...
	// [RomuloSM]: Battleground Expansion
	if( battle_config.bg_expansion_enable && sd->bg_id && battle_config.bg_name_position ) {
		if( clif_bg_positioninfolist(sd) )
			return;
	}
...
void clif_name( struct block_list* src, struct block_list *bl, send_target target ){
	...
	switch( bl->type ){
		case BL_PC: {
			...
			// [RomuloSM]: Battleground Expansion
			if( sd->bg_id && battle_config.bg_expansion_enable && battle_config.bg_name_position ) {
				std::shared_ptr<s_battleground_data> bg = util::umap_find(bg_team_db, sd->bg_id);
				if (bg != nullptr && bg->expansion.g != NULL) {
					int position;
					map_session_data *tsd = BL_CAST(BL_PC,src);

					safestrncpy(packet.name, sd->status.name, NAME_LENGTH);

					// Will get the position of the guild the player is in
					position = bg->expansion.master_id == sd->status.char_id ? 0 : 1;
					if( bg->expansion.teamId) {
						safestrncpy( packet.guild_name, msg_txt(tsd,1671), NAME_LENGTH );
						safestrncpy( packet.position_name, msg_txt(tsd,(1675 + position)), NAME_LENGTH );
					}
					else {
						safestrncpy( packet.guild_name, msg_txt(tsd,1665), NAME_LENGTH );
						safestrncpy( packet.position_name, msg_txt(tsd,(1669 + position)), NAME_LENGTH );
					}
		...
		case BL_MOB: {
			...
			// [RomuloSM]: Battleground Expansion
			if( md->bg_id && battle_config.bg_expansion_enable && battle_config.bg_name_position ) {
				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };

				std::shared_ptr<s_battleground_data> bg = util::umap_find(bg_team_db, md->bg_id);

				packet.packet_id = HEADER_ZC_ACK_REQNAMEALL;
				packet.gid = bl->id;
				safestrncpy(packet.name, md->name, NAME_LENGTH);
				if (bg != nullptr && bg->expansion.g != NULL && md->bg_desc[0] != '\0' && strlen(md->bg_desc) > 0 ) {
					map_session_data *tsd = BL_CAST(BL_PC,src);

					safestrncpy( packet.guild_name, msg_txt(tsd, bg->expansion.teamId ? 1671 : 1665), NAME_LENGTH );
					safestrncpy( packet.position_name, md->bg_desc, NAME_LENGTH );
				}
				clif_send(&packet, sizeof(packet), src, target);
				break;
			}
			...
			if( md->guardian_data && md->guardian_data->guild_id ){
				...
				clif_send(&packet, sizeof(packet), src, target);
			} else if (!md->player_bot && battle_config.show_mob_info && !md->chat_clone_id) { // Chat Clone
				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };
				...
				clif_send(&packet, sizeof(packet), src, target);
			} else if( mob_fake_guild(md) ) {
				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };

				packet.packet_id = HEADER_ZC_ACK_REQNAMEALL;
				packet.gid = bl->id;
				safestrncpy( packet.name, md->name, NAME_LENGTH );
				if( md->fake_guild.name[0] != '\0' && strlen(md->fake_guild.name) > 0 )
					safestrncpy( packet.guild_name, md->fake_guild.name, NAME_LENGTH );
				if( md->fake_guild.position[0] != '\0' && strlen(md->fake_guild.position) > 0 )
					safestrncpy( packet.position_name, md->fake_guild.position, NAME_LENGTH );
				...
				if(md->bl.champion_monster){
					...
					if (ud != nullptr) {
					    ...
					    if (ud->group_id) {
							...
							safestrncpy(ud->title, msg_txt(sd,2000+md->bl.champion_monster), NAME_LENGTH);
...
// Define os dados do pacote se ud não for nulo
if (ud != nullptr) {
    memcpy(packet.title, ud->title, NAME_LENGTH);
    packet.groupId = ud->group_id;
}
		...
		case BL_STALL: {
			...
			s_stall_data *stl = (s_stall_data *)bl;

			safestrncpy( packet.name, stl->name, NAME_LENGTH );
			clif_send(&packet, sizeof(packet), src, target);
		}
			break;
		case BL_CHAT:
...
void clif_parse_LoadEndAck(int fd,map_session_data *sd)
{
	...
	// PvP Mode
	if(sd->state.pvpmode){
		map_data *pmapdata = map_getmapdata(sd->state.pmap);
		if(mapdata_nopvpmode(mapdata))
			pc_pvpmodeoff(sd,1,0);
		else if(!mapdata->getMapFlag(MF_FORCED_PVPMODE) && pmapdata->getMapFlag(MF_FORCED_PVPMODE))
			pc_pvpmodeoff(sd,1,0);
	}else if(mapdata->getMapFlag(MF_FORCED_PVPMODE))
		pc_pvpmode(sd);
	...
	if(!pc_isinvisible(sd) && mapdata->getMapFlag(MF_PVP)) {
		...
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
	else if(sd->state.pvpmode)	// PvP Mode
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
	...
	else if( battle_config.bg_expansion_enable && mapdata->getMapFlag(MF_BATTLEGROUND) ) // [RomuloSM]: Battleground Expansion
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
	else
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
...
#ifdef VIP_ENABLE
			// VIP State - force VIP reset on login
	...
	if( battle_config.bg_expansion_enable && sd->bg_id ) {
		...
		if( bgteam != nullptr && bgteam->expansion.g != NULL ) {
			guild_guildaura_refresh(sd, GD_LEADERSHIP, guild_fake_checkskill(bgteam->expansion.g, GD_LEADERSHIP));
			guild_guildaura_refresh(sd, GD_GLORYWOUNDS, guild_fake_checkskill(bgteam->expansion.g, GD_GLORYWOUNDS));
			guild_guildaura_refresh(sd, GD_SOULCOLD, guild_fake_checkskill(bgteam->expansion.g, GD_SOULCOLD));
			guild_guildaura_refresh(sd, GD_HAWKEYES, guild_fake_checkskill(bgteam->expansion.g, GD_HAWKEYES));
		}
	}
	...
	// [RomuloSM]: Force the removal of unauthorized items on a particular map
	// Fix: Stuff Items, Costume System and Pack Guild but works for others.
	if( battle_config.unequip_prohibited_items ) {
		for (int i = 0; i < EQI_MAX; i++) {
	...
	if(battle_config.bg_expansion_enable && sd->bg_id ) {
		std::shared_ptr<s_battleground_data> bgteam = util::umap_find(bg_team_db, sd->bg_id);
		if( bgteam != NULL && bgteam->expansion.g != NULL )
			bg_player_data(sd);
	}
...
void clif_parse_Emotion(int fd, map_session_data *sd)
{
	...
	struct PACKET_CZ_REQ_EMOTION_EXPANSION* p = (struct PACKET_CZ_REQ_EMOTION_EXPANSION*)RFIFOP(fd, 0);
	int emoticon = p->type;
#else
...
void clif_parse_WisMessage(int fd, map_session_data* sd)
{
	...
	if (dstsd == NULL || strcmp(dstsd->status.name, target) != 0) {
		// [RomuloSM]: Player Bot
		if( mob_playerbot_search_nick(target, sd->bl.m) ) {
			clif_wis_end( fd, 0 );
			return;
		}
	...
	if (dstsd->state.afk == 1){
		safesnprintf(output,sizeof(output),"%s is in afk mode and cannot receive whispered messages.", dstsd->status.name);
		clif_wis_message(sd, wisp_server_name, output, strlen(output) + 1, 0);
		return;
	}
...
{
	// [RomuloSM]: Player Bot
	struct block_list *bl = map_id2bl(RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0]));
...
void clif_parse_skill_toid( map_session_data* sd, uint16 skill_id, uint16 skill_lv, int target_id ){
	...
	if( SKILL_CHK_GUILD(skill_id) ) {
		...
		if( battle_config.bg_expansion_enable && sd->bg_id && (bgteam = util::umap_find(bg_team_db, sd->bg_id)) != nullptr && bgteam->expansion.g != NULL )
			skill_lv = guild_fake_checkskill(bgteam->expansion.g, skill_id);
		else
...
void clif_parse_Cooking(int fd,map_session_data *sd) {
	...
	if (sd->state.menu_or_input) {
		...
		if (sd->npc_idle_timer == INVALID_TIMER && !sd->state.ignoretimeout)
			return;
#endif
		sd->npc_amount = p->itemId;                         // Get item number
		if (battle_config.idletime_option & IDLE_NPC_INPUT) // Update idle time
			sd->idletime = last_tick;

		npc_scriptcont(sd, sd->menuskill_val2, false);      // Script continues to execute
		return;
	}
...
void clif_parse_SelectArrow(int fd,map_session_data *sd) {
	// [RomuloSM]: Extended Stall
	if( battle_config.extended_stall_enable && (sd->menuskill_id == ALL_ASSISTANT_VENDING || sd->menuskill_id == ALL_ASSISTANT_BUYING) ) {
	...
	else if( battle_config.extended_searchstore_enable && sd->menuskill_id == EX_SEARCH_STORE ) {
		if( sd->state.vending || sd->state.buyingstore || sd->state.trading ) {
			//Make it fail to avoid shop exploits where you sell something different than you see.
			clif_skill_fail(sd, sd->ud.skill_id, USESKILL_FAIL_LEVEL, 0);
			clif_menuskill_clear(sd);
	...
	else if( battle_config.extended_buyingstore_enable && sd->menuskill_id == EX_BUYING_STORE ) {
		if( sd->state.vending || sd->state.buyingstore || sd->state.trading ) {
			//Make it fail to avoid shop exploits where you sell something different than you see.
			clif_skill_fail(sd, sd->ud.skill_id, USESKILL_FAIL_LEVEL, 0);
			clif_menuskill_clear(sd);
	...
	switch (sd->menuskill_id) {
		...
		case NC_MAGICDECOY:
			...
			break;
		// [RomuloSM]: Player Info
		case CL_PLAYER_INFO:
			skill_playerinfo(sd,p->itemId);
			break;
		// [RomuloSM]: Extended Vending
		case MC_VENDING:
		...
		case ALL_ASSISTANT_BUYING:
			skill_stall(sd,sd->menuskill_id,sd->menuskill_val2,p->itemId);
			break;
		// [RomuloSM]: Extended Search Store
		case EX_SEARCH_STORE:
			searchstore_open(sd, sd->searchstore.uses, sd->searchstore.effect);
			break;
		// [RomuloSM]: Extended Buying Store
		case EX_BUYING_STORE:
			sd->extended_vend.nameid = p->itemId;
			buyingstore_setup(sd, sd->buyingstore.slots);
			break;
	}
...
void clif_parse_PartyInvite(int fd, map_session_data *sd)
{
	...
	// [RomuloSM]: Player Bot
	struct block_list *tsd = map_id2bl(RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0]));
	if( tsd == NULL )
		return;
...
void clif_parse_PartyInvite2(int fd, map_session_data *sd){
	...
	// [RomuloSM]: Player Bot
	map_session_data *tsd = map_nick2sd( name, false );
	if( tsd == NULL ) {
		if( mob_playerbot_search_nick(name, sd->bl.m) ) {
			return;
		}
	}
...
void clif_parse_FriendsListAdd(int fd, map_session_data *sd)
{
	...
	if (f_sd == NULL) {
		// [RomuloSM]: Player Bot
		if( mob_playerbot_search_nick(RFIFOCP(fd,packet_db[RFIFOW(fd,0)].pos[0]), sd->bl.m) )
			return;
...
void clif_parse_ViewPlayerEquip(int fd, map_session_data* sd)
{
	...
	// [RomuloSM]: Player Informations
	int check = 0;
#ifdef GUILD_RANKING
	if( battle_config.guild_ranking_enable )
		check++;
#endif
#ifdef BATTLEGROUND_EXPANSION
	if( battle_config.bg_expansion_enable )
		check++;
#endif

#ifdef RANKED_SYSTEM
	if( battle_config.ranked_enable )
		check++;
#endif

	if( check > 0 ) {
		sd->playerinfo_id = tsd->status.account_id;
		clif_player_info(sd, tsd, 1);
		return;
	}
...
void clif_sendbgemblem_area(map_session_data *sd)
{
	...
	// [RomuloSM]: Battleground Expansion
	if( battle_config.bg_expansion_enable && !(battle_config.bg_enable_cross_swords&0x02) )
		return;
...
void clif_parse_bg_queue_apply_request(int fd, map_session_data *sd)
{
	...
	// [RomuloSM]: Battleground Expansion
	if (battle_config.bg_interface) {
		if(battle_config.feature_bgqueue) {
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],"It is not possible to use Battleground registration through the interface.",false,SELF);
			clif_messagecolor(&sd->bl,color_table[COLOR_RED],"Please use the @bgjoin command to register or try another means of registration.",false,SELF);
			return;
		}
	}
...
void clif_bg_queue_lobby_notify(const char *name, map_session_data *sd)
{
	...
	// [RomuloSM]: Battleground Expansion
	if( sd->bge.queue_silent || battle_config.bg_skip_accept ) {
...
void clif_buyingstore_entry( map_session_data& sd, struct block_list* tbl ){
	...
	// [RomuloSM]: Extended Buyingstore
	if( battle_config.extended_buyingstore_enable && battle_config.extended_buyingstore_showboard && sd.extended_vend.nameid ) {
		char out_msg[1024];
		std::shared_ptr<item_data> item = item_db.find(sd.extended_vend.nameid);
		memset(out_msg, '\0', sizeof(out_msg));
		strcat(strcat(strcat(strcat(out_msg, "["), item->ename.c_str()), "] "), sd.message);
...
void roulette_generate_bonus( map_session_data& sd ){
    if( battle_config.feature_roulette_bonus_reward && sd.roulette.bonusItemID == 0 ){
        ...
        if( sd.roulette_point.bronze > 0 ){
            ...
            next_stage = 2;
        }else if( sd.roulette_point.gold > 1 ){
            next_stage = 4;
...
void clif_parse_roulette_open(int fd, map_session_data* sd){
    ...
    if (sd) {
        ...
        {
            struct npc_data* nd;
            nd = npc_name2id("DOAR_FUNCTOR");
            if (nd == NULL) return; // Corrigido aqui: substitu do break por return
...
#if PACKETVER_MAIN_NUM >= 20160601 || PACKETVER_RE_NUM >= 20160525 || defined(PACKETVER_ZERO)
	if( sd->state.laphine_synthesis == 0 && sd->state.laphine_creation == 0 ){
		return;
	}

	struct PACKET_CZ_REQ_RANDOM_COMBINE_ITEM* p = (struct PACKET_CZ_REQ_RANDOM_COMBINE_ITEM*)RFIFOP( fd, 0 );

	if(sd->state.laphine_creation > 0) {
		clif_parse_laphine_creation(fd, sd, p);
		return;
	}
...
#if PACKETVER_MAIN_NUM >= 20170726 || PACKETVER_RE_NUM >= 20170621 || defined(PACKETVER_ZERO)
	if( sd->state.laphine_upgrade == 0 && sd->state.laphine_option == 0){
		return;
	}

	struct PACKET_CZ_REQ_RANDOM_UPGRADE_ITEM* p = (struct PACKET_CZ_REQ_RANDOM_UPGRADE_ITEM*)RFIFOP( fd, 0 );

	if( sd->state.laphine_option) {
		clif_parse_laphine_option(fd, sd, p);
		return;
	}
...
// Laphine Creation
// ===============
void clif_laphine_creation_open( map_session_data* sd, std::shared_ptr<s_laphine_creation> creation ){
#if PACKETVER_MAIN_NUM >= 20170726 || PACKETVER_RE_NUM >= 20170621 || defined(PACKETVER_ZERO)
	nullpo_retv( sd );

	sd->state.laphine_creation = creation->item_id;

	struct PACKET_ZC_RANDOM_COMBINE_ITEM_UI_OPEN p = {};

	p.packetType = HEADER_ZC_RANDOM_COMBINE_ITEM_UI_OPEN;
	p.itemId = client_nameid( creation->item_id );
...
void clif_parse_laphine_creation( int fd, map_session_data* sd, struct PACKET_CZ_REQ_RANDOM_COMBINE_ITEM* p ){
#if PACKETVER_MAIN_NUM >= 20160601 || PACKETVER_RE_NUM >= 20160525 || defined(PACKETVER_ZERO)

	if( sd->state.laphine_creation != p->itemId ){
		return;
	}

	int nameid = sd->state.laphine_creation;

	std::shared_ptr<s_laphine_creation> creation = laphine_creation_db.find( sd->state.laphine_creation );
	...
	if( count != creation->requiredRequirements ){
		return;
	}
	...
	int diagram = 0, element = 0, anvil = 0, flag = 0;
	int wlv = itemdb_wlv(sd->state.laphine_creation);
	int make_per = 5000 + status->dex*8 + status->luk*8;
	int requirements_cnt = creation->requiredRequirements;
	static const int anvil_table[4] = { 0, 300, 500, 1000 };
	static const int ele_table[5] = { ELE_NEUTRAL, ELE_FIRE, ELE_WATER, ELE_WIND, ELE_EARTH };

	int optionidx = sd->state.laphine_optionidx;

	if(sd->inventory_data[optionidx] == nullptr || sd->inventory.u.items_inventory[optionidx].nameid != nameid){
		clif_laphine_synthesis_result( sd, LAPHINE_SYNTHESIS_ITEM );
		return;
	}
	...
	for( size_t i = 0; i < count; i++ ){
		...
		std::shared_ptr<s_laphine_creation_requirement> requirement = util::umap_find( creation->requirements, item->nameid );
		...
		if(item->nameid >= ITEMID_ANVIL && item->nameid <= ITEMID_EMPERIUM_ANVIL){
			if(anvil){
				clif_laphine_synthesis_result( sd, LAPHINE_SYNTHESIS_AMOUNT );
				return;
			}
		...
		if(item->nameid >= ITEMID_NEUTRAL_REACTOR && item->nameid <= ITEMID_EARTH_REACTOR){
			if(element){
				clif_laphine_synthesis_result( sd, LAPHINE_SYNTHESIS_AMOUNT );
				return;
			}
	...
	if(!anvil || !element || !diagram){
		clif_laphine_synthesis_result( sd, LAPHINE_SYNTHESIS_AMOUNT );
		return;
	}

	if( pc_delitem( sd, optionidx, 1, 0, 0, LOG_TYPE_LAPHINE ) != 0 ){
		return;
	}
	...
	make_per += anvil_table[anvil - ITEMID_ANVIL];
	make_per += pc_checkskill(sd, BS_ORIDEOCON) * 100;

	if(pc_famerank(sd->status.char_id, sd->status.class_)){	
		make_per += 1000;
	}
	make_per -= ( wlv * 1000 );
	...
	if(diagram >= 51004 && diagram <= 51007){
		make_per += 20000;
	}

	if (make_per < 1) make_per = 1;

	if (rnd()%10000 < make_per){ //Success, or crafting multiple items.
		...
		tmp_item.card[1] = ele_table[element - ITEMID_NEUTRAL_REACTOR];

		group_chance = rnd()%10000;
		...
		if(group_chance > 9900) tmp_item.enchantgrade = 4;
		else if(group_chance > 9000) tmp_item.enchantgrade = 3;
		else if(group_chance > 7000) tmp_item.enchantgrade = 2;	
		else tmp_item.enchantgrade = 1;	

		int randomoption_id = creation->randomOptionGroups[tmp_item.enchantgrade-1];

		std::shared_ptr<s_random_opt_group> randomOption = random_option_group.find( randomoption_id );

		if (randomOption == nullptr) {
			return;
		}

		randomOption->apply( tmp_item );
		...
		pc_addfame(*sd, wlv * 3);
		 ...
		 if (tmp_item.amount) { //Success
			...
			if(tmp_item.enchantgrade == 4){
				std::string itemlstr = item_db.create_item_link( tmp_item );
				char message[256];
				sprintf (message, "[Alfheim]: O jogador %s acaba de criar o item %s lend rio!", sd->status.name, itemlstr.c_str());
...
void clif_laphine_option_open( map_session_data* sd, std::shared_ptr<s_laphine_options> option ){
#if PACKETVER_MAIN_NUM >= 20170726 || PACKETVER_RE_NUM >= 20170621 || defined(PACKETVER_ZERO)
	nullpo_retv( sd );

	sd->state.laphine_option = option->skill_id;

	struct PACKET_ZC_RANDOM_UPGRADE_ITEM_UI_OPEN p = {};

	p.packetType = HEADER_ZC_RANDOM_UPGRADE_ITEM_UI_OPEN;
	p.itemId = client_nameid( option->skill_id );

	clif_send( &p, sizeof( p ), &sd->bl, SELF );
#endif
}

void clif_parse_laphine_option( int fd, map_session_data* sd,struct PACKET_CZ_REQ_RANDOM_UPGRADE_ITEM* p ){
#if PACKETVER_MAIN_NUM >= 20170726 || PACKETVER_RE_NUM >= 20170621 || defined(PACKETVER_ZERO)


	if( sd->state.laphine_option != p->itemId ){
		return;
	}

	std::shared_ptr<s_laphine_options> option = laphine_options_db.find( sd->state.laphine_option );
	...
	// Not a valid target item
	if( !util::vector_exists( option->target_item_ids, item->nameid ) ){
		clif_laphine_upgrade_result( sd, true );
		return;
	}
	...
	std::shared_ptr<s_laphine_creation> creation = laphine_creation_db.find( item->nameid );

	if(creation == nullptr){
		return;
	}

	sd->state.laphine_optionidx = index;

	clif_laphine_upgrade_result( sd, false );
	clif_laphine_creation_open(sd, creation);
...
void clif_parse_enchantwindow_upgrade_random(int fd, map_session_data *sd)
{
...
// [RomuloSM]: Fake Guild Emblem

/// Sends emblems bitmap data to the client that requested it (ZC_GUILD_EMBLEM_IMG).
/// 0152 <packet len>.W <guild id>.L <emblem id>.L <emblem data>.?B
void clif_fake_guild_emblem(map_session_data *sd,struct guild *g)
{
	int fd;
	nullpo_retv(sd);
	nullpo_retv(g);
...
int clif_visual_guild_id(struct block_list* bl)
{
	...
	// always before BG Expansion
	if( battle_config.ranked_enable && !ranked_getbl_disable(bl) && ranked_show_map(bl->m,RANK_SHOW_MAP) && (ranked_getbl_status(bl) == RANK_SHOW_RANKED || (!bg_team_get_id(bl) && !status_get_guild_id(bl))) )
	{
		int rank_id = ranked_getbl_rank(bl,(enum ranked_mode)battle_config.ranked_show_mode);
		std::shared_ptr<s_ranked_db> rank = ranked_search(rank_id);
		if( rank != nullptr && rank->g != NULL )
			return rank->g->guild_id;
	}
#endif
#ifdef BATTLEGROUND_EXPANSION
	// [RomuloSM]: Battleground Expansion
	if ( battle_config.bg_expansion_enable ) {
		...
		if (bg != nullptr && bg->expansion.g != NULL) {
			return bg->expansion.g->guild_id;
		}
...
int clif_visual_emblem_id(struct block_list* bl)
{
	...
	if( battle_config.bg_expansion_enable ) {
		...
		std::shared_ptr<s_battleground_data> bg = util::umap_find(bg_team_db, bg_id);
		if (bg != nullptr && bg->expansion.g != NULL) {
			return bg->expansion.g->emblem_id;
		}
	}
#endif
...
// [RomuloSM]: Battleground Expansion
// Send Cross Sword icon to the battleground monster
void clif_sendbgmobemblem_single(int fd, struct mob_data *md)
{
	nullpo_retv(md);

	if( battle_config.bg_expansion_enable && !(battle_config.bg_enable_cross_swords&0x01) )
		return;
...
// Send Fake Guild Basicinfo to the player in Battleground
bool clif_bg_basicinfo(map_session_data *sd)
{
	int fd;
	...
	if ( !sd->bg_id || (bg = util::umap_find(bg_team_db, sd->bg_id)) == NULL || bg->expansion.g == NULL )
		return false;

	WFIFOHEAD(fd, packet_len(cmd));
	WFIFOW(fd, 0) = cmd;
	WFIFOL(fd, 2) = bg->expansion.g->guild_id;
	WFIFOL(fd, 6) = bg->expansion.g->guild_lv;
	WFIFOL(fd,10) = bg->members.size();
	WFIFOL(fd,14) = bg->expansion.g->max_member;
	WFIFOL(fd,18) = 0;
	WFIFOL(fd,22) = 0;
	WFIFOL(fd,26) = 0;
	...
	WFIFOL(fd,42) = bg->expansion.g->emblem_id;
	safestrncpy(WFIFOCP(fd,46), bg->expansion.g->name, NAME_LENGTH);
#if PACKETVER < 20160622
	safestrncpy(WFIFOCP(fd,70), bg->warzone.g->master, NAME_LENGTH);
#endif
	safestrncpy(WFIFOCP(fd,70+offset), bg->expansion.g->master, 16);
	WFIFOL(fd,70+offset+16) = 0;
#if PACKETVER >= 20160622
	WFIFOL(fd,70+offset+20) = bg->expansion.master_id;  // leader
#endif
...
// Send Fake Guild info to the player in the Battleground
bool clif_bg_belonginfo(map_session_data *sd)
{
	int fd;
	std::shared_ptr<s_battleground_data> bg = NULL;
	...
	WFIFOW(fd, 0) = 0x16c;
	WFIFOL(fd, 2) = bg->expansion.g->guild_id;
	WFIFOL(fd, 6) = bg->expansion.g->emblem_id;
	WFIFOL(fd, 10) = 0x10;
	WFIFOB(fd, 14) = 0;
	WFIFOL(fd, 15) = 0;  // InterSID (unknown purpose)
	memcpy(WFIFOP(fd, 19), bg->expansion.g->name, NAME_LENGTH);
	WFIFOSET(fd, packet_len(0x16c));
	return true;
}

// Send Fake Guild member list to the player in the Battleground
bool clif_bg_memberlist(map_session_data *sd)
{
	int fd;
	int c = 0;
	...
	for (auto &pl_sd : bg->members) {
		...
		WFIFOL(fd, c*size+30) = bg->expansion.master_id == psd->status.char_id ? 0 : 1;
#if PACKETVER < 20161026
...
#else
		WFIFOL(fd,c*size+34)=(uint32)time(NULL);
#endif
		c++;
	}
...
// Send Fake Guild position name list to the player in the Battleground
bool clif_bg_positionnamelist(map_session_data *sd)
{
	int fd;
	std::shared_ptr<s_battleground_data> bg = NULL;
	...
	if( bg->expansion.teamId ) {
		WFIFOL(fd,0*28+4)=0;
		safestrncpy(WFIFOCP(fd,0*28+8),msg_txt(sd,1675),NAME_LENGTH);
		WFIFOL(fd,1*28+4)=1;
		safestrncpy(WFIFOCP(fd,1*28+8),msg_txt(sd,1676),NAME_LENGTH);
	}
	else {
		WFIFOL(fd,0*28+4)=0;
		safestrncpy(WFIFOCP(fd,0*28+8),msg_txt(sd,1669),NAME_LENGTH);
		WFIFOL(fd,1*28+4)=1;
		safestrncpy(WFIFOCP(fd,1*28+8),msg_txt(sd,1670),NAME_LENGTH);
	}
	WFIFOW(fd,2)=2*28+4;
	WFIFOSET(fd,WFIFOW(fd,2));
	return true;
...
// Send Fake Guild position info list to the player in the Battleground
bool clif_bg_positioninfolist(map_session_data *sd)
{
	int i,fd;
	std::shared_ptr<s_battleground_data> bg = NULL;
	...
	WFIFOW(fd, 0)=0x160;
	for(i=0;i<=1;i++){
		struct guild_position_f *p=&bg->expansion.g->position[i];
		WFIFOL(fd,i*16+ 4)=i;
		WFIFOL(fd,i*16+ 8)=p->mode;
		WFIFOL(fd,i*16+12)=i;
		WFIFOL(fd,i*16+16)=p->exp_mode;
...
// Send Fake Guild skill list to the player in the Battleground
bool clif_bg_skillinfo(map_session_data *sd)
{
	int fd;
	std::shared_ptr<s_battleground_data> bg = NULL;
	...
	for(i = 0, c = 0; i < MAX_GUILDSKILL; i++)
	{
		if(bg->expansion.g->skill[i].id > 0 && guild_fake_check_skill_require(bg->expansion.g, bg->expansion.g->skill[i].id))
		{
			int id = bg->expansion.g->skill[i].id;
			int p = 6 + c*37;
			WFIFOW(fd,p+0) = id;
			WFIFOL(fd,p+2) = skill_get_inf(id);
			WFIFOW(fd,p+6) = bg->expansion.g->skill[i].lv;
			WFIFOW(fd,p+8) = skill_get_sp(id, bg->expansion.g->skill[i].lv);
			WFIFOW(fd,p+10) = skill_get_range(id, bg->expansion.g->skill[i].lv);
			safestrncpy(WFIFOCP(fd,p+12), skill_get_name(id), NAME_LENGTH);
			WFIFOB(fd,p+36)= (bg->expansion.g->skill[i].lv < guild_skill_get_max(id) && sd->status.char_id == bg->expansion.master_id) ? 1 : 0;
			c++;
		}
	}
	WFIFOW(fd,2) = 6 + c*37;
...
// Send Fake Guild notice to the player in the Battleground
bool clif_bg_notice(map_session_data *sd)
{
	int fd;
	std::shared_ptr<s_battleground_data> bg = NULL;
	...
	if(bg->expansion.g->mes1[0] == '\0' && bg->expansion.g->mes2[0] == '\0')
		return false;

	sd->bge.guild_notice = true;
	WFIFOHEAD(fd,packet_len(0x16f));
	WFIFOW(fd,0) = 0x16f;
	memcpy(WFIFOP(fd,2), bg->expansion.g->mes1, MAX_GUILDMES1);
	memcpy(WFIFOP(fd,62), bg->expansion.g->mes2, MAX_GUILDMES2);
	WFIFOSET(fd,packet_len(0x16f));
	return true;
}
 ...
 * [RomuloSM]: Player Info
 **/
int clif_player_info(map_session_data* sd, map_session_data* tsd, int skill_lv) {
	...
	if( battle_config.playerinfo_equip_item && item_db.find(battle_config.playerinfo_equip_item) != nullptr ) {
		p->items[count].itemId = client_nameid(battle_config.playerinfo_equip_item);
		count++;
	}

#ifdef GUILD_RANKING
	// [RomuloSM]: Guild Ranking
	if(battle_config.guild_ranking_enable && battle_config.playerinfo_guild_item && itemdb_exists(battle_config.playerinfo_guild_item) ) {
		p->items[count].itemId = client_nameid(battle_config.playerinfo_guild_item);
		count++;
	}
#endif
...
#ifdef BATTLEGROUND_EXPANSION
	// [RomuloSM]: Battleground Expansion
	if( battle_config.bg_expansion_enable && battle_config.playerinfo_bg_item && item_db.find(battle_config.playerinfo_bg_item) != nullptr) {
		p->items[count].itemId = client_nameid(battle_config.playerinfo_bg_item);
		count++;
	}
#endif
...
#ifdef RANKED_SYSTEM
	// [RomuloSM]: Ranked System
	if( battle_config.ranked_enable && battle_config.playerinfo_ranked_item && item_db.find(battle_config.playerinfo_ranked_item) != nullptr ) {
		p->items[count].itemId = client_nameid(battle_config.playerinfo_ranked_item);
		count++;
	}
#endif
	...
	if( count > 0 ){
		sd->menuskill_id = CL_PLAYER_INFO;
		sd->menuskill_val = skill_lv;
	}
	else {
		if (sd->bl.m != tsd->bl.m)
...
int clif_equiphide(struct block_list *bl, va_list ap) {
	struct block_list *tbl;
	struct view_data *vd;
	TBL_PC *sd, *tsd;
	tbl=va_arg(ap,struct block_list*);
	...
	if (tsd && tsd->fd) {
		if (sd != tsd) {
			if (tbl->type == BL_PC && tsd->state.equiphide) {
				if (tsd->state.equiphide&2) {
	...
	if (sd && sd->fd) {
		if (sd != tsd) {
			if (bl->type == BL_PC && sd->state.equiphide) {
				if (sd->state.equiphide&2) {
...
void clif_stall_vending_open(map_session_data *sd){
	...
	if(stall_isStallOpen(sd->status.char_id)){
		...
		clif_displaymessage(sd->fd, msg_txt(sd,1950));
		return;
	}

	if(map_getmapflag(sd->bl.m, MF_NOSTALL)) {
		clif_stall_ui_close(sd,100,STALLSTORE_OK);
		clif_displaymessage(sd->fd, msg_txt(sd,1952));
		return;
	}
...
void clif_stall_showunit(map_session_data *sd, struct s_stall_data *stl){
#if PACKETVER >= 20170208
	nullpo_retv( sd );
	nullpo_retv( stl );
	...
	p.vender_id = stl->vender_id;
	p.job = stl->vd.class_;
	p.xPos = stl->bl.x;
	p.yPos = stl->bl.y;
	p.sex = stl->vd.sex;
	p.head = stl->vd.hair_style;
	p.weapon = stl->vd.weapon;
	p.shield = stl->vd.shield;
	p.MidAccessory = stl->vd.head_mid;
	p.TopAccessory = stl->vd.head_top;
	p.BottomAccessory = stl->vd.head_bottom;
	p.headpalette = stl->vd.hair_color;
	p.bodypalette = stl->vd.cloth_color;
	p.BackAccessory = stl->vd.body_style;
	safestrncpy(p.name, stl->message, NAME_LENGTH);
	p.body = stl->vd.class_; // needed to display the correct class on 2024 clients
...
void clif_stall_vending_list(map_session_data *sd, s_stall_data *stl){
#if PACKETVER >= 20170208
	nullpo_retv( sd );
	...
	p->PacketLength = len;
	p->unique_id = stl->char_id;
	p->vender_id = stl->vender_id;
	if(stl->char_id == sd->status.char_id)
		p->myStall = 1;
	else
		p->myStall = 0;
	p->expireTime = stl->expire_time ? static_cast<int>((stl->expire_time - time(NULL)) * 1000) : 0 ; // 0: unlimited on client

	short slot = 0;
	for(int i = 0; i < stl->vend_num ; i++){
		if(stl->items_inventory[i].amount > 0){
			struct item_data* data = itemdb_search( stl->items_inventory[i].nameid );

			p->items[slot].price = stl->price[i];
			p->items[slot].amount = stl->items_inventory[i].amount;
			p->items[slot].index = i+1;
			p->items[slot].itemType = itemtype( stl->items_inventory[i].nameid );
			p->items[slot].itemId = client_nameid( stl->items_inventory[i].nameid );
			p->items[slot].identified = stl->items_inventory[i].identify;
			p->items[slot].damaged = stl->items_inventory[i].attribute;
			clif_addcards( &p->items[slot].slot, &stl->items_inventory[i] );
			clif_add_random_options( p->items[slot].option_data, stl->items_inventory[i] );
			p->items[slot].location = pc_equippoint_sub( sd, data );
			p->items[slot].viewSprite = data->look;
			p->items[slot].refine = stl->items_inventory[i].refine;
			p->items[slot].enchantgrade = stl->items_inventory[i].enchantgrade;
			slot++;
		}
	}
...
void clif_stall_buying_list(map_session_data *sd, s_stall_data *stl){
#if PACKETVER >= 20170208
	nullpo_retv( sd );
	...
	for(int i = 0; i < stl->vend_num ; i++){
		if(stl->amount[i] > 0){
			p->items[k].price = stl->price[i];
			p->items[k].amount = stl->amount[i];
			p->items[k].itemType = itemtype( stl->itemId[i] );
			p->items[k].itemId = stl->itemId[i];

			total_price += stl->price[i] * stl->amount[i];
			k++;
		}
	}
...
// Close from gm
void clif_parse_stall_close_from_gm( int fd, map_session_data* sd ){
#if PACKETVER >= 20170208
	const struct PACKET_CZ_STALL_CLOSE_FROM_GM *p = (struct PACKET_CZ_STALL_CLOSE_FROM_GM *)RFIFOP( fd, 0 );

	if(sd->status.char_id)
		stall_close_from_gm(p->vender_id);
#endif
}
 ...
 * [RomuloSM] Extended Stall
 **/
int clif_stall(map_session_data *sd, int type, int slots) {
	nullpo_ret( sd );
	...
	uint16 skill_id = type ? ALL_ASSISTANT_BUYING : ALL_ASSISTANT_VENDING;
	if( count > 0 ){
		sd->menuskill_id = skill_id;
		sd->menuskill_val = count;
		sd->menuskill_val2 = slots;
	}
	else {
		clif_skill_fail(sd, skill_id, USESKILL_FAIL_LEVEL, 0);
		return 0;
 ...
 * [RomuloSM] Extended Buying Store
 **/
int clif_buyingstore(map_session_data *sd, int skill_lv, int slots) {
	nullpo_ret( sd );
	...
	if( count > 0 ){
		sd->menuskill_id = EX_BUYING_STORE;
		sd->menuskill_val = count;
		...
		clif_skill_fail(sd, EX_BUYING_STORE, USESKILL_FAIL_LEVEL, 0);
		return 0;
	}
	return 1;
}
