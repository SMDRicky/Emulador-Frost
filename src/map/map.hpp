...
struct spawn_data {
	...
	char bg_desc[NAME_LENGTH]; // [RomuloSM]: Battleground Expansion
};
...
enum e_mapflag : int16 {
	...
	MF_GVG_CONSUME,
	MF_PVP_CONSUME,
	MF_NO_PVPMODE,	// PvP Mode
	MF_FORCED_PVPMODE, // PvP Mode
	MF_NOSTALL,
	MF_MAX
};
...
enum cell_t{
	...
	CELL_NOBGBATTLE,		// [RomuloSM]: No Battleground Expansion vs in cell.
	CELL_NOBGSKILLCALL,		// [RomuloSM]: No Battleground Expansion emergency call.
...
enum cell_chk : uint8 {
	...
	CELL_CHKNOBGBATTLE,		// [RomuloSM] No Battleground Expansion vs in cell.
	CELL_CHKNOBGSKILLCALL,	// [RomuloSM] No Battleground Expansion emergency call.

	CELL_CHKNOSTALL,		// Whether the cell denies stall
};
...
struct mapcell
{
	...
	// [RomuloSM]: Battleground Expansion
	unsigned char bg_battle : 1;
	unsigned char bg_skillcall : 1;
};
...
// PvP Mode
inline bool mapdata_nopvpmode(struct map_data *mapdata) {
	if (mapdata == nullptr)
		return false;

	if (mapdata->getMapFlag(MF_NO_PVPMODE) || mapdata->getMapFlag(MF_GVG) || mapdata->getMapFlag(MF_PVP) || mapdata->getMapFlag(MF_GVG_CASTLE) || mapdata->getMapFlag(MF_BATTLEGROUND))
		return true;
